...
namespace Audio
{
	...
	bool FMixerPlatformAudioUnit::InitializeHardware()
	{
		...
		Success = [AudioSession setIntendedSpatialExperience:AVAudioSessionSpatialExperienceBypassed options:nil error:&error];
		if (!Success)
		{
			UE_LOG(LogAudioMixerAudioUnit, Display, TEXT("Error while setting spatial experience to bypass for VisionOS."));
		}
#endif
	...
	bool FMixerPlatformAudioUnit::OpenAudioStream(const FAudioMixerOpenStreamParams& Params)
	{
		...
		FCoreDelegates::AudioInterruptionDelegate.AddRaw(this, &FMixerPlatformAudioUnit::InterruptContext);
	...
	void FMixerPlatformAudioUnit::InterruptContext(bool Interrupted)
	{
		if (Interrupted)
		{
