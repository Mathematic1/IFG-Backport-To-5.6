...
struct FVulkanVertexDeclarationKey
{
		...
		: VertexElements(InElements)
	{
		...
		FVertexDeclarationElementList SortedElements = VertexElements;
		SortedElements.Sort([](FVertexElement const& A, FVertexElement const& B)
		{
		...
		HashNoStride = 0;
		for (int32 ElementIndex = 0; ElementIndex < SortedElements.Num(); ElementIndex++)
		{
			FVertexElement Element = SortedElements[ElementIndex];
			Element.Stride = 0;
