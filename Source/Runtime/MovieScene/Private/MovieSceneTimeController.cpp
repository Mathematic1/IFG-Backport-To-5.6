...
{
	UE_LOG(LogMovieScene, VeryVerbose, TEXT("TimeController PrepareToPlay: Sequence started preparing: frame %d, subframe %f. Frame rate: %f fps."), InStartTime.Time.FrameNumber.Value, InStartTime.Time.GetSubFrame(), InStartTime.Rate.AsDecimal());

	OnPrepareToPlay(InStartTime);
}

bool FMovieSceneTimeController::IsReadyToPlay()
{
	return OnIsReadyToPlay();
}
