...
private:
	...
	struct FComponentDependencies
	{
		FTaskBitSet ReadTasks;
		FTaskID WriteTask;
	};
	/** Map that defines tasks that read from/write to specific components on specific allocations. */
	TMap<TPair<int32, FComponentTypeID>, FComponentDependencies> ComponentDepedenciesByAllocation;
