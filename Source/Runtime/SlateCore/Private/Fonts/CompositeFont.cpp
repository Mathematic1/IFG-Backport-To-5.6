...
void FFontFaceData::SetData(TArray<uint8>&& InData)
{
	UntrackMemoryUsage();
	Data = MoveTemp(InData);
	PreprocessedFontGeometries.Reset();
	TrackMemoryUsage();
}
...
{
	UntrackMemoryUsage();
	PreprocessedFontGeometries = MoveTemp(InPreprocessedFontGeometries);
	TrackMemoryUsage();
}

SIZE_T FFontFaceData::GetAllocatedSize() const
{
	SIZE_T TotalSize = Data.GetAllocatedSize()+PreprocessedFontGeometries.GetAllocatedSize();
	for (const UE::Slate::FPreprocessedFontGeometry &SubFaceGeometry : PreprocessedFontGeometries)
	{
		TotalSize += SubFaceGeometry.GetAllocatedSize();
	}
	return TotalSize;
}
...
void FFontFaceData::Serialize(FArchive& Ar)
{
	...
	Ar << Data;
	if (Ar.CustomVer(FFortniteMainBranchObjectVersion::GUID) >= FFortniteMainBranchObjectVersion::AddedPreprocessedFontGeometry)
	{
		Ar << PreprocessedFontGeometries;
	}
...
{
	INC_DWORD_STAT_BY(STAT_SlateRawFontDataMemory, GetAllocatedSize());
}

void FFontFaceData::UntrackMemoryUsage() const
{
	DEC_DWORD_STAT_BY(STAT_SlateRawFontDataMemory, GetAllocatedSize());
}
...
{
	if (FontFaceAsset && IsSlateSdfTextFeatureEnabled())
	{
		const IFontFaceInterface* FontFace = CastChecked<const IFontFaceInterface>(FontFaceAsset);
