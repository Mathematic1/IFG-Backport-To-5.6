...
float FSlateStyleSet::GetFloat(const FName PropertyName, const ANSICHAR* Specifier, float DefaultValue, const ISlateStyle* RequestingStyle) const
{
	...
	// In case the required property is not available, and no DefaultValue was provided, let's log a Warning
	if (Result == nullptr && DefaultValue == FStyleDefaults::GetFloat())
	{
		const ISlateStyle* ReportingStyle = RequestingStyle != nullptr ? RequestingStyle : this;
		ReportingStyle->LogMissingResource(EStyleMessageSeverity::Warning, FText::Format(NSLOCTEXT("SlateStyleSet", "UnknownSlateFloat", "Unable to find float property '{0}' in style."), FText::FromName(PropertyName)), PropertyName);
	}

	return Result ? *Result : DefaultValue;
}

UE::Slate::FDeprecateVector2DResult FSlateStyleSet::GetVector(const FName PropertyName, const ANSICHAR* Specifier, UE::Slate::FDeprecateVector2DParameter DefaultValue, const ISlateStyle* RequestingStyle) const
{
	...
	// In case the required property is not available, and no DefaultValue was provided, let's log a Warning
	if (Result == nullptr && DefaultValue == FStyleDefaults::GetVector2D())
	{
		const ISlateStyle* ReportingStyle = RequestingStyle != nullptr ? RequestingStyle : this;
		ReportingStyle->LogMissingResource(EStyleMessageSeverity::Warning, FText::Format(NSLOCTEXT("SlateStyleSet", "UnknownSlateVector", "Unable to find vector property '{0}' in style."), FText::FromName(PropertyName)), PropertyName);
	}

	return Result ? FVector2f(*Result) : DefaultValue;
}
...
const FLinearColor& FSlateStyleSet::GetColor(const FName PropertyName, const ANSICHAR* Specifier, const FLinearColor& DefaultValue, const ISlateStyle* RequestingStyle) const
{
	...
	// In case the required property is not available, and no DefaultValue was provided, let's log a Warning
	if (Result == nullptr && DefaultValue == FStyleDefaults::GetColor())
	{
		const ISlateStyle* ReportingStyle = RequestingStyle != nullptr ? RequestingStyle : this;
		ReportingStyle->LogMissingResource(EStyleMessageSeverity::Warning, FText::Format(NSLOCTEXT("SlateStyleSet", "UnknownColor", "Unable to find Color '{0}'."), FText::FromName(LookupName)), LookupName);
	}
...
const FSlateColor FSlateStyleSet::GetSlateColor(const FName PropertyName, const ANSICHAR* Specifier, const FSlateColor& DefaultValue, const ISlateStyle* RequestingStyle) const
{
	...
	// In case the required property is not available, and no DefaultValue was provided, let's log a Warning
	if (Result == nullptr && LinearResult == nullptr && DefaultValue == FStyleDefaults::GetSlateColor())
	{
		const ISlateStyle* ReportingStyle = RequestingStyle != nullptr ? RequestingStyle : this;
		ReportingStyle->LogMissingResource(EStyleMessageSeverity::Warning, FText::Format(NSLOCTEXT("SlateStyleSet", "UnknownSlateColor", "Unable to find SlateColor '{0}'."), FText::FromName(StyleName)), StyleName);
	}
...
const FMargin& FSlateStyleSet::GetMargin(const FName PropertyName, const ANSICHAR* Specifier, const FMargin& DefaultValue, const ISlateStyle* RequestingStyle) const
{
	...
	// In case the required property is not available, and no DefaultValue was provided, let's log a Warning
	if (Result == nullptr && DefaultValue == FStyleDefaults::GetMargin())
	{
		const ISlateStyle* ReportingStyle = RequestingStyle != nullptr ? RequestingStyle : this;
		ReportingStyle->LogMissingResource(EStyleMessageSeverity::Warning, FText::Format(NSLOCTEXT("SlateStyleSet", "UnknownMargin", "Unable to find Margin '{0}'."), FText::FromName(StyleName)), StyleName);
	}
...
const FSlateBrush* FSlateStyleSet::GetBrush(const FName PropertyName, const ANSICHAR* Specifier, const ISlateStyle* RequestingStyle) const
{
	...
	{
		const ISlateStyle* ReportingStyle = RequestingStyle != nullptr ? RequestingStyle : this;
		ReportingStyle->LogMissingResource(EStyleMessageSeverity::Warning, FText::Format(NSLOCTEXT("SlateStyleSet", "UnknownBrush", "Unable to find Brush '{0}'."), FText::FromName(StyleName)), StyleName);
	}
...
const FSlateSound& FSlateStyleSet::GetSound(const FName PropertyName, const ANSICHAR* Specifier, const ISlateStyle* RequestingStyle) const
{
	...
	{
		const ISlateStyle* ReportingStyle = RequestingStyle != nullptr ? RequestingStyle : this;
		ReportingStyle->LogMissingResource(EStyleMessageSeverity::Warning, FText::Format(NSLOCTEXT("SlateStyleSet", "UnknownSound", "Unable to find Sound '{0}'."), FText::FromName(StyleName)), StyleName);
	}
