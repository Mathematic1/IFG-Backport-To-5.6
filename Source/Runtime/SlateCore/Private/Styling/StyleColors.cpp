...
bool USlateThemeManager::ExportCurrentThemeTo(const FString& Filename)
{
	FString Output;
	if (!WriteCurrentTheme(Output))
	{
		return false;
	}
...
bool USlateThemeManager::WriteCurrentTheme(FString& OutString)
{
	const FStyleTheme& CurrentTheme = GetCurrentTheme();

	TSharedRef<TJsonWriter<>> WriterRef = TJsonWriterFactory<>::Create(&OutString);
