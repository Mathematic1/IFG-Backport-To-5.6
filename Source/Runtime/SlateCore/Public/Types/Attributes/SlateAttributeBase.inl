...
namespace SlateAttributePrivate
{
	...
	public:
		...
		bool Assign(ContainerType& Widget, const TAttribute<ObjectType>& OtherAttribute, ESlateAttributeBindAction Actions = UE_SLATE_WITH_ATTRIBUTE_DEFAULT_INITIALIZATION_ACTION)
		{
			...
			{
				return AssignBinding(Widget, OtherAttribute.GetBinding(), Actions);
			}
		...
		bool Assign(ContainerType& Widget, TAttribute<ObjectType>&& OtherAttribute, ESlateAttributeBindAction Actions = UE_SLATE_WITH_ATTRIBUTE_DEFAULT_INITIALIZATION_ACTION)
		{
			...
			if (OtherAttribute.IsBound())
			{
				return AssignBinding(Widget, MoveTemp(OtherAttribute.Steal().template Get<FGetter>()), Actions);
			}
			else if (OtherAttribute.IsSet())
			{
	...
	private:
		void ConstructWrapper(ContainerType& Widget, const FGetter& Getter, ESlateAttributeBindAction Actions = UE_SLATE_WITH_ATTRIBUTE_DEFAULT_INITIALIZATION_ACTION)
		{
			...
			if (EnumHasAllFlags(Actions, ESlateAttributeBindAction::Initialize))
			{
				ProtectedUpdateNow(Widget, InAttributeType);
			}
		}

		void ConstructWrapper(ContainerType& Widget, FGetter&& Getter, ESlateAttributeBindAction Actions = UE_SLATE_WITH_ATTRIBUTE_DEFAULT_INITIALIZATION_ACTION)
		{
		...
		bool AssignBinding(ContainerType& Widget, const FGetter& Getter, ESlateAttributeBindAction Actions = UE_SLATE_WITH_ATTRIBUTE_DEFAULT_INITIALIZATION_ACTION)
		{
			...
			{
				ConstructWrapper(Widget, Getter, Actions);
				return true;
			}
			return false;
		}

		bool AssignBinding(ContainerType& Widget, FGetter&& Getter, ESlateAttributeBindAction Actions = UE_SLATE_WITH_ATTRIBUTE_DEFAULT_INITIALIZATION_ACTION)
		{
			const FDelegateHandle PreviousGetterHandle = ProtectedFindGetterHandle(Widget, InAttributeType);
			if (PreviousGetterHandle != Getter.GetHandle())
			{
				ConstructWrapper(Widget, MoveTemp(Getter), Actions);
				return true;
			}
			return false;
		}
