...
public:
	 ...
	 * Allows customization of whether a warning should be logged.
	 * @return The found widget style or the default value.
	 */
	...
	/**
	 * Locates a widget style of a specified type and name.
	 * @param DefaultValue The value to use if a widget style could not be found. If null and no style could be found, a warning will be logged.
	 * @return The found widget style or the default value.
	 */
	template< typename WidgetStyleType >
    ...
    {
    	return GetWidgetStyle< WidgetStyleType >( PropertyName, Specifier, DefaultValue, DefaultValue == nullptr );
    }
	...
	{
		const FSlateWidgetStyle* ExistingStyle = GetWidgetStyleInternal(WidgetStyleTypeName, Join( PropertyName, Specifier ), DefaultValue, bWarnIfNotFound);
		...
		return ExistingStyle ? ExistingStyle : DefaultValue;
	}
