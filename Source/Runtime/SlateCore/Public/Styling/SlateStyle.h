...
public:
	...
	SLATECORE_API virtual UE::Slate::FDeprecateVector2DResult GetVector(const FName PropertyName, const ANSICHAR* Specifier = nullptr, UE::Slate::FDeprecateVector2DParameter DefaultValue = FStyleDefaults::GetVector2D(), const ISlateStyle* RequestingStyle = nullptr) const override;
...
public:
	...
	FORCENOINLINE void Set(const FName PropertyName, const DefinitionType& InStyleDefintion)
	{
		...
		KnownWidgetStyles.Add(DefinitionType::TypeName);
	}
...
protected:
	...
	SLATECORE_API virtual TSet<FName> GetStyleKeys() const override;

	SLATECORE_API virtual const TSet<FName>& GetWidgetStyleNames() const override;
