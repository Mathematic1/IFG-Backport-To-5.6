...
inline void VCell::AddRef(FAccessContext Context)
{
	GlobalStrongCellRegistry.Get()->Add(Context, this);
}

inline void VCell::ReleaseRef()
{
	GlobalStrongCellRegistry.Get()->Remove(this);
}
...
{
	Visitor.Visit(GetEmergentType(), TEXT("EmergentType"));
