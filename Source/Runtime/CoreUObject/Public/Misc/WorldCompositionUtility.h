...
public:
	...
	friend FArchive& operator<<(FArchive& Ar, FWorldTileInfo& D) { D.StreamArchive(Ar); return Ar; }
	friend void operator<<(FStructuredArchive::FSlot Slot, FWorldTileInfo& D) { D.StreamStructuredArchive(Slot); }
...
private:
	COREUOBJECT_API void StreamArchive(FArchive& Ar);
	COREUOBJECT_API void StreamStructuredArchive(FStructuredArchive::FSlot Slot);
};
