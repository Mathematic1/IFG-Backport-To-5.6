	...
	if (NumItemsSerialized > 0)
	{
		if (Ar.IsLoading())
		{
			...
			for (int32 Index = 0; Index < NumItemsSerialized; Index++)
			{
				...
				Views.Add(FConstStructView(SerializedScriptStruct, nullptr));
			}
			...
			for (int32 Index = 0; Index < NumItemsSerialized; Index++)
			{
				...
				// Serialized memory
				if (Item.ScriptStruct == nullptr && SerialSize > 0)
				{
				...
				else if (Item.ScriptStruct != nullptr)
				{
					ConstCast(Item.ScriptStruct)->SerializeItem(Ar, Memory + Item.Offset, /* Defaults */ nullptr);
				}
			}
		}
		else if (Ar.IsSaving())
...
void FInstancedStructContainer::GetPreloadDependencies(TArray<UObject*>& OutDeps) const
{
	for (int32 Index = 0; Index < NumItems; Index++)
	{
		...
		{
			OutDeps.Add(ConstCast(Item.ScriptStruct));
