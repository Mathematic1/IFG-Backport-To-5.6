...
#if WITH_VERSE_VM || defined(__INTELLISENSE__)
	: CppClassInfo(nullptr)
#else
	: CppClassInfo(NAME_None)
#endif
	, SerialOffset(0)
	, SerialLayoutSize(0)
	, SerialSize(0)
...
#if WITH_VERSE_VM || defined(__INTELLISENSE__)
FCellExport::FCellExport(Verse::VCell* InCell)
	: CppClassInfo(InCell->GetCppClassInfo())
	, SerialOffset(0)
	, SerialLayoutSize(0)
	, SerialSize(0)
	, Cell(InCell)
...
FArchive& operator<<(FArchive& Ar, FCellExport& E)
{
	FStructuredArchiveFromArchive(Ar).GetSlot() << E;
	return Ar;
}
...
void operator<<(FStructuredArchive::FSlot Slot, FCellExport& E)
{
	...
	if (BaseArchive.IsSaving())
	{
		CppClassInfo = E.CppClassInfo->Name;
		VersePath = E.VersePath->AsStringView();
	}
	Record << SA_VALUE(TEXT("CppClassInfo"), CppClassInfo);
	Record << SA_VALUE(TEXT("VersePath"), VersePath);
	if (BaseArchive.IsLoading())
	{
		E.CppClassInfo = Verse::VCppClassInfoRegistry::GetCppClassInfo(*CppClassInfo.ToString());

		Verse::FAllocationContext Context = Verse::FAllocationContextPromise{};
		E.VersePath = &Verse::VUniqueString::New(Context, VersePath);
	}
#else
	Record << SA_VALUE(TEXT("CppClassInfo"), E.CppClassInfo);
	Record << SA_VALUE(TEXT("VersePath"), E.VersePath);
#endif

	Record << SA_VALUE(TEXT("SerialOffset"), E.SerialOffset);
	Record << SA_VALUE(TEXT("SerialLayoutSize"), E.SerialLayoutSize);
	Record << SA_VALUE(TEXT("SerialSize"), E.SerialSize);
...
FArchive& operator<<(FArchive& Ar, FCellImport& I)
{
	FStructuredArchiveFromArchive(Ar).GetSlot() << I;
	return Ar;
}
...
void operator<<(FStructuredArchive::FSlot Slot, FCellImport& I)
{
	...
	Record << SA_VALUE(TEXT("PackageIndex"), I.PackageIndex);
#if WITH_VERSE_VM || defined(__INTELLISENSE__)
	FUtf8String VersePath;
	if (BaseArchive.IsSaving())
	{
		VersePath = I.VersePath->AsStringView();
	}
	Record << SA_VALUE(TEXT("VersePath"), VersePath);
	if (BaseArchive.IsLoading())
	{
		Verse::FAllocationContext Context = Verse::FAllocationContextPromise{};
		I.VersePath = &Verse::VUniqueString::New(Context, VersePath);
	}
#else
	Record << SA_VALUE(TEXT("VersePath"), I.VersePath);
#endif
