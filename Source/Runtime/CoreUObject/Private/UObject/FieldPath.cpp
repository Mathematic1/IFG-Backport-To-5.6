...
UStruct* FFieldPath::TryToResolveOwnerFromStruct(UStruct* InCurrentStruct /*= nullptr*/, EPathResolveType InResolveType /*= FFieldPath::UseStructIfOuterNotFound*/) const
{
	...
	if (!Owner && LastOuter)
	{
		// The outer resolve uses names, so follow a redirector chain if needed
		while (UObjectRedirector* Redirector = Cast<UObjectRedirector>(LastOuter))
		{
			LastOuter = Redirector->DestinationObject;
			Owner = Cast<UStruct>(LastOuter);
		}
	}
...
FString FFieldPath::ToString() const
{
	...
	{
		if (FField* LocalResolvedField = GetTyped(FField::StaticClass()))
		{
			Result = LocalResolvedField->GetPathName();
		}
		else
		{
