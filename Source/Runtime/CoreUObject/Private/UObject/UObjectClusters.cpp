...
#if !UE_GCCLUSTER_VERBOSE_LOGGING
	UObject* ClusterRootObject = static_cast<UObject*>(RootObjectItem->GetObject());
#endif
...
void DumpClusterToLog(const FUObjectCluster& Cluster, bool bHierarchy, bool bIndexOnly)
{
	...
	UObject* RootObject = static_cast<UObject*>(RootItem->GetObject());
#if WITH_VERSE_VM || defined(__INTELLISENSE__)
		...
		for (int32 ClusterRootIndex : Cluster.ReferencedClusters)
		{
			if (ClusterRootIndex >= 0)
			{
				if (!bIndexOnly)
				{
					...
					UObject* ClusterRootObject = static_cast<UObject*>(ClusterRootItem->GetObject());
...
void ListClusters(const TArray<FString>& Args)
{
	...
	for (const FString& Arg : Args)
	{
		if (Arg == TEXT("SortByName"))
		{
			...
			{
				return GUObjectArray.IndexToObject(A->RootIndex)->GetObject()->GetFName();
			}, FNameLexicalLess());
		}
...
void FindStaleClusters(const TArray<FString>& Args)
{
	...
	for (FRawObjectIterator It(true); It; ++It)
	{
		...
		if (ObjectItem->HasAnyFlags(EInternalObjectFlags::ClusterRoot))
		{
			...
			UObject* ClusterRootObject = static_cast<UObject*>(ObjectItem->GetObject());
...
void DumpRefsToCluster(FUObjectCluster* Cluster)
{
	...
	UE_LOG(LogObj, Display, TEXT("Dumping references to objects in cluster %s"), *static_cast<UObject*>(RootItem->GetObject())->GetFullName());

	bool bIsReferenced = false;
	for (int32 ObjectIndex : Cluster->Objects)
	{
		FUObjectItem* ObjectItem = GUObjectArray.IndexToObject(ObjectIndex);
		FReferenceChainSearch SearchRefs(static_cast<UObject*>(ObjectItem->GetObject()), EReferenceChainSearchMode::ExternalOnly | EReferenceChainSearchMode::Shortest);
	...
	if (!bIsReferenced)
	{
		UE_LOG(LogObj, Display, TEXT("Cluster %s is not currently referenced by anything."), *static_cast<UObject*>(RootItem->GetObject())->GetFullName());
	}
}
...
void DumpRefsToCluster(const TArray<FString>& Args)
{
	...
	for (int32 RootIndex : RootObjects)
	{
		...
		{
			FUObjectCluster* Cluster = GUObjectClusters.GetObjectCluster(static_cast<UObject*>(RootItem->GetObject()));
...
public:
	...
	{
		return static_cast<UObject*>(GUObjectArray.IndexToObject(Cluster.RootIndex)->GetObject());
	}
...
void UObjectBaseUtility::AddToCluster(UObjectBaseUtility* ClusterRootOrObjectFromCluster, bool bAddAsMutableObject /* = false */)
{
	...
	if (Cluster)
	{
		...
		{
			UObject* ClusterRootObject = static_cast<UObject*>(GUObjectArray.IndexToObjectUnsafeForGC(Cluster->RootIndex)->GetObject());
