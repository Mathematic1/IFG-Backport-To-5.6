...
void FPropertyValueIterator::FillStructProperties(const UStruct* Struct, FPropertyValueStackEntry& Entry)
{
	...
	{
		const FInstancedStruct* InstancedStruct = reinterpret_cast<const FInstancedStruct*>(Entry.Owner);
		if (InstancedStruct->GetScriptStruct() && InstancedStruct->GetMemory())
		{
			Entry.Owner = InstancedStruct->GetMemory();
	...
	auto AddPropertyToValueArray = [this, Owner, &ValueArray](const FProperty* Property)
	{
		...
		if (PropertyValueFlags != EPropertyValueFlags::None)
		{
			...
			{
				const void* PropertyValue = Property->ContainerPtrToValuePtr<void>(Owner, StaticIndex);
				ValueArray.Emplace(BasePairType(Property, PropertyValue), PropertyValueFlags);
			}
		}
	};

	if (PropertyClass->IsChildOf(FObjectPropertyBase::StaticClass()))
	{
		const bool bIncludeDeprecated = EnumHasAnyFlags((EFieldIterationFlags)DeprecatedPropertyFlags, EFieldIterationFlags::IncludeDeprecated);

		// Note: This includes properties that contain object references (e.g. array), along with any inherited reference properties.
		for (FProperty* RefProp = Struct->RefLink; RefProp; RefProp = RefProp->NextRef)
		{
			if (!bIncludeDeprecated && RefProp->HasAnyPropertyFlags(CPF_Deprecated))
			{
				continue;
			}

			AddPropertyToValueArray(RefProp);
		}
	}
	else
	{
