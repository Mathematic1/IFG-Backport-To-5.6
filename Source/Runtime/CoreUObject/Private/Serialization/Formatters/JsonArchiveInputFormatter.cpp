...
FStructuredArchiveFormatter* FJsonArchiveInputFormatter::CreateSubtreeReader()
{
	...
	Cloned->SetResolveCell(CopyTemp(this->ResolveCell));
#endif
	return Cloned;
}
...
void FJsonArchiveInputFormatter::Serialize(UTF32CHAR& Value)
{
	verify(ValueStack.Top()->TryGetNumber(*(uint32*)&Value));
}
...
#if WITH_VERSE_VM || defined(__INTELLISENSE__)
void FJsonArchiveInputFormatter::Serialize(Verse::VCell*& Value)
{
	FString StringValue;
	if (ValueStack.Top()->TryGetString(StringValue))
	{
		FPackageIndex CellIndex;
		LexFromString(CellIndex, *StringValue);
		Value = ResolveCell(CellIndex);
	}
	else
	{
		Value = nullptr;
...
#if WITH_VERSE_VM || defined(__INTELLISENSE__)
void FJsonArchiveInputFormatter::SetResolveCell(TFunction<Verse::VCell* (const FPackageIndex)>&& InResolveCell)
{
	ResolveCell = MoveTemp(InResolveCell);
}
#endif
