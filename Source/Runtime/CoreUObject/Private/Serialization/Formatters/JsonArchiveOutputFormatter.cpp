...
void FJsonArchiveOutputFormatter::Serialize(UTF32CHAR& Value)
{
...
void FJsonArchiveOutputFormatter::Serialize(Verse::VCell*& Value)
{
	if (Value != nullptr && IsCellAllowed(Value))
	{
		FPackageIndex CellIndex = CellIndicesMap->FindChecked(Value);
		SerializeStringInternal(LexToString(CellIndex));
	}
	else
	{
		WriteValue(TEXT("null"));
...
#if WITH_VERSE_VM || defined(__INTELLISENSE__)
bool FJsonArchiveOutputFormatter::IsCellAllowed(Verse::VCell* InCell) const
{
	return CellIndicesMap && CellIndicesMap->Contains(InCell);
}
#endif
