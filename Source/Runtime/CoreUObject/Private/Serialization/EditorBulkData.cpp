...
/** 
 * Returns true if the project wants to log if an editor bulkdata is cooked or uncooked and returns
...
bool ShouldGenerateNewIdentifier(FArchive& Ar, UObject* Owner)
{
	...
	if (Ar.HasAllPortFlags(PPF_Duplicate) || (LinkerLoad && LinkerLoad->GetInstancingContext().IsInstanced()))
	{
		return true;
	}
...
FEditorBulkData::~FEditorBulkData()
{
	...
	// If we have registered with the BulkData registry, keep our ownership of our guid registered,
	// and possibly loadable from our on-disk location, but let the BulkDataRegistry know that we
	// no longer have an in-memory copy of the data.
	OnExitMemory();
}
...
void FEditorBulkData::Serialize(FArchive& Ar, UObject* Owner, bool bAllowRegister)
{
	...
	else if (Ar.IsPersistent() && !Ar.IsObjectReferenceCollector() && !Ar.ShouldSkipBulkData())
	{
		...
		{
			if (BulkDataId.IsValid() && ShouldGenerateNewIdentifier(Ar, Owner))
			{
				BulkDataId = FGuid::NewGuid();
			}
	...
	if (!IsDataVirtualized() && !PackagePath.IsEmpty())
	{
		...
		if (EnumHasAnyFlags(Flags, EFlags::HasRegistered))
		{
			...
			else
			{
				...
				// but let it know that we no longer have an in-memory copy of the data.
				OnExitMemory();
			}
		}
	}
