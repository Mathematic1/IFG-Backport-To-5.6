...
/** Returns if the current running commandlet requires the BulkDataRegistry or not */
bool IsAllowedForCurrentCommandlet()
{
	/** Cache the results so we don't have to redo this every time the function is called */
	struct FConfigEnabled
	{
		FConfigEnabled()
		{
			FStringView CommandletToken(TEXTVIEW("commandlet"));

			FString CommandletName = GetCommandletNameFromCmdline();
			if (CommandletName.EndsWith(CommandletToken))
			{
				CommandletName.LeftChopInline(CommandletToken.Len(), EAllowShrinking::No);
			}

			TArray<FString> CommandletsUsingRegistry;
			GConfig->GetArray(TEXT("BulkDataRegistry"), TEXT("CommandletsUsingBulkDataRegistry"), CommandletsUsingRegistry, GEditorIni);

			for (FString& CommandletUsingRegistry : CommandletsUsingRegistry)
			{
				if (CommandletUsingRegistry.EndsWith(CommandletToken))
				{
					CommandletUsingRegistry.LeftChopInline(CommandletToken.Len(), EAllowShrinking::No);
				}

				if (CommandletUsingRegistry == CommandletName)
				{
					bEnabled = true;
					break;
				}
	...
	} static ConfigEnabled;

	return ConfigEnabled.bEnabled;
}
...
{
	if (IsRunningCommandlet() && !IsRunningCookCommandlet() && !UE::BulkDataRegistry::Private::IsAllowedForCurrentCommandlet())
	{
