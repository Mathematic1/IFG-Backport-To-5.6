...
void FSceneExtensions::CreateRenderers(FSceneRendererBase& InSceneRenderer, const FEngineShowFlags& EngineShowFlags, FRendererList& OutRenderers)
{
	...
	for (auto It = Extensions.CreateIterator(); It; ++It)
	{
		const int32 Index = It.GetIndex();
		check(Index <= FSceneExtensionRegistry::Get().GetMaxRegistrationID());
		if (auto Renderer = Extensions[Index]->CreateRenderer(InSceneRenderer, EngineShowFlags))
		{
...
void FSceneExtensionsRenderers::Begin(FSceneRendererBase& InSceneRenderer, const FEngineShowFlags& EngineShowFlags, bool bInValidateCallbacks)
{
	bValidateCallbacks = false;//bInValidateCallbacks;
	ValidateAdvanceCallbackStage(ECallbackStage::Begin);
	...
	SceneRenderer->Scene->SceneExtensions.CreateRenderers(InSceneRenderer, EngineShowFlags, Renderers);
...
{
	ValidateAdvanceCallbackStage(ECallbackStage::End);
	for (auto Renderer : Renderers)
	{
		Renderer->End();
		delete Renderer;
