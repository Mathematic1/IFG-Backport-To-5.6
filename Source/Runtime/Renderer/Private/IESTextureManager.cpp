...
struct FIESTextureManager : public FRenderResource
{
	...
	uint32 RequestedSliceCountWhenTextureAllocated = 0;
...
float GetAtlasSlot(uint32 InSlotId)
{
	...
	{
		uint32 SlotSliceIndex = GIESTextureManager.Slots[InSlotId].SliceIndex;
		return (SlotSliceIndex != InvalidSliceIndex) ? float(SlotSliceIndex) : float(INDEX_NONE);
	}
	else
	{
...
void UpdateAtlasTexture(FRDGBuilder& GraphBuilder, const FStaticShaderPlatform ShaderPlatform)
{
	...
	const bool bForceUpdate = CVarIESForceUpdate.GetValueOnRenderThread() > 0;
	...
	if (GIESTextureManager.bHasPendingAdds || bForceUpdate)
	{
		RequestedSliceCount = FMath::Min(CachedAtlasMaxSlice, GetRequestedSlotCount(GIESTextureManager));
		bForceRecreate = (GIESTextureManager.AtlasTexture == nullptr) || (GIESTextureManager.RequestedSliceCountWhenTextureAllocated < RequestedSliceCount);
	}
	...
	if (bForceUpdate || bForceRecreate)
	{
	...
	if (GIESTextureManager.bHasPendingAdds)
	{
		...
		// 3.2. Apply layout modification
		{
			...
			if (bForceRecreate)
			{
				...
				GIESTextureManager.RequestedSliceCountWhenTextureAllocated = RequestedSliceCount;
			}
			else
			{
