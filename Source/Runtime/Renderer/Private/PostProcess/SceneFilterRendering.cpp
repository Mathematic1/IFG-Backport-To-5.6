	...
	)
{
	...
	const FRHIBufferCreateDesc CreateDesc =
		FRHIBufferCreateDesc::CreateVertex<FFilterVertex>(TEXT("DrawTransformedRectangle"), 4)
		.AddUsage(EBufferUsageFlags::Volatile)
		.SetInitActionInitializer()
		.DetermineInitialState();

	TRHIBufferInitializer<FFilterVertex> Vertices = RHICmdList.CreateBufferInitializer(CreateDesc);
	...
	FBufferRHIRef VertexBufferRHI = Vertices.Finalize();
