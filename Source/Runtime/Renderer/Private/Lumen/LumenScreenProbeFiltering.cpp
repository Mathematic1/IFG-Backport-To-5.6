...
class FScreenProbeCompositeTracesWithScatterCS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float>, RWScreenProbeExtraAO)
	...
	using FPermutationDomain = TShaderPermutationDomain<FThreadGroupSize, FStructuredImportanceSampling, FScreenProbeExtraAO>;
...
class FScreenProbeFilterGatherTracesCS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float3>, RWScreenProbeRadiance)
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float3>, RWScreenProbeExtraAO)
		SHADER_PARAMETER_RDG_TEXTURE(Texture2D, ScreenProbeRadiance)
		SHADER_PARAMETER_RDG_TEXTURE(Texture2D, ScreenProbeExtraAO)
	...
	class FScreenProbeExtraAO : SHADER_PERMUTATION_BOOL("SCREEN_PROBE_EXTRA_AO");
	using FPermutationDomain = TShaderPermutationDomain<FScreenProbeExtraAO>;
...
class FScreenProbeConvertToIrradianceCS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float3>, RWScreenProbeExtraAOWithBorder)
		SHADER_PARAMETER_RDG_TEXTURE(Texture2D, ScreenProbeRadiance)
		SHADER_PARAMETER_RDG_TEXTURE(Texture2D, ScreenProbeExtraAO)
		SHADER_PARAMETER_RDG_TEXTURE(Texture2D, ScreenProbeHitDistance)
		SHADER_PARAMETER_STRUCT_REF(FViewUniformShaderParameters, View)
	...
	class FScreenProbeExtraAO : SHADER_PERMUTATION_BOOL("SCREEN_PROBE_EXTRA_AO");
	using FPermutationDomain = TShaderPermutationDomain<FThreadGroupSize, FWaveOpWaveSize, FProbeIrradianceFormat, FScreenProbeExtraAO>;
	...
	ERDGPassFlags ComputePassFlags)
{
	...
	FRDGTextureRef ScreenProbeExtraAO = GraphBuilder.CreateTexture(ScreenProbeHitDistanceDesc, TEXT("Lumen.ScreenProbeGather.ScreenProbeExtraAO"));

	{
		...
		PassParameters->RWScreenProbeExtraAO = ScreenProbeExtraAO ? GraphBuilder.CreateUAV(FRDGTextureUAVDesc(ScreenProbeExtraAO)) : nullptr;
		...
		PermutationVector.Set< FScreenProbeCompositeTracesWithScatterCS::FScreenProbeExtraAO >(LumenScreenProbeGather::UseScreenProbeExtraAO());
	...
	if (LumenScreenProbeGather::UseProbeSpatialFilter() && GLumenScreenProbeSpatialFilterHalfKernelSize > 0)
	{
		for (int32 PassIndex = 0; PassIndex < GLumenScreenProbeSpatialFilterNumPasses; PassIndex++)
		{
			...
			FRDGTextureRef FilteredScreenProbeAO = GraphBuilder.CreateTexture(ScreenProbeRadianceDesc, TEXT("Lumen.ScreenProbeGather.ScreenProbeFilteredAO"));
			...
			PassParameters->RWScreenProbeExtraAO = ScreenProbeExtraAO ? GraphBuilder.CreateUAV(FRDGTextureUAVDesc(FilteredScreenProbeAO)) : nullptr;
			PassParameters->ScreenProbeRadiance = ScreenProbeRadiance;
			PassParameters->ScreenProbeExtraAO = ScreenProbeExtraAO;
			...
			FScreenProbeFilterGatherTracesCS::FPermutationDomain PermutationVector;
			PermutationVector.Set< FScreenProbeFilterGatherTracesCS::FScreenProbeExtraAO >(LumenScreenProbeGather::UseScreenProbeExtraAO());
			auto ComputeShader = View.ShaderMap->GetShader<FScreenProbeFilterGatherTracesCS>(PermutationVector);
			...
			ScreenProbeExtraAO = FilteredScreenProbeAO;
		}
	}
	...
	FRDGTextureRef ScreenProbeExtraAOWithBorder = nullptr;

	if (LumenScreenProbeGather::UseScreenProbeExtraAO())
	{
		const FIntPoint ScreenProbeExtraAOWithBorderBufferSize(ScreenProbeParameters.ScreenProbeAtlasBufferSize.X * LumenScreenProbeGather::IrradianceProbeWithBorderRes, ScreenProbeParameters.ScreenProbeAtlasBufferSize.Y * LumenScreenProbeGather::IrradianceProbeWithBorderRes);
		FRDGTextureDesc ScreenProbeExtraAOWithBorderDesc(FRDGTextureDesc::Create2D(ScreenProbeExtraAOWithBorderBufferSize, PF_R8, FClearValueBinding::Black, TexCreate_ShaderResource | TexCreate_UAV));
		ScreenProbeExtraAOWithBorder = GraphBuilder.CreateTexture(ScreenProbeExtraAOWithBorderDesc, TEXT("Lumen.ScreenProbeGather.ScreenProbeExtraAOWithBorder"));
	}
	...
	if (ConvertToSHThreadGroupSize != MAX_uint32)
	{
		...
		PassParameters->RWScreenProbeExtraAOWithBorder = ScreenProbeExtraAOWithBorder ? GraphBuilder.CreateUAV(FRDGTextureUAVDesc(ScreenProbeExtraAOWithBorder)) : nullptr;
		PassParameters->ScreenProbeRadiance = ScreenProbeRadiance;
		PassParameters->ScreenProbeExtraAO = ScreenProbeExtraAO;
		PassParameters->ScreenProbeHitDistance = ScreenProbeHitDistance;
		PassParameters->View = View.ViewUniformBuffer;
		...
		PermutationVector.Set< FScreenProbeConvertToIrradianceCS::FScreenProbeExtraAO >(ScreenProbeExtraAOWithBorder != nullptr);
		auto ComputeShader = View.ShaderMap->GetShader<FScreenProbeConvertToIrradianceCS>(PermutationVector);

		FComputeShaderUtils::AddPass(
			GraphBuilder,
			RDG_EVENT_NAME("ScreenProbeConvertToIrradiance Format:%d WaveOpWaveSize:%d AO:%d", (int32)ScreenProbeIrradianceFormat, WaveOpWaveSize, ScreenProbeExtraAOWithBorder ? 1 : 0),
			ComputePassFlags,
			ComputeShader,
			PassParameters,
			ScreenProbeParameters.ProbeIndirectArgs,
	...
	GatherParameters.ScreenProbeExtraAOWithBorder = ScreenProbeExtraAOWithBorder;
