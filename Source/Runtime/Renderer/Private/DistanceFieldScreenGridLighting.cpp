...
public:
	...
	static void ModifyCompilationEnvironment(const FGlobalShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
	{
		...
		if (IsMobilePlatform(Parameters.Platform) && !IsMobileDeferredShadingEnabled(Parameters.Platform))
		{
			OutEnvironment.SetDefine(TEXT("ROTATE_WITH_TANGENT"), 0);
		}
		else
		{
			OutEnvironment.SetDefine(TEXT("ROTATE_WITH_TANGENT"), 1);
		}
...
void FSceneRenderer::RenderDistanceFieldAOScreenGrid(
	FRDGBuilder& GraphBuilder,
	const FSceneTextures& SceneTextures,
	const FViewInfo& View,
