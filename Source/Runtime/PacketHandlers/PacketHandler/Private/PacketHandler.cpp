								...
								TSharedPtr<IAnalyticsProvider> InProvider/*=nullptr*/, FDDoSDetection* InDDoS/*=nullptr*/, FName InDriverProfile/*=NAME_None*/)
{
	...
	if (!bConnectionlessHandler)
	{
		...
		// all of code below till end of scope can go away once deprecated GetAddComponentDelegate method is removed
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
			...
			for (int32 i=0; i<HandlerComponents.Num() && !OutgoingPacket.IsError(); ++i)
			{
				...
				if (CurComponent.IsActive())
				{
					...
					else
					{
						...
						UE_LOG(PacketHandlerLog, Error, TEXT("Packet exceeded HandlerComponents 'MaxOutgoingBits' value: %" INT64_FMT " vs %i"),
		...
		for (int32 i=0; i<HandlerComponents.Num() && !Writer.IsError(); ++i)
		{
			...
			if (CurComponent.IsActive())
			{
				...
				else
				{
					...
					UE_LOG(PacketHandlerLog, Error, TEXT("Handler packet exceeded HandlerComponents 'MaxOutgoingBits' value: %" INT64_FMT " vs %i"),
