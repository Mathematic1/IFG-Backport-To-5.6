...
void FLocalFileStreamFArchive::Serialize(void* V, int64 Length) 
{
	if (IsLoading())
	{
		...
		{
			UE_LOG(LogLocalFileReplay, Error, TEXT("FLocalFileStreamFArchive::Serialize: Attempted to serialize past end of archive: Position = %" INT64_FMT ", Size=%i, Requested = %" INT64_FMT), ArchivePos, Buffer.Num(), Length);
			SetError();
			return;
		}
...
void FLocalFileNetworkReplayStreamer::GotoCheckpointIndex(const int32 CheckpointIndex, const FGotoCallback& Delegate, EReplayCheckpointType CheckpointType)
{
	...
	if (CheckpointType == EReplayCheckpointType::Delta)
	{
			...
			[this, CheckpointIndex, Delegate](TLocalFileRequestCommonData<FGotoResult>& RequestData)
		{
			...
			{
				UE_LOG(LogLocalFileReplay, Warning, TEXT("FLocalFileNetworkReplayStreamer::GotoCheckpointIndexDelta. Clamped to checkpoint: %" INT64_FMT), LastGotoTimeInMS);
	...
	else
	{
		...
		[this, CheckpointIndex, Delegate](TLocalFileRequestCommonData<FGotoResult>& RequestData)
		{
			...
			{
				UE_LOG(LogLocalFileReplay, Warning, TEXT("FLocalFileNetworkReplayStreamer::GotoCheckpointIndex. Clamped to checkpoint: %" INT64_FMT), LastGotoTimeInMS);
