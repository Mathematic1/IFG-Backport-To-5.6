...
void FThinZone2DFinder::FindCloseSegments()
{
	...
	for (FEdgeSegment* Segment : SortedThinZoneSegments)
	{
		...
		const FVector2d SegmentMiddle = Segment->GetCenter();
		...
		const double SegmentMiddleMax = FVectorUtil::DiagonalAxisCoordinate(SegmentMiddle) + MaxSpacePlusLength;
		...
		for (int32 CandidateIndex = SegmentToBeginIndex; CandidateIndex < SortedThinZoneSegments.Num(); ++CandidateIndex)
		{
			...
			const FVector2d& FirstPointCandidate = Candidate->GetExtemity(ELimit::Start);
			...
			{
				const double SquareDistance = FVector2d::DistSquared(SegmentMiddle, FirstPointCandidate);
			...
			const FVector2d& SecondPointCandidate = Candidate->GetExtemity(ELimit::End);
			double Coordinate;
			const FVector2d Projection = ProjectPointOnSegment<FVector2d>(SegmentMiddle, FirstPointCandidate, SecondPointCandidate, Coordinate, true);
			...
			const double SquareDistance = FVector2d::DistSquared(SegmentMiddle, Projection);
...
bool FThinZone2DFinder::CheckIfCloseSideOfThinSideIsNotDegenerated(TArray<FEdgeSegment*>& Side)
{
	...
	TFunction<FVector2d(const FVector2d&)> GetOppositeSegment = [&CloseSegments](const FVector2d& Candidate) -> FVector2d
	{
		FVector2d OutOppositePoint = FVector2d::ZeroVector;
		double MinSquareThickness = HUGE_VALUE;
		for (int32 Index = 0; CloseSegments[Index] != nullptr; ++Index)
		{
			const FEdgeSegment* CloseSegment = CloseSegments[Index];
			const FVector2d& FirstPointCandidate = CloseSegment->GetExtemity(ELimit::Start);
			const FVector2d& SecondPointCandidate = CloseSegment->GetExtemity(ELimit::End);

			double Coordinate;
			const FVector2d Projection = ProjectPointOnSegment<FVector2d>(Candidate, FirstPointCandidate, SecondPointCandidate, Coordinate, true);

			const double SquareDistance = FVector2d::DistSquared(Candidate, Projection);
			if (MinSquareThickness > SquareDistance)
			{
				MinSquareThickness = SquareDistance;
	...
	const FVector2d& StartPoint = Side[0]->GetExtemity(Start);
	FVector2d FirstOppositePoint = GetOppositeSegment(StartPoint);
	...
	for (const FEdgeSegment* Segment : Side)
	{
		...
		const FVector2d& NextPoint = Segment->GetExtemity(End);
		FVector2d OppositePoint = GetOppositeSegment(NextPoint);
		const double SquarewDistance = FVector2d::DistSquared(OppositePoint, FirstOppositePoint);
	...
	for (const TSharedPtr<FTopologicalLoop>& Loop : Loops)
	{
		...
		for (const FOrientedEdge& Edge : Edges)
		{
			...
			// Remove duplicated points (vs GeometricTolerance) of the end
			{
				...
				while (EndPointIndex > 0 && FVector2d::Distance(ScaledPoints[EndPointIndex], ScaledPoints[EndPointIndex - 1]) < GeometricTolerance)
				{
					EndPointIndex--;
			...
			// Remove duplicated points (vs GeometricTolerance)
			{
				...
				{
					while (PointIndex < ScaledPoints.Num() && FVector2d::Distance(ScaledPoints[PointIndex - 1], ScaledPoints[PointIndex]) < GeometricTolerance)
					{
...
void FThinZoneSide::GetExistingMeshNodes(const FTopologicalFace& Face, FModelMesh& MeshModel, FReserveContainerFunc& Reserve, FAddMeshNodeFunc& AddMeshNode, const bool bWithTolerance) const
{
	...
	TFunction<void(const FEdgeSegment&)> AddEdgeMeshNodeIfInclude = [&](const FEdgeSegment& EdgeSegment)
	{
		...
		for (;; Index += Increment)
		{
			...
			const FVector2d MeshNode2D = EdgeSegment.ComputeEdgePoint(MeshNodeCoordinate);
	...
	// Count the existing mesh vertices on the Side
	{
		...
		for (const FEdgeSegment& EdgeSegment : GetSegments())
		{
			if (Edge != EdgeSegment.GetEdge())
			{
				...
				if (Edge->GetLinkActiveEdge()->IsMeshed())
				{
					const TArray<FVector>& EdgeMeshNodes = Edge->GetMesh()->GetNodeCoordinates();
	...
	// Get the existing mesh vertices and fill output data
	{
		...
		for (const FEdgeSegment& EdgeSegment : GetSegments())
		{
			...
			FMath::GetMinMax(SegmentUMin, SegmentUMax);
