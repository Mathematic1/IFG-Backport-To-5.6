...
bool FGridBase::ScaleGrid()
{
	...
	for (int32 IndexU = 0; IndexU < CuttingCount[EIso::IsoU]; ++IndexU)
	{
		...
		{
			Length += FVector::Distance(Points3D[Index], Points3D[Index + CuttingCount[EIso::IsoU]]);
	...
	for (int32 IndexV = 0, Index = 0; IndexV < CuttingCount[EIso::IsoV]; ++IndexV)
	{
		...
		{
			Length += FVector::Distance(Points3D[Index], Points3D[Index + 1]);
			Index++;
		}
		Index++;
	...
	if (StandardDeviationV > StandardDeviationU)
	{
		...
		{
			double Length = 0;
			FVector LastPoint = MiddlePoints.Points3D[IPointU];
			for (int32 IPointV = StartIndexUp; IPointV < CuttingCount[EIso::IsoV]; ++IPointV)
			{
				NumUV = IPointV * CuttingCount[EIso::IsoU] + IPointU;
				Length += FVector::Distance(LastPoint, Points3D[NumUV]);
				Points2D[EGridSpace::Scaled][NumUV].Set(Points2D[EGridSpace::UniformScaled][NumUV].X, Length);
	...
	else
	{
		...
		{
			double Length = 0;
			FVector LastPoint = MiddlePoints.Points3D[IPointV];
			for (int32 IPointU = StartIndexUp; IPointU < CuttingCount[EIso::IsoU]; ++IPointU)
			{
				NumUV = IPointV * CuttingCount[EIso::IsoU] + IPointU;
				Length += FVector::Distance(LastPoint, Points3D[NumUV]);
				Points2D[EGridSpace::Scaled][NumUV].Set(Length, Points2D[EGridSpace::UniformScaled][NumUV].Y);
				LastPoint = Points3D[NumUV];
			}
...
void FGridBase::TransformPoints(EGridSpace DestinationSpace, const TArray<FVector2d>& InPointsToScale, TArray<FVector2d>& OutTransformedPoints) const
{
	const FCoordinateGrid& CoordinateGrid = GetCoordinateGrid();
	...
	{
		const FVector2d& Point = InPointsToScale[Index];
