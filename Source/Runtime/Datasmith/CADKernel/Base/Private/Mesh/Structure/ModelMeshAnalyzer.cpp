...
void FModelMeshAnalyzer::BuildMesh()
{
	...
	TFunction<Analyzer::FEdge*(uint32, uint32)> FindOrBuildEdge = [&](uint32 VertexIndex0, uint32 VertexIndex1) -> Analyzer::FEdge*
	{
		...
		FMath::GetMinMax(VertexIndex0, VertexIndex1, EdgeVertexIndex0, EdgeVertexIndex1);
...
double FModelMeshAnalyzer::ComputeMaxAngle() const 
{
	...
	for (const Analyzer::FEdge& Edge : Edges)
	{
		...
		FVector Normal0 = Edge.Triangles[0]->ComputeNormal(NodeCoordinates);
		FVector Normal1 = Edge.Triangles[1]->ComputeNormal(NodeCoordinates);

		double Angle = FVectorUtil::ComputeAngle(Normal0, Normal1);
		if (Angle > MaxAngle)
		{
			MaxAngle = Angle;
		}
...
FVector Analyzer::FTriangle::ComputeNormal(const TArray<FVector>& NodeCoordinates) const
{
