...
void FMatrixH::BuildChangeOfCoordinateSystemMatrix(const FVector& Xaxis, const FVector& Yaxis, const FVector& Zaxis, const FVector& Origin)
{
	Get(0, 0) = Xaxis[0];
	Get(1, 0) = Xaxis[1];
	Get(2, 0) = Xaxis[2];
...
void FMatrixH::FromAxisOrigin(const FVector& Axis, const FVector& Origin)
{
	FVector Zaxis = FVector(0, 1, 0);
	FVector Xaxis = Zaxis ^ Axis;
...
FMatrixH FMatrixH::MakeRotationMatrix(double Angle, FVector Axe)
{
	FMatrixH Matrix;
	Matrix.SetIdentity();
...
FMatrixH FMatrixH::MakeTranslationMatrix(const FVector& Point)
{
	FMatrixH Matrix;
	Matrix.SetIdentity();
...
FMatrixH FMatrixH::MakeScaleMatrix(FVector& Scale)
{
