...
protected:
	...
	{
		double ABLength = PointType::Distance(PointA, PointB) / 2;
	...
	int32 CheckTangentError(const PointType& APoint, double ACoordinate, const PointType& BPoint, double BCoordinate, int32 FirstIndex, int32 EndIndex, int32 InStartSamplingSegmentIndex)
	{
		...
		if (FirstIndex > 0 && InStartSamplingSegmentIndex > 0)
		{
			...
			double ChordA = DistanceOfPointToSegment<PointType>(APoint, PreviousPoint, Middle);
		...
		if (EndIndex + 1 < CandidatePoints.Coordinates.Num())
		{
			...
			double ChordB = DistanceOfPointToSegment<PointType>(BPoint, NextPoint, Middle);
...
public:
		...
		{
			{
				...
				for (const FVector& Point : Sampling.GetPoints())
				{
			...
			}
			{
				...
				for (const FVector& Point : CandidatePoints.GetPoints())
				{
	...
	void CheckResult()
	{
		...
		for (int32 Index = 0; Index < EvaluationCount; ++Index)
		{
			...
			PointType ProjectedPoint = ProjectPointOnSegment<PointType>(CandidatePoints.GetPoints()[Index], Sampling.GetPoints()[Index], Sampling.GetPoints()[Index + 1], CurvilinearAbscise);
