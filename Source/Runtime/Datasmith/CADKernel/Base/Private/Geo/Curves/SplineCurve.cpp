...
void FSplineCurve::Offset(const FVector& OffsetDirection)
{
	ensureCADKernel(false);
...
void FSplineCurve::ExtendTo(const FVector& Point)
{
	ensureCADKernel(false);
	//PolylineTools::ExtendTo(Poles, Point);
}

void FSplineCurve::SetSplinePoints(const TArray<FVector>& Points)
{
	...
	{
		Position.Points.Emplace(InputKey, Point, FVector::ZeroVector, FVector::ZeroVector, CIM_CurveAuto);
		InputKey += 1.0;
	}
...
void FSplineCurve::SetSplinePoints(const TArray<FVector>& Points, const TArray<FVector>& Tangents)
{
	const int32 NumPoints = Points.Num();
...
void FSplineCurve::SetSplinePoints(const TArray<FVector>& Points, const TArray<FVector>& ArriveTangents, const TArray<FVector>& LeaveTangents)
{
	const int32 NumPoints = Points.Num();
	Position.Points.Reset(NumPoints);
	ensureCADKernel(false);
...
FVector FSplineCurve::GetLocationAtSplineInputKey(double Coordinate) const
{
	return Position.Eval((float)Coordinate, FVector::ZeroVector);
}

FVector FSplineCurve::GetTangentAtSplineInputKey(double Coordinate) const
{
	return Position.EvalDerivative((float)Coordinate, FVector::ZeroVector);
}

FVector FSplineCurve::GetDirectionAtSplineInputKey(double Coordinate) const
{
	return Position.EvalDerivative(Coordinate, FVector::ZeroVector).GetSafeNormal();
}
...
double FSplineCurve::GetSegmentLength(const int32 Index, const double Param, bool bInClosedLoop) const
{
	...
	const FVector Coeff1 = ((P0 - P1) * 2.0 + T0 + T1) * 3.0;
	const FVector Coeff2 = (P1 - P0) * 6.0 - T0 * 4.0f - T1 * 2.0;
