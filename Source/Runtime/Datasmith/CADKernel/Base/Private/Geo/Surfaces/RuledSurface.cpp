...
void FRuledSurface::EvaluatePoint(const FVector2d& InPoint2D, FSurfacicPoint& OutPoint3D, int32 InDerivativeOrder) const
{
	double CoordinateCurve0 = Curves[0]->GetBoundary().Min + InPoint2D.X * (Curves[0]->GetBoundary().Max - Curves[0]->GetBoundary().Min);
	double CoordinateCurve1 = Curves[1]->GetBoundary().Min + InPoint2D.X * (Curves[1]->GetBoundary().Max - Curves[1]->GetBoundary().Min);
	...
	OutPoint3D.Point = CurvePoint[0].Point + OutPoint3D.GradientV * InPoint2D.Y;

	if (InDerivativeOrder > 0)
	{
		OutPoint3D.LaplacianUV = CurvePoint[1].Gradient - CurvePoint[0].Gradient;
		OutPoint3D.GradientU = CurvePoint[0].Gradient + InPoint2D.Y * OutPoint3D.LaplacianUV;

		if (InDerivativeOrder > 1)
		{
			OutPoint3D.LaplacianU = CurvePoint[0].Laplacian + InPoint2D.Y * (CurvePoint[1].Laplacian - CurvePoint[0].Laplacian);
...
void FRuledSurface::EvaluatePointGrid(const FCoordinateGrid& Coordinates, FSurfacicSampling& OutPoints, bool bComputeNormals) const
{
	...
	{
		FVector GradientV = CurveCoords[1][Undex].Point - CurveCoords[0][Undex].Point;
		...
		if (bComputeNormals)
		{
			for (int32 Vndex = 0, Index = Undex; Vndex < VCount; ++Vndex)
			{
				FVector GradientU = CurveCoords[0][Undex].Gradient + Coordinates[EIso::IsoV][Vndex] * (CurveCoords[1][Undex].Gradient - CurveCoords[0][Undex].Gradient);
				OutPoints.Normals[Index] = (FVector3f)(GradientU ^ GradientV);
				Index += UCount;
			}
		}
