...
void FRevolutionSurface::EvaluatePoint(const FVector2d& InSurfacicCoordinate, FSurfacicPoint& OutPoint3D, int32 InDerivativeOrder) const
{
	FCurvePoint GeneratrixPoint;
	Generatrix->EvaluatePoint(InSurfacicCoordinate.X, GeneratrixPoint, InDerivativeOrder);

	FMatrixH Matrix = FMatrixH::MakeRotationMatrix(InSurfacicCoordinate.Y, RotationAxis);
	...
	{
		FVector Vector = OutPoint3D.Point - Axis->GetStartPoint();
...
void FRevolutionSurface::EvaluatePointGrid(const FCoordinateGrid& Coordinates, FSurfacicSampling& OutPoints, bool bComputeNormals) const
{
	...
	TArray<FVector> GeneratrixGradientU;
	...
	if (bComputeNormals)
	{
		for (int32 Vndex = 0, Index = 0; Vndex < Coordinates.IsoCount(EIso::IsoV); ++Vndex)
		{
			for (FCurvePoint GeneratrixPoint : GeneratrixPoints)
			{
				FVector GradientU = Matrix[Vndex].MultiplyVector(GeneratrixPoint.Gradient);

				FVector Vector = OutPoints.Points3D[Index] - Axis->GetStartPoint();
				FVector GradientV = RotationAxis ^ Vector;
