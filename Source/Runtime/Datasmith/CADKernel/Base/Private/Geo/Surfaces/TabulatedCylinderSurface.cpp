...
void FTabulatedCylinderSurface::EvaluatePoint(const FVector2d& InSurfacicCoordinate, FSurfacicPoint& OutPoint3D, int32 InDerivativeOrder) const
{
	const FLinearBoundary& CurveBounds = GuideCurve->GetBoundary();

	FCurvePoint PointU;
	GuideCurve->EvaluatePoint(CurveBounds.GetAt(InSurfacicCoordinate.X), PointU, InDerivativeOrder);

	OutPoint3D.DerivativeOrder = InDerivativeOrder;
	OutPoint3D.Point = PointU.Point + DirectorVector * InSurfacicCoordinate.Y;
...
void FTabulatedCylinderSurface::EvaluatePointGrid(const FCoordinateGrid& Coordinates, FSurfacicSampling& OutPoints, bool bComputeNormals) const
{
	...
	if (bComputeNormals)
	{
		...
		{
			int32 Index = Undex++;
			FVector3f Normal = (FVector3f)(Point.Gradient ^ DirectorVector);
...
TSharedPtr<FEntityGeom> FTabulatedCylinderSurface::ApplyMatrix(const FMatrixH& InMatrix) const
{
	...
	FVector Point = GuideCurve->EvaluatePoint(GuideUMin);
	Point += DirectorVector;

	FVector TransformedPoint = InMatrix.Multiply(Point);
	Point = StaticCastSharedPtr<FCurve>(TransformedGuideCurve)->EvaluatePoint(GuideUMin);
	FVector NewDirector = TransformedPoint - Point;
