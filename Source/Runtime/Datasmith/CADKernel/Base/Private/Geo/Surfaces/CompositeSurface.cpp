...
void FCompositeSurface::EvaluatePoint(const FVector2d& InSurfacicCoordinate, FSurfacicPoint& OutPoint3D, int32 InDerivativeOrder) const
{
	FVector2d Coordinate = InSurfacicCoordinate;

	Boundary.MoveInsideIfNot(Coordinate);

	FVector2d Point2DPatch;
	FDichotomyFinder FinderU(GlobalCoordinates[EIso::IsoU]);
	int32 PatchUIndex = FinderU.Find(Coordinate.X);
	Point2DPatch.X = (Coordinate.X - GlobalCoordinates[EIso::IsoU][PatchUIndex]) / (GlobalCoordinates[EIso::IsoU][PatchUIndex + 1] - GlobalCoordinates[EIso::IsoU][PatchUIndex]);

	FDichotomyFinder FinderV(GlobalCoordinates[EIso::IsoV]);
	int32 PatchVIndex = FinderV.Find(Coordinate.Y);
	Point2DPatch.Y = (Coordinate.Y - GlobalCoordinates[EIso::IsoV][PatchVIndex]) / (GlobalCoordinates[EIso::IsoV][PatchVIndex + 1] - GlobalCoordinates[EIso::IsoV][PatchVIndex]);

	int32 SurfaceIndex = PatchVIndex * GetSurfNum(EIso::IsoU) + PatchUIndex;

	ensureCADKernel(SurfaceIndex == Surfaces.Num());

	Point2DPatch.X = NativeUVBoundaries[SurfaceIndex][EIso::IsoU].GetAt(Point2DPatch.X);
	Point2DPatch.Y = NativeUVBoundaries[SurfaceIndex][EIso::IsoV].GetAt(Point2DPatch.Y);
