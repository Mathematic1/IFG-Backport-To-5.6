...
void FTorusSurface::EvaluatePointGridInCylindricalSpace(const FCoordinateGrid& Coordinates, TArray<FVector2d>& OutPoints) const
{
...
void FTorusSurface::EvaluatePointGrid(const FCoordinateGrid& Coordinates, FSurfacicSampling& OutPoints, bool bComputeNormals) const
{
	...
	for (double Angle : Coordinates[EIso::IsoV])
	{
		...
		if (bComputeNormals)
		{
			...
			for (int32 Undex = 0; Undex < UCount; Undex++)
			{
				FVector GradientU = FVector(Rho * (-SinU[Undex]), Rho * CosU[Undex], 0.0);
				FVector GradientV = FVector(-Height * CosU[Undex], -Height * SinU[Undex], DeltaHeight);
				OutPoints.Normals.Emplace(GradientU ^ GradientV);
			}
		}
	}

	for (FVector& Point : OutPoints.Points3D)
	{
	...
	if (bComputeNormals)
	{
		FVector3f Center = (FVector3f)Matrix.Column(3);
