...
class FDatasmithUEPbrMaterialElementImpl : public FDatasmithBaseMaterialElementImpl< IDatasmithUEPbrMaterialElement >
{
public:
	UE_API explicit FDatasmithUEPbrMaterialElementImpl( const TCHAR* InName );
	virtual ~FDatasmithUEPbrMaterialElementImpl() = default;

	UE_API virtual FMD5Hash CalculateElementHash(bool bForce) override;
	...
	UE_API virtual IDatasmithMaterialExpression* GetExpression( int32 Index ) override;
	UE_API virtual int32 GetExpressionIndex( const IDatasmithMaterialExpression* Expression ) const override;

	UE_API virtual IDatasmithMaterialExpression* AddMaterialExpression( const EDatasmithMaterialExpressionType ExpressionType ) override;

	UE_API virtual void ResetExpressionGraph( bool bRemoveAllExpressions ) override;

	virtual void SetParentLabel( const TCHAR* InParentLabel ) override { ParentLabel = InParentLabel; }
	UE_API virtual const TCHAR* GetParentLabel() const override;
	...
	UE_API virtual void CustomSerialize(class DirectLink::FSnapshotProxy& Ar) override;
