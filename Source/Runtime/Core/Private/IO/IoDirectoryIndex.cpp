...
{
public:
	FIoStatus Initialize(TConstArrayView<uint8> InBuffer, FAES::FAESKey InDecryptionKey)
	{
		if (InBuffer.Num() == 0)
		{
		...
		{
			TArray<uint8, TInlineAllocator<2048>> Bytes(InBuffer); // temp buffer for dencryption
			FAES::DecryptData(Bytes.GetData(), Bytes.Num(), InDecryptionKey);
			FMemoryReaderView Ar(MakeArrayView<const uint8>(Bytes.GetData(), Bytes.Num()));
			Ar << DirectoryIndex;
		}
		else
		{
...
FIoStatus FIoDirectoryIndexReader::Initialize(TConstArrayView<uint8> InBuffer, FAES::FAESKey InDecryptionKey)
{
