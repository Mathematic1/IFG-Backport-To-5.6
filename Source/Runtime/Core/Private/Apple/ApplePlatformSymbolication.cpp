	...
	if(!bOK && GAllowApplePlatformSymbolication && !CSIsNull(Database.AppleDB) && (ModuleSignature.Len()))
	{
		...
		if(!CSIsNull(SymbolOwner))
		{
			...
			if(!CSIsNull(Symbol))
			{
				...
				if(FunctionName)
				{
					FCStringAnsi::Strncpy(Info.FunctionName, FunctionName, UE_ARRAY_COUNT(Info.FunctionName));
				}
				...
				if(!CSIsNull(SymbolInfo))
				{
					...
					if(FileName)
					{
						FCStringAnsi::Strncpy(Info.Filename, FileName, UE_ARRAY_COUNT(Info.FunctionName));
					}
				}
	...
	if (GAllowApplePlatformSymbolication)
	{
		...
		if(!CSIsNull(Symbolicator))
		{
			...
			if(!CSIsNull(Symbol))
			{
				...
				FGenericPlatformStackWalk::CopyVirtualPathToLocal(out_SymbolInfo.Filename, UE_ARRAY_COUNT(out_SymbolInfo.Filename), CSSourceInfoGetPath(Symbol));
				FCStringAnsi::Strncpy(out_SymbolInfo.FunctionName, CSSymbolGetName(CSSourceInfoGetSymbol(Symbol)), UE_ARRAY_COUNT(out_SymbolInfo.FunctionName));
				...
				if(!CSIsNull(Owner))
				{
					...
					FCStringAnsi::Strncpy(out_SymbolInfo.ModuleName, DylibName, UE_ARRAY_COUNT(out_SymbolInfo.ModuleName));
