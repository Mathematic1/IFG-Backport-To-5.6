...
const TCHAR* FApplePlatformProcess::UserTempDir()
{
	...
	if (!TempDir.Len())
	{
		// The reason we use TMPDIR here is to support applications being detoured/remotely distributed (ShaderCompileWorker)
#if IS_PROGRAM
		TempDir = getenv("TMPDIR");
#else
		TempDir = NSTemporaryDirectory(); // NSTemporaryDirectory() can return a different temp directory than TMPDIR and detouring is not supported.
#endif
	}
	return *TempDir;
}
