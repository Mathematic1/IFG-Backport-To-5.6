...
void FBitWriter::SerializeBits( void* Src, int64 LengthBits )
{
	if( AllowAppend(LengthBits) )
	{
		...
		{
			// Just use memcpy when setting bits directy after Reset()
			FMemory::Memcpy(Buffer.GetData(), Src, (LengthBits + 7) / 8);
			Num = LengthBits;
		}
		else
		{
...
void FBitWriter::SetOverflowed(int32 LengthBits)
{
	if (!bAllowOverflow)
	{
		UE_LOG(LogNetSerialization, Error, TEXT("FBitWriter overflowed! (WriteLen: %" INT64_X_FMT ", Remaining: %" INT64_X_FMT ", Max: %" INT64_X_FMT ")"),
