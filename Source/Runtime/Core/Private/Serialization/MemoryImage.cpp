...
bool FPointerTableBase::LoadFromArchive(FArchive& Ar, const FPlatformTypeLayoutParameters& LayoutParams, void* FrozenObject)
{
	...
	for (int32 i = 0u; i < NumDependencies; ++i)
	{
		...
		{
			// When the content was cooked with a different build configuration than the editor, some type layouts are unknown to the editor build.
			// This condition can be considered a benign warning, so we only emit it as a Verbose log entry.
			UE_LOG(LogMemoryImage, Verbose, TEXT("Unknown TypeLayout %s"), *Name.ToString());
		}
