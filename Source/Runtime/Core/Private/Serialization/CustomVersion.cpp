...
FCustomVersionContainer FCurrentCustomVersions::GetAll()
{
	...
	{
		UE::TReadScopeLock Scope(Registry.Lock);
	...
	UE::TWriteScopeLock Scope(Registry.Lock);
...
void FCustomVersionContainer::Serialize(FStructuredArchive::FSlot Slot, ECustomVersionSerializationFormat Format)
{
	switch (Format)
	{
	default: check(false);
