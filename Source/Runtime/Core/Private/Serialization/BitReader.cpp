...
void FBitReader::SetOverflowed(int64 LengthBits)
{
	...
	{
		UE_LOG(LogNetSerialization, Fatal, TEXT("FBitReader::SetOverflowed() called! (ReadLen: %" INT64_X_FMT ", Remaining: %" INT64_X_FMT ", Max: %" INT64_X_FMT ")"),
				LengthBits, (Num - Pos), Num);
	}
	else if (GEnsureOnOverflow)
	{
		ensureMsgf(false, TEXT("FBitReader::SetOverflowed() called! (ReadLen: %" INT64_X_FMT ", Remaining: %" INT64_X_FMT ", Max: %" INT64_X_FMT ")"),
					LengthBits, (Num - Pos), Num);
	}
	else
	{
		UE_LOG(LogNetSerialization, Error, TEXT("FBitReader::SetOverflowed() called! (ReadLen: %" INT64_X_FMT ", Remaining: %" INT64_X_FMT ", Max: %" INT64_X_FMT ")"),
				LengthBits, (Num - Pos), Num);
	}
