...
#include "Unix/UnixPlatformMutex.h"
...
FUnixSystemWideMutex::FUnixSystemWideMutex(const FString& InName, FTimespan InTimeout)
{
	check(InName.Len() > 0);
...
FUnixSystemWideMutex::~FUnixSystemWideMutex()
{
	Release();
}

bool FUnixSystemWideMutex::IsValid() const
{
	return FileHandle != -1;
}

void FUnixSystemWideMutex::Release()
{
	if (IsValid())
	{
