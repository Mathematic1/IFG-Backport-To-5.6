...
struct OodleScratchBuffers
{
	...
	int64 OodleDecode(const void * InCompBuf, int64 InCompBufSize64, void * OutRawBuf, int64 InRawLen64) 
	{
		...
		if ( InCompBufSize <= 0 )
		{
			UE_LOG(OodleDataCompression, Display, TEXT("OodleDecode: no compressed buffer size to decode!"));
			return OODLELZ_FAILED;
		}
		else if ( static_cast<const OO_U8 *>(InCompBuf)[0] == 0 )
		{
			UE_LOG(OodleDataCompression, Display, TEXT("OodleDecode: compressed buffer starts with zero byte; invalid or corrupt compressed stream."));
			return OODLELZ_FAILED;
		}
		...
		for (int i = 0; i < OodleScratchBufferCount; ++i)
		{
			if (OodleScratches[i].OodleScratchMemoryMutex.TryLock()) 
			{
				if (OodleScratches[i].OodleScratchMemory == nullptr)
				{
					...
					if (OodleScratches[i].OodleScratchMemory == nullptr)
					{
						UE_LOG(OodleDataCompression, Error, TEXT("OodleDecode: Failed to allocate scratch buffer %" PTRINT_FMT " bytes!"), OodleScratchMemorySize);
						return OODLELZ_FAILED;
					}
				}
		...
		{
			UE_LOG(OodleDataCompression, Display, TEXT("OodleDecode : no Oodle compressor found!; likely an invalid or corrupt compressed stream."));
			return OODLELZ_FAILED;
		}
		...
		{
			UE_LOG(OodleDataCompression, Display, TEXT("OodleDecode : MemorySizeNeeded invalid; likely an invalid or corrupt compressed stream."));
			return OODLELZ_FAILED;
		}
		...
		for (int i = 0; i < OodleScratchBufferCount; ++i)
		{
			if (OodleScratches[i].OodleScratchMemoryMutex.TryLock()) 
			{
				if (OodleScratches[i].OodleScratchMemory == nullptr)
				{
					...
					if (OodleScratches[i].OodleScratchMemory == nullptr)
					{
						UE_LOG(OodleDataCompression, Error, TEXT("OodleEncode: Failed to allocate scratch buffer %" PTRINT_FMT " bytes!"), OodleScratchMemorySize);
						return OODLELZ_FAILED;
					}
				}
						...
						)
{
	...
	{
		UE_LOG(OodleDataCompression,Display,TEXT("OodleLZ_FillSeekTable failed\n"));
		return false;
	}
