...
FTimespan FMonitoredProcess::GetDuration() const
{
	if (StartTime.Load() == 0)
	{
		// Not initialized yet - monitoring thread might not have set StartTime yet.
		return 0;
	}
	if (bIsRunning)
	{
		return (FDateTime::UtcNow() - StartTime.Load());
	}

	return (EndTime.Load() - StartTime.Load());
}
...
bool FMonitoredProcess::Launch()
{
	...
	{
		StartTime.Store(FDateTime::UtcNow());
	}
...
{
	check((!FPlatformProcess::SupportsMultithreading()) || (!bIsRunning));	// Don't touch the output buffer while running with a background task
	return OutputBuffer;
}
...
void FMonitoredProcess::TickInternal()
{
	...
	if (Canceling)
	{
		...
		EndTime.Store(FDateTime::UtcNow());
	...
	else if (!FPlatformProcess::IsProcRunning(ProcessHandle))
	{
		...
		{
			ReturnCode.Store(ProcessReturnCode);
		}
		else
		{
			ReturnCode.Store(-1);
