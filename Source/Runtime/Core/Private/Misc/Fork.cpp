...
void FForkProcessHelper::OnForkingOccured()
{
	...
	{
		ensureMsgf(UE::Private::GMalloc->IsInternallyThreadSafe(), TEXT("The BaseAllocator %s is not threadsafe. Switch to a multithread allocator or ensure the FMallocThreadSafeProxy wraps it."), UE::Private::GMalloc->GetDescriptiveName());
