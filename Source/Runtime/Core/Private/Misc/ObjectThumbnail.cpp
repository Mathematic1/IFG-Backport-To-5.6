...
void FObjectThumbnail::Serialize(FStructuredArchive::FSlot Slot)
{
	...
	if (Slot.GetArchiveState().IsLoading())
	{
		...
		if ((ImageWidth == 0) || (ImageHeight == 0))
		{
			// The thumbnail data doesn't exist and might be corrupt. Attempting to read the
			// image data may lead to crashes. Mark the thumbnail dirtsy so we may regenerate it
			bIsDirty = true; 
			return;
		}
	}
