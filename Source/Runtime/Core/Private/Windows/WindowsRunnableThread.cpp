	...
	else
	{
		...
		__except (FPlatformMisc::GetCrashHandlingType() == ECrashHandlingType::Default ? ReportCrash(GetExceptionInformation()) : EXCEPTION_CONTINUE_SEARCH)
		{
			__try
			{
				...
				FCString::StrncatTruncateDest( GErrorHist, UE_ARRAY_COUNT(GErrorHist), TEXT(LINE_TERMINATOR_ANSI "Crash in runnable thread " ) );
				FCString::StrncatTruncateDest( GErrorHist, UE_ARRAY_COUNT(GErrorHist), *ThreadName );
