...
void FInstanceCountingObject::IncrementStats()
{
	...
	if (DoLog)
	{
		UE_LOG(LogInstanceCount, Log, TEXT("Constructed %s at 0x%08" UPTRINT_X_FMT ", count=%d"), *Name.ToString(), this, Count);
	}
}
...
void FInstanceCountingObject::DecrementStats()
{
	...
	if (DoLog)
	{
		UE_LOG(LogInstanceCount, Log, TEXT("Destructed %s at 0x%08" UPTRINT_X_FMT ", count=%d"), *Name.ToString(), this, Count);
	}
}
