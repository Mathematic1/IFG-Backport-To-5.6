		...
		switch (*Src)
		{
			...
			case 'g':
			{
				...
				if (RetCnt >= UE_ARRAY_COUNT(AnsiNum))
				{
					...
					checkf(0, TEXT("Attempting to read past the size our buffer. Buffer Size: %" SIZE_T_FMT " Size to read: %d. Current contents: '%s'\n"), UE_ARRAY_COUNT(AnsiNum), RetCnt, UTF8_TO_TCHAR(AnsiNum));
				}
				if (RetCnt < 0 || !DestIter.Write(AnsiNum, RetCnt))
				{
					return -1;
