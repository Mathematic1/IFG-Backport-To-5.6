...
struct FMallocBinned::Private
{
	...
	inline static UE::FPlatformRecursiveMutex SmallLock;
	...
	static FFreeMem* GetAllocFromSmallBlockGrab(FMallocBinned& Allocator, UPTRINT OsBytes)
	{
		...
		UE::TUniqueLock ScopedLock(SmallLock);
	...
	*/
	...
	static FPoolInfo* AllocatePoolMemory(FMallocBinned& Allocator, FPoolTable* Table, uint32 PoolSize, uint16 TableIndex)
	{
		...
		FPoolInfo* Pool;
		{
			...
			UE::TUniqueLock PoolInfoLock(Allocator.AccessGuard);
#endif
	...
	*/
...
#ifdef USE_FINE_GRAIN_LOCKS
			UE::TUniqueLock TableLock(Table->CriticalSection);
#endif
#if STATS
			Table->ActiveRequests--;
#endif
	...
	static void PushFreeLockless(FMallocBinned& Allocator, void* Ptr)
	{
		...
		UE::TUniqueLock ScopedLock(Allocator.AccessGuard);
#endif
...
#ifdef USE_FINE_GRAIN_LOCKS
		UE::TUniqueLock MainLock(Allocator.AccessGuard);
#endif
...
void FMallocBinned::GetAllocatorStats( FGenericMemoryStats& out_Stats )
{
	...
	
	{
		...
		UE::TUniqueLock ScopedLock(AccessGuard);
#endif
...
void* FMallocBinned::Malloc(SIZE_T Size, uint32 Alignment)
{
	...
	if (bUsePools)
	{
		...
		else
		{
			...
			{
#ifdef USE_FINE_GRAIN_LOCKS
				UE::TUniqueLock PoolInfoLock(AccessGuard);
#endif
	...
	if( Ptr && NewSize )
	{
		...
		if(FPlatformMemory::PtrIsOSMalloc(Ptr))
		{
            ...
            if (bFallback)
            {
                ...
                FMemory::Memcpy(NewPtr, Ptr, FMath::Min(OldSize, NewSize));
