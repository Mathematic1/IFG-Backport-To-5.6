...
void* FPooledVirtualMemoryAllocator::Allocate(SIZE_T Size, uint32 AllocationHint, UE::FPlatformRecursiveMutex* Mutex)
{
	if (Size > Limits::MaxAllocationSizeToPool)
	{
		...
		{
			// do not report to LLM here, the platform functions will do that
			UE::TUniqueLock Lock(OsAllocatorCacheLock);
	...
	else
	{
		...
		UE::TUniqueLock Lock(ClassesLocks[SizeClass]);
...
void FPooledVirtualMemoryAllocator::Free(void* Ptr, SIZE_T Size, UE::FPlatformRecursiveMutex* Mutex, bool /*ThreadIsTimeCritical = false*/)
{
	if (Size > Limits::MaxAllocationSizeToPool)
	{
		if (OsAllocatorCache.IsOSAllocation(Size))
...
void FPooledVirtualMemoryAllocator::FreeAll(UE::FPlatformRecursiveMutex* /*Mutex = nullptr*/)
{
	UE::TUniqueLock Lock(OsAllocatorCacheLock);
...
uint64 FPooledVirtualMemoryAllocator::GetCachedFreeTotal() const
{
	...
	{
		UE::TUniqueLock Lock(ClassesLocks[IdxSizeClass]);
