...
FMallocLeakDetection::~FMallocLeakDetection()
{
	bRecursive = true; // OpenPointers.Empty() may reallocate / free.
...
{
	FScopeLock Lock(&AllocatedPointersCritical);
	CallstackHashType CallstackHash = Callstack.GetHash();
...
void FMallocLeakDetection::GetOpenCallstacks(TArray<CallstackHashType>& OutCallstacks, SIZE_T& OutTotalSize, const FMallocLeakReportOptions& Options /*= FMallocLeakReportOptions()*/)
{
	...
	TMap<CallstackHashType, float> HashesToAllocRate;
...

	{
		...
		// now sort
		OutCallstacks.Sort([this, &Options, &HashesToAllocRate](CallstackHashType lhs, CallstackHashType rhs) {
...
int32 FMallocLeakDetection::DumpOpenCallstacks(const TCHAR* FileName, const FMallocLeakReportOptions& Options /*= FMallocLeakReportOptions()*/)
{
	...
	SIZE_T TotalSize = 0;
	TArray<CallstackHashType> SortedKeys;
