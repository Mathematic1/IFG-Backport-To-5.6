...
namespace ICUUtilities
{
	...
	void FStringConverter::ConvertString(FStringView Source, icu::UnicodeString& Destination, const bool ShouldNullTerminate)
	{
		ConvertString(Source.GetData(), 0, Source.Len(), Destination, ShouldNullTerminate);
	}
	...
	icu::UnicodeString FStringConverter::ConvertString(FStringView Source, const bool ShouldNullTerminate)
	{
	...
	void ConvertString(FStringView Source, icu::UnicodeString& Destination, const bool ShouldNullTerminate)
	{
		if (Source.Len() > 0)
		{
	...
	icu::UnicodeString ConvertString(FStringView Source, const bool ShouldNullTerminate)
	{
		icu::UnicodeString Destination;
		ConvertString(Source, Destination, ShouldNullTerminate);
		return Destination;
	...
	{
		return GetUnicodeStringLength(Source.GetData(), 0, Source.Len());
	}
