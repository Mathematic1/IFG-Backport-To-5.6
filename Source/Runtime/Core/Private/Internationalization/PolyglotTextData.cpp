...
{
	checkf(!Key.IsEmpty(), TEXT("Polyglot data cannot have an empty key! (Namespace: '%s', Key: '%s', Native String: '%s')"), *Namespace, *Key, *NativeString);
	checkf(!NativeString.IsEmpty(), TEXT("Polyglot data cannot have an empty native string! (Namespace: '%s', Key: '%s', Native String: '%s')"), *Namespace, *Key, *NativeString);
}
