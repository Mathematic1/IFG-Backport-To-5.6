...
static int32 bFastDecimalFormatCanIndicateIfNearlyInteger = 1;
static FAutoConsoleVariableRef CVarIndicateIfNearlyInteger(
	TEXT("Core.bFastDecimalFormatCanIndicateIfNearlyInteger"),
	bFastDecimalFormatCanIndicateIfNearlyInteger,
	TEXT("True implies we will indicate nearly integer values with '...' - such as 1e-16 which would normally be displayed as 0."));
...
public:
	FDecimalNumberSigningStrings(const FDecimalNumberFormattingRules& InFormattingRules, const EDecimalNumberSigningStringsFlags InFlags)
	{
		...
		if (EnumHasAnyFlags(InFlags, EDecimalNumberSigningStringsFlags::UseASCIISigns))
		{
			...
			if (!InFormattingRules.NegativePrefixString.IsEmpty())
			{
				NegativePrefixStringPtr = &GeneratedNegativePrefixString;
			}
			if (!InFormattingRules.NegativeSuffixString.IsEmpty())
			{
				NegativeSuffixStringPtr = &GeneratedNegativeSuffixString;
			}
			if (!InFormattingRules.PositivePrefixString.IsEmpty())
			{
				PositivePrefixStringPtr = &GeneratedPositivePrefixString;
			}
			if (!InFormattingRules.PositiveSuffixString.IsEmpty())
			{
				PositiveSuffixStringPtr = &GeneratedPositiveSuffixString;
			}
		}
	...
	{
		return *NegativePrefixStringPtr;
	}

	const FString& GetNegativeSuffixString() const
	{
		return *NegativeSuffixStringPtr;
	}

	const FString& GetPositivePrefixString() const
	{
		return *PositivePrefixStringPtr;
	}

	const FString& GetPositiveSuffixString() const
	{
		return *PositiveSuffixStringPtr;
	}

private:
	const FString* NegativePrefixStringPtr;
	const FString* NegativeSuffixStringPtr;
	const FString* PositivePrefixStringPtr;
	const FString* PositiveSuffixStringPtr;
...
void FractionalToString(const double InVal, const FDecimalNumberFormattingRules& InFormattingRules, FNumberFormattingOptions InFormattingOptions, FString& OutString)
{
	...
	// Capture if nearly integer so we can warn users about denorms / near zero values being displayed as zero (Ex: 1e-16)
	const bool bNearlyInteger = bFastDecimalFormatCanIndicateIfNearlyInteger 
		&& InFormattingOptions.IndicateNearlyInteger 
		&& FMath::IsNearlyEqual(IntegralPart, InVal) 
		&& IntegralPart != InVal;
	...
	// Append an '...' if the value is nearly an integer
	if (bNearlyInteger && FractionalPartLen + 3 < UE_ARRAY_COUNT(FractionalPartBuffer))
	{
		FractionalPartBuffer[FractionalPartLen++] = '.';
		FractionalPartBuffer[FractionalPartLen++] = '.';
		FractionalPartBuffer[FractionalPartLen++] = '.';
		FractionalPartBuffer[FractionalPartLen] = TCHAR('\0');
	}
