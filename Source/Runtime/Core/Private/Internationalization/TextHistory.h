...
public:
	...
	//~ IRefCountedObject
	virtual FReturnedRefCountValue AddRef() const override final { return TRefCountingMixin<FTextHistory>::AddRef(); }
...
private:
	...
	/** Mutex to prevent races when multiple threads are updating out of date state. */
	mutable UE::FTransactionallySafeMutex Mutex = UE::FTransactionallySafeMutex();
};
...
private:
	...
	private:
		...
		mutable FTransactionallySafeCriticalSection DataCS;
	};
