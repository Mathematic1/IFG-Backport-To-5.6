...
inline FFrameTime ConvertFrameTime(FFrameTime SourceTime, FFrameRate SourceRate, FFrameRate DestinationRate)
{
	...
	// If SourceRate.Numerator is 0 then NewDenominator will be 0 and will cause a divide by 0 error.
	check(SourceRate.Numerator != 0);
 ...
 * Note: user code may need to #include "Misc/ExpressionParserTypes.h"
 */
