	...
	public:
		...
		static void MergeCommandlineWithSaved(TCHAR CommandLine[16384])
		{
			...
			if (CommandLine[0] == '-' || CommandLine[0] == 0)
			{
				...
				FCString::Strncpy(CommandLine, *CombinedCommandLine, CMD_LINE_MAX);
			}
			else
			{
				...
				if (Space == NULL)
				{
					...
					FCString::StrncatTruncateDest(CommandLine, CMD_LINE_MAX, *GSavedCommandLine);
				}
				else
				{
					...
					FCString::Strncpy(Space, *GSavedCommandLine, CMD_LINE_MAX - (Space - CommandLine));
					// now put back the 2nd and so on token
					FCString::StrncatTruncateDest(CommandLine, CMD_LINE_MAX, *AfterSpace);
				}
			}
		}
