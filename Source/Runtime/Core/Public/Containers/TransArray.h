...
public:
	...
	// Constructors.
	[[nodiscard]] explicit TTransArray( UObject* InOwner )
	:	Owner( InOwner )
	{
		checkSlow(Owner);
	}

	[[nodiscard]] TTransArray( UObject* InOwner, const Super& Other )
	:	Super( Other )
	,	Owner( InOwner )
	{
		checkSlow(Owner);
	...
	[[nodiscard]] TTransArray(TTransArray&&) = default;
	[[nodiscard]] TTransArray(const TTransArray&) = default;
