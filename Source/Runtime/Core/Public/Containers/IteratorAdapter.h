...
public:
	...
	[[nodiscard]] TIteratorAdapter() = default;

	/**
	 * Perfect forwarding constructor to the iterator class.
	 */
	template < typename... Args>
	[[nodiscard]] TIteratorAdapter(EInPlace, Args&&... InArgs) : BaseType(Forward<Args>(InArgs)...)
	{
	}
...
public:
	[[nodiscard]] bool operator==(const TIteratorAdapter& Right) const
	{
		return Equals(Right);
	}
