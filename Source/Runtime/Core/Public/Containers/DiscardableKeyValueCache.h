...
public:
	...
	[[nodiscard]] TDiscardableKeyValueCache() :
		CurrentMap(&Map1)
		, BackfillMap(&Map2)
	{
	}
	...
	[[nodiscard]] FRWLock&  RWLock()
	{
		return LockObject;
	}

	/* Reference to the current map */
	[[nodiscard]] TypedMap& Current()
	{
		return *CurrentMap;
	}

	/* Reference to the current map */
	[[nodiscard]] TypedMap& Backfill()
	{
		return *BackfillMap;
	}
	...
	*/
	[[nodiscard]] bool Find(const KeyType& Key, ValueType& OutType)
	{
		uint32 LockFlags = ApplyLock(0, LockFlags::ReadLock);
	 ...
	 */
	[[nodiscard]] bool Find(const KeyType& Key, ValueType& OutType, uint32 InCurrentLockFlags, uint32& OutLockFlags)
	{
...
protected:
	...
	*/
	[[nodiscard]] bool  InternalFindWhileLocked(const KeyType& Key, ValueType& OutType, uint32 InCurrentLockFlags, uint32& OutFlags)
	{
