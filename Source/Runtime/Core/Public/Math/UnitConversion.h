...
public:
	...
	[[nodiscard]] CORE_API FUnitSettings();
...
struct FUnitConversion
{
	...
	[[nodiscard]] static CORE_API FUnitSettings& Settings();

	/** Check whether it is possible to convert a number between the two specified units */
	[[nodiscard]] static CORE_API bool AreUnitsCompatible(EUnit From, EUnit To);

	/** Check whether a unit is of the specified type */
	[[nodiscard]] static CORE_API bool IsUnitOfType(EUnit Unit, EUnitType Type);

	/** Get the type of the specified unit */
	[[nodiscard]] static CORE_API EUnitType GetUnitType(EUnit);

	/** Get the display string for the the specified unit type */
	[[nodiscard]] static CORE_API const TCHAR* GetUnitDisplayString(EUnit Unit);

	/** Helper function to find a unit from a string (name or display string) */
	[[nodiscard]] static CORE_API TOptional<EUnit> UnitFromString(const TCHAR* UnitString);

	/** Helper function to get all supported units */
	[[nodiscard]] static CORE_API TConstArrayView<const TCHAR*> GetSupportedUnits();

public:

	/** Convert the specified number from one unit to another. Does nothing if the units are incompatible. */
	template<typename T>
	[[nodiscard]] static T Convert(T InValue, EUnit From, EUnit To);

	/** Quantizes this number to the most appropriate unit for user friendly presentation (e.g. 1000m returns 1km). */
	template<typename T>
	[[nodiscard]] static FNumericUnit<T> QuantizeUnitsToBestFit(T Value, EUnit Units);
	...
	template<typename T>
	[[nodiscard]] static EUnit CalculateDisplayUnit(T Value, EUnit InUnits);
...
struct FNumericUnit
{
	...
	/** Constructors */
	[[nodiscard]] FNumericUnit();
	[[nodiscard]] FNumericUnit(const NumericType& InValue, EUnit InUnits = EUnit::Unspecified);

	/** Copy construction/assignment from the same type */
	[[nodiscard]] FNumericUnit(const FNumericUnit& Other);
	...
	[[nodiscard]] FNumericUnit(const FNumericUnit<OtherType>& Other);
	template<typename OtherType>
	FNumericUnit& operator=(const FNumericUnit<OtherType>& Other);

	/** Convert this quantity to a different unit */
	[[nodiscard]] TOptional<FNumericUnit<NumericType>> ConvertTo(EUnit ToUnits) const;

public:

	/** Quantizes this number to the most appropriate unit for user friendly presentation (e.g. 1000m returns 1km). */
	[[nodiscard]] FNumericUnit<NumericType> QuantizeUnitsToBestFit() const;
...
private:
	...
	operator const NumericType&() const = delete;
	...
	template<typename OtherType>
	[[nodiscard]] friend inline bool operator==(const FNumericUnit<NumericType>& LHS, const FNumericUnit<OtherType>& RHS)
	{
