...
public:
	...
	[[nodiscard]] TScale() :Scale(1.0f) {}
	/** Ctor. initialize from a uniform scale. */
	[[nodiscard]] explicit TScale(T InScale) :Scale(InScale) {}
	/** Ctor. initialize from an UE::Math::TVector<T> defining the 3D scale. */
	template<typename VType>
	[[nodiscard]] explicit TScale(const UE::Math::TVector<VType>& InScale) : Scale((Vector3Type)InScale) {}
	/** Access to the underlying UE::Math::TVector<T> that stores the scale. */
	[[nodiscard]] const Vector3Type& GetVector() const
	{
		return Scale;
	}
	/** Concatenate two scales. */
	[[nodiscard]] TScale Concatenate(const TScale& RHS) const
	{
		return TScale(Scale * RHS.GetVector());
	}
	/** Invert the scale. */
	[[nodiscard]]  TScale Inverse() const
	{
