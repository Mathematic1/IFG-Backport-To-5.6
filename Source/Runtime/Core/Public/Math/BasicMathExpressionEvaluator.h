...
public:
	...
	[[nodiscard]] CORE_API FBasicMathExpressionEvaluator();

	/** Evaluate the given expression, resulting in either a double value, or an error */
	[[nodiscard]] CORE_API TValueOrError<double, FExpressionError> Evaluate(const TCHAR* InExpression, double InExistingValue = 0) const;
	 ...
	 */
	CORE_API static void DefineArithmeticTokens(FTokenDefinitions& TokenDefinitions);
	CORE_API static void DefineArithmeticOperators(FExpressionGrammar& TokenDefinitions);
	CORE_API static void MapArithmeticOperators(FOperatorJumpTable& JumpTable);
