...
namespace ConvexHull2D
{
	...
	[[nodiscard]] inline FVector::FReal ComputeDeterminant(const FVector& A, const FVector& B, const FVector& C)
	{
	...
	[[nodiscard]] inline bool ComparePoints(const FVector& A, const FVector& B)
	{
		if (A.X < B.X)
		{
			return true;
	...
	/** Returns <0 if C is left of A-B */
	[[nodiscard]] inline FVector2D::FReal ComputeDeterminant2D(const FVector2D& A, const FVector2D& B, const FVector2D& C)
	{
