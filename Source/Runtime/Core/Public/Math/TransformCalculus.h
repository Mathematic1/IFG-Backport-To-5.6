...
[[nodiscard]] inline decltype(auto) TransformCast(const TransformType& Transform)
{
 ...
 */
template<typename TransformTypeA, typename TransformTypeB>
[[nodiscard]] inline typename ConcatenateRules<TransformTypeA, TransformTypeB>::ResultType Concatenate(const TransformTypeA& LHS, const TransformTypeB& RHS)
{
...
[[nodiscard]] inline ReturnType Concatenate(const LHSType& LHS, const RHSType& RHS)
{
	return Concatenate(TransformCast<ReturnType>(LHS), TransformCast<ReturnType>(RHS));
}
 ...
 */
template<typename TransformType>
[[nodiscard]] inline auto Concatenate(const TransformType& LHS, const TransformType& RHS) -> decltype(LHS.Concatenate(RHS))
{
	// If you get a compiler error here about "no member function Concatenate found for TransformType" you know
	// your transform type doesn't support a Concatenate method. Either add one or provide an overload of Concatenate that does this.
...
/**
 * Concatenates three or more transforms. Uses two-argument Concatenate to do its work,
 ...
 * @param TransformCToRest Transformation that goes from space C onwards.
 * @return a new Transform representing the complete transformation.
 */
template<typename TransformType1, typename TransformType2, typename... TransformTypeRest>
[[nodiscard]] inline decltype(auto) Concatenate(const TransformType1& TransformAToB, const TransformType2& TransformBToC, TransformTypeRest&&... TransformCToRest)
{
	return Concatenate(Concatenate(TransformAToB, TransformBToC), Forward<TransformTypeRest>(TransformCToRest)...);
}
 ...
 */
template<typename TransformType>
[[nodiscard]] inline auto Inverse(const TransformType& Transform) -> decltype(Transform.Inverse())
{
...
[[nodiscard]] inline PositionType TransformPoint(const TransformType& Transform, const PositionType& Point)
{
...
[[nodiscard]] inline VectorType TransformVector(const TransformType& Transform, const VectorType& Vector)
{
...
[[nodiscard]] inline FloatType Concatenate(FloatType LHS, FloatType RHS)
{
	return LHS * RHS;
}
 ...
 */
template<typename FloatType UE_REQUIRES(std::is_floating_point_v<FloatType>)>
[[nodiscard]] inline FloatType Inverse(FloatType Scale)
{
	return 1.0f / Scale;
}
...
[[nodiscard]] inline UE::Math::TVector<PositionType> TransformPoint(float Transform, const UE::Math::TVector<PositionType>& Point)
{
	return Transform * Point;
}

template<typename PositionType>
[[nodiscard]] inline UE::Math::TVector<PositionType> TransformPoint(double Transform, const UE::Math::TVector<PositionType>& Point)
{
	return Transform * Point;
}
...
[[nodiscard]] inline UE::Math::TVector<VectorType> TransformVector(float Transform, const UE::Math::TVector<VectorType>& Vector)
{
	return Transform * Vector;
}

template<typename VectorType>
[[nodiscard]] inline UE::Math::TVector<VectorType> TransformVector(double Transform, const UE::Math::TVector<VectorType>& Vector)
{
	return Transform * Vector;
}
