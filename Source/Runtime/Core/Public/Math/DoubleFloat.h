...
struct FDFVector2
{
	[[nodiscard]] FVector2d GetVector2d() const
	{
		return FVector2d{ X().GetDouble(), Y().GetDouble() };
	}

	template<typename TInputScalar = double>
	[[nodiscard]] FDFVector2(const UE::Math::TVector2<TInputScalar>& In)
	{
		FDFScalar X(In.X);
		FDFScalar Y(In.Y);
		High = { X.High, Y.High };
	...
	[[nodiscard]] FDFVector2(FVector2f High, FVector2f Low)
		: High(High), Low(Low)
	{}

	[[nodiscard]] FDFVector2() = default;

	[[nodiscard]] FDFScalar X() const
	{
		return { High.X, Low.X };
	}

	[[nodiscard]] FDFScalar Y() const
	{
		return { High.Y, Low.Y };
	}
...
struct FDFVector3
{
	[[nodiscard]] FVector3d GetVector3d() const
	{
...
struct FDFVector4
{
	[[nodiscard]] FVector4d GetVector4d() const
	{
...
struct FDFMatrix
{
	...
	[[nodiscard]] CORE_API static FMatrix44f SafeCastMatrix(const FMatrix& Matrix);

	// Apply post-translation to matrix
	[[nodiscard]] CORE_API static FDFMatrix MakeToRelativeWorldMatrix(const FVector3f Origin, const FMatrix& ToWorld);
	[[nodiscard]] CORE_API static FMatrix   MakeToRelativeWorldMatrixDouble(const FVector Origin, const FMatrix& ToWorld);

	[[nodiscard]] CORE_API static FDFMatrix MakeClampedToRelativeWorldMatrix(const FVector3f Origin, const FMatrix& ToWorld);
	[[nodiscard]] CORE_API static FMatrix   MakeClampedToRelativeWorldMatrixDouble(const FVector Origin, const FMatrix& ToWorld);
};
...
struct FDFInverseMatrix
{
	...
	[[nodiscard]] CORE_API static FDFInverseMatrix MakeFromRelativeWorldMatrix(const FVector3f Origin, const FMatrix& FromWorld);
	[[nodiscard]] CORE_API static FMatrix          MakeFromRelativeWorldMatrixDouble(const FVector Origin, const FMatrix& FromWorld);
};
