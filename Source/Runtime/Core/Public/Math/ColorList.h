...
public:
	...
	[[nodiscard]] CORE_API const FColor& GetFColorByName( const TCHAR* ColorName ) const;

	/** Returns a linear color based on ColorName. If not found, returns White. */
	[[nodiscard]] CORE_API const FLinearColor GetFLinearColorByName( const TCHAR* ColorName ) const;

	/** Returns true if color is valid common colors. If not found returns false. */
	[[nodiscard]] CORE_API bool IsValidColorName( const TCHAR* ColorName ) const;

	/** Returns index of color based on ColorName. If not found returns 0. */
	[[nodiscard]] CORE_API int32 GetColorIndex( const TCHAR* ColorName ) const;

	/** Returns a color based on index. If index is invalid, returns White. */
	[[nodiscard]] CORE_API const FColor& GetFColorByIndex( int32 ColorIndex ) const;

	/** Returns color's name based on index. If index is invalid, returns BadIndex. */
	[[nodiscard]] CORE_API const FString& GetColorNameByIndex( int32 ColorIndex ) const;

	/** Returns the number of colors. */
	[[nodiscard]] int32 GetColorsNum() const
	{
