...
{
public:
	CORE_API FMemScopePtr(uint64 InPtr, HeapId InRootHeap = EMemoryTraceRootHeap::SystemMemory);
...
// UE_MEMSCOPE_PTR(InPtr) or UE_MEMSCOPE_PTR(InPtr, InRootHeap)
#define UE_MEMSCOPE_PTR(InPtr, ...)			FMemScopePtr PREPROCESSOR_JOIN(MemPtrScope,__LINE__)((uint64)InPtr, ##__VA_ARGS__);
