...
public:
	 ...
	 * @param RangeBoundsHint A box to potentially limit the iteration range across cells. Note that
	 *  this might be ignored if the allocation of cells is such that trying to check for cells in that
	 *  range would take more time than just iterating over all allocated cells.
	 * @param BoundsOverlapFn Must be thread-safe. Used to determine if the cell is part
	 *  of the desired range.
	 * @param ObjectIDsOut collected ObjectIDs are stored here
	 */
	GEOMETRYCORE_API void ParallelRangeQuery(
		const FAxisAlignedBox3d& RangeBoundsHint,
		TFunctionRef<bool (const FAxisAlignedBox3d& CellBounds)> BoundsOverlapFn,
		TArray<int32>& ObjectIDs) const;
	 ...
	 * @return index of overlapping ObjectID or INDEX_NONE if no overlaps are found
	 */
	int ParallelOverlapAnyQuery(const FAxisAlignedBox3d& ShapeBounds,
...
private:
	// Gather objects in all cells that pass BoundsOverlapFn, starting from a set of root cells
	void ParallelCollectObjectsInOverlappingCells(
		TArray<const FSparseOctreeCell*, TInlineAllocator<32>> RelevantRootCells,
		TFunctionRef<bool(const FAxisAlignedBox3d& CellBounds)> BoundsOverlapFn,
		TArray<int32>& ObjectIDs) const;
	...
	/**
	 * Helper to find root-level cells that satisfy BoundsOverlapFn
	 * 
	 * @param RangeBounds If the allocation of cells is such that it is worth doing, only cells in these
	 *   bounds are looked for and iterated over. May be ignored if the allocation is such that it is
	 *   faster to iterate over all allocated root cells than to check for allocation in a big range.
	 * @param BoundsOverlapFn Function for determining whether cell should be included in output queue
	 * @param bCheckOverlapFnInRangeBounds If true, then BoundsOverlapFn is still checked when iterating
	 *   over RangeBounds (i.e., RangeBounds is an optimization hint, but not sufficient for determining
	 *   actual overlap).
	 */
	TArray<const FSparseOctreeCell*, TInlineAllocator<32>> InitializeQueryQueue(
		const FAxisAlignedBox3d& RangeBounds, 
		TFunctionRef<bool(const FAxisAlignedBox3d& CellBounds)> BoundsOverlapFn, 
		bool bCheckOverlapFnInRangeBounds = true) const;
	...
	{
		return InitializeQueryQueue(FAxisAlignedBox3d(Point, Point),
			[&Point](const FAxisAlignedBox3d& CellBounds) { return CellBounds.Contains(Point); }, 
			false);
	}

	// helper to find the root-level cells that could intersect with a given query object, specialized for box queries
	TArray<const FSparseOctreeCell*, TInlineAllocator<32>> InitializeQueryQueue(const FAxisAlignedBox3d& Bounds) const
	{
		return InitializeQueryQueue(Bounds,
			[&Bounds](const FAxisAlignedBox3d& CellBounds) { return CellBounds.Intersects(Bounds); },
			false);
	}
};
