...
public:
	...
	{
		return AttribValues.GetAllocatedSize();
	}
public:
	...
	{
		// Appending bone attributes is a special case that must be handled AttributeSet level,
		// because it may require also consistently remapping bone weights
		ensureMsgf(false, TEXT("Directly appending bone attributes is not supported"));
		return false;
	}

	virtual void AppendDefaulted(const UE::Geometry::FDynamicMesh3::FAppendInfo& Info) override
	{
		// bone attributes are not attached to mesh vertices/triangles/edges, so there is no work to do in the append-defaulted case
	}
