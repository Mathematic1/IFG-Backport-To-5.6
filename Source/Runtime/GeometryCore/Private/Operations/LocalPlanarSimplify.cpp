...
void FLocalPlanarSimplify::CollapseDegenerateEdges(FDynamicMesh3& Mesh, TSet<int32>& InOutEdges, bool bRemoveAllDegenerateFromInputSet, double DegenerateEdgeLenTol, TSet<int>* TriangleSelection)
{
	double Tol2 = DegenerateEdgeLenTol * DegenerateEdgeLenTol;
	FVector3d A, B;
	...
	{
		Collapsed = 0;
		for (auto EdgesIter = InOutEdges.CreateIterator(); EdgesIter; ++EdgesIter)
		{
			int32 EID = *EdgesIter;
			if (!Mesh.IsEdge(EID))
			{
