...
namespace UE::MeshNormalsLocals
{
...
bool FMeshNormals::QuickRecomputeOverlayNormals(FDynamicMesh3& Mesh, bool bInvert, bool bWeightByArea, bool bWeightByAngle, bool bParallelCompute)
{
	...
	{
		// for the parallel case we want to compute once per triangle normal, and accumulate results in element normals
		// there is some overhead to using the atomic float buffer, so if not threading it is better to not do this
		...
		ParallelFor(Mesh.MaxTriangleID(), [&Normals, &Mesh, bWeightByArea, bWeightByAngle, NormalOverlay](int32 TID)
		{
			...
			double TriArea;
			TriNormal = VectorUtil::NormalArea(V0, V1, V2, TriArea);
			FVector3f TriNormalWeights = (FVector3f)TMeshQueries<FDynamicMesh3>::GetVertexWeightsOnTriangle(Mesh, TID, TriArea, bWeightByArea, bWeightByAngle);
			FVector3f TriNormalf = (FVector3f)TriNormal;

			for (int32 SubIdx = 0; SubIdx < 3; ++SubIdx)
			{
				int32 ElID = ElTri[SubIdx];
				FVector3f AddNormal = TriNormalf * TriNormalWeights[SubIdx];
				int32 NormalArrayIdx = ElID * 3;
				for (int32 VecIdx = 0; VecIdx < 3; ++VecIdx)
				{
					UE::MeshNormalsLocals::AtomicFloatFetchAdd(Normals[NormalArrayIdx + VecIdx], AddNormal[VecIdx]);
				}
			}
		}, bForceSingleThreaded);

		float Sign = (bInvert) ? -1.0f : 1.0f;
		ParallelFor(NormalOverlay->MaxElementID(), [&Normals, Sign, NormalOverlay](int32 ElID)
		{
			if (NormalOverlay->IsElement(ElID))
			{
				int32 NormalsIdx = ElID * 3;
