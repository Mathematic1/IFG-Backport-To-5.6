...
// @UE BEGIN
#if UE_EXACT_PREDICATE_DOUBLE_PRECISION
// The double precision case -- use dekker two_product for additional precision
...
#else
// The single precision case -- just use doubles for the required extra precision
REAL orient2dadapt_origin(const REAL ax, const REAL ay, const REAL bx, const REAL by)
{
	double detleft = double(ax) * double(by);
	double detright = double(ay) * double(bx);
	return REAL(detleft - detright);
}
#endif
...
REAL orient2d_origin(const REAL ax, const REAL ay, const REAL bx, const REAL by)
{
	...
	// since float rounding happens after multiply, and should preserve order, only ambiguous case is the exact zero
	if (det == 0)
	{
