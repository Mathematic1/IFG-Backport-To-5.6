...
void TConvexHull3<RealType>::GetFaces(TFunctionRef<void(TArray<int32>&, TVector<RealType>)> PolygonFunc, TFunctionRef<TVector<RealType>(int32)> GetPointFunc) const
{
	...
	for (int32 TriIdx = 0; TriIdx < Hull.Num(); ++TriIdx)
	{
		...
		while (ToProcess.Num() > 0)
		{
			...
			// skip the triangle if it's already assigned a group or we've already visited it in this pass
			if (GroupIDs[NbrTriIdx] >= 0 || GroupIDs[NbrTriIdx] < -CurGroupID - 1)
			{
				continue;
			}

			// region grow to the triangle if its vertices are all coplanar w/ the source triangle
			const FIndex3i& NbrTri = Hull[NbrTriIdx];
			bool bFoundOnlyNewCoplanar = false;
			for (int32 SubIdx = 0; SubIdx < 3; ++SubIdx)
			{
				int32 V = NbrTri[SubIdx];
				// only bother testing the vertex if it's new (wasn't on the source triangles)
				if (V != Tri.A && V != Tri.B && V != Tri.C)
				{
					TVector<RealType> OtherPos = GetPointFunc(V);
					RealType O3DVal = ExactPredicates::Orient3<RealType>(TriPosns[0], TriPosns[1], TriPosns[2], OtherPos);
					if (O3DVal == 0)
					{
						// found a coplanar vertex; consider growing to this tri
						bFoundOnlyNewCoplanar = true;
					}
					else
					{
						// found a non-coplanar vertex; stop searching and do not grow to this tri
						bFoundOnlyNewCoplanar = false;
						break;
					}
				}
			}
			if (bFoundOnlyNewCoplanar)
			{
				GroupIDs[NbrTriIdx] = CurGroupID;

				// if we failed to compute a stable normal from the source triangle, compute one now
				if (!bValidNormal)
				{
				...
				// grow to unlabelled neighbors
				const FIndex3i& NbrNbrInds = HullNeighbors[NbrTriIdx];
				for (int32 SubIdx = 0; SubIdx < 3; ++SubIdx)
				{
					int32 NbrNbrTriIdx = NbrNbrInds[SubIdx];
					// consider the triangle for region growing if it's not assigned a group and we haven't visited it yet this pass
