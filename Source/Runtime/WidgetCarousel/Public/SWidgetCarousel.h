...
//			]
	...
	TSharedRef<SWidget> GetWidgetContent() const
	{
		if (FXWidget.IsValid())
		{
			FChildren* Children = FXWidget->GetChildren();
...
public:
	...
	void GenerateCurrentWidgets()
	{
		...
		if (ItemsSourceRef.Num() > 0)
		{
			...
			TSharedRef<SWidget> LeftWidget = OnGenerateWidget.Execute(ItemsSourceRef[GetLeftWidgetIndex(WidgetIndex)]);
			TSharedRef<SWidget> RightWidget = OnGenerateWidget.Execute(ItemsSourceRef[GetRightWidgetIndex(WidgetIndex)]);
			TSharedRef<SWidget> CenterWidget = OnGenerateWidget.Execute(ItemsSourceRef[WidgetIndex]);

			// Each carousel widget must have unique content since Slate Global Invalidation requires that widgets only exist in a single slot
			const bool bIsLeftWidgetUnique = (LeftWidget != CenterWidget) && (RightWidget != LeftWidget);
			const bool bIsRightWidgetUnique = RightWidget != CenterWidget;
			LeftCarouselWidget->SetWidgetContent(bIsLeftWidgetUnique ? LeftWidget : SNullWidget::NullWidget);
			RightCarouselWidget->SetWidgetContent(bIsRightWidgetUnique ? RightWidget : SNullWidget::NullWidget);
			...
			CenterCarouselWidget->SetWidgetContent(CenterWidget);
		}
	...
	{
		constexpr bool bClearTransition = true;
		CenterCarouselWidget->SetSlide(0.f, bClearTransition);
		LeftCarouselWidget->SetSlide(SlideValueLeftLimit, bClearTransition);
		RightCarouselWidget->SetSlide(SlideValueRightLimit, bClearTransition);
	}
...
protected:
	...
	void SwapBuffer(EWidgetCarouselScrollDirection::Type ScrollDirection, int32 OverrideWidgetIndex = INDEX_NONE)
	{
		...
		TSharedRef<SWidget> LeftWidget = LeftCarouselWidget->GetWidgetContent();
		TSharedRef<SWidget> RightWidget = RightCarouselWidget->GetWidgetContent();
		TSharedRef<SWidget> CenterWidget = CenterCarouselWidget->GetWidgetContent();

		const bool bIsScrollingLeft = ScrollDirection == EWidgetCarouselScrollDirection::Carousel_Left;
		...
		if (ItemsSourceRef.Num() > 0)
		{
			if (bIsScrollingLeft)
			{
				...
				RightWidget = (CenterWidget == SNullWidget::NullWidget) ? OnGenerateWidget.Execute(ItemsSourceRef[GetRightWidgetIndex(WidgetIndex)]) : CenterWidget;
				CenterCarouselWidget = LeftCarouselWidget;
				CenterWidget = (LeftWidget == SNullWidget::NullWidget) ? OnGenerateWidget.Execute(ItemsSourceRef[WidgetIndex]) : LeftWidget;
				LeftCarouselWidget = TempWidget;
				LeftWidget = OnGenerateWidget.Execute(ItemsSourceRef[GetLeftWidgetIndex(WidgetIndex)]);
			...
			else
			{
				...
				LeftWidget = (CenterWidget == SNullWidget::NullWidget) ? OnGenerateWidget.Execute(ItemsSourceRef[GetLeftWidgetIndex(WidgetIndex)]) : CenterWidget;
				CenterCarouselWidget = RightCarouselWidget;
				CenterWidget = (RightWidget == SNullWidget::NullWidget) ? OnGenerateWidget.Execute(ItemsSourceRef[WidgetIndex]) : RightWidget;
				RightCarouselWidget = TempWidget;
				RightWidget = OnGenerateWidget.Execute(ItemsSourceRef[GetRightWidgetIndex(WidgetIndex)]);
				RightCarouselWidget->SetSlide(CenterCarouselWidget->GetSlideValue() + SlideValueRightLimit, true);
			}
		}

		if (ItemsSourceRef.IsValidIndex(OverrideWidgetIndex))
		{
			// Need to find a way to clear left and right images
			WidgetIndex = OverrideWidgetIndex;
			CenterWidget = OnGenerateWidget.Execute(ItemsSourceRef[OverrideWidgetIndex]);
		}
		
		// Ensure the 3 CarouselWidgets here have unique widgets, if that isn't the case, prioritize the 2 widgets that will display during the transition
		const bool bIsLeftWidgetUnique = (LeftWidget != CenterWidget) && (!bIsScrollingLeft || RightWidget != LeftWidget);
		const bool bIsRightWidgetUnique = (RightWidget != CenterWidget) && (bIsScrollingLeft || RightWidget != LeftWidget);
		LeftCarouselWidget->SetWidgetContent(bIsLeftWidgetUnique ? LeftWidget : SNullWidget::NullWidget);
		RightCarouselWidget->SetWidgetContent(bIsRightWidgetUnique ? RightWidget : SNullWidget::NullWidget);
		CenterCarouselWidget->SetWidgetContent(CenterWidget);
