...
void FMacApplication::OnDisplayReconfiguration(CGDirectDisplayID Display, CGDisplayChangeSummaryFlags Flags, void* UserInfo)
{
	...
	{
		App->bDisplayReconfiguring = true;
	}
	else if (Flags & kCGDisplayDesktopShapeChangedFlag)
	{
		App->bDisplayReconfiguring = false;
...
void FMacApplication::OnWindowDidMove(TSharedRef<FMacWindow> Window)
{
    ...
    {
		__block NSRect WindowFrame, OpenGLFrame;
		MainThreadCall(^{
			// Make sure to take notch and padding into account
			WindowFrame = [Window->GetWindowHandle() frame];
			OpenGLFrame = [Window->GetWindowHandle() openGLFrame];
		});
