...
void FMacCursor::Lock(const RECT* const Bounds)
{
	...
	{
		bIsPositionInitialised = false; // After exiting HighPrecisionMode, force GetIntPosition() to update its cached position in case the cursor was warped by another app while we were in high precision mode
	}
