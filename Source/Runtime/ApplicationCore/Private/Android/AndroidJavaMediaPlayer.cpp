...
bool ForceAndroidBitmapRendererLegacy = false;
...
void FJavaAndroidMediaPlayer::SelectBitmapRenderer()
{
	...
	{
		if (ForceAndroidBitmapRendererLegacy)
		{
			BRSelection = BitmapRendererSelection::Legacy;
			ForceAndroidBitmapRendererLegacy = false;
		}
		else
		if (UsingVulkanRenderer)
		{
			if (AndroidEnableMediaPlayerBRIR && FAndroidMisc::IsExternalMemoryAndroidHardwareBufferExtensionLoaded())
			{
		...
		{
			if (AndroidEnableMediaPlayerBRIR)
			{
				BRSelection = BitmapRendererSelection::ImageReader;
			}
			else
