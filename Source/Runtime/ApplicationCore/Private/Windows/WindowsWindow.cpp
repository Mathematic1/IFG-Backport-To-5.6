...
TArray<FColor> FWindowsWindow::GetWindowPixels(uint32& OutWidth, uint32& OutHeight)
{
	...
	HBITMAP CaptureBitmap = CreateCompatibleBitmap(WindowDC, WindowWidth, WindowHeight);
	if (!CaptureBitmap)
	{
		ReleaseDC(HWnd, WindowDC);
		DeleteDC(CaptureDC);
	...
	if (SelectObject(CaptureDC, CaptureBitmap))
	{
		// Use the undocumented flag PW_RENDERFULLCONTENT (2) which is available since Windows 8.1,
		// and that allows to capture applications that use hardware-accelerated graphics.
		UINT Flags = PW_CLIENTONLY | 2;
		if (PrintWindow(HWnd, CaptureDC, Flags))
		{
			BITMAPINFO BitmapInfo = {};
			BitmapInfo.bmiHeader.biSize = sizeof(BitmapInfo.bmiHeader);
			BitmapInfo.bmiHeader.biWidth = WindowWidth;
			BitmapInfo.bmiHeader.biHeight = -WindowHeight; // Negative height to indicate we want top-to-bottom scanlines.
			BitmapInfo.bmiHeader.biPlanes = 1;
			BitmapInfo.bmiHeader.biBitCount = 32;
			BitmapInfo.bmiHeader.biCompression = BI_RGB;

			// Get bitmap pixels and convert to RGB.
			TArray<RGBQUAD> BitmapPixels;
			BitmapPixels.SetNumUninitialized(WindowWidth * WindowHeight);
			uint32 ActualHeight = GetDIBits(CaptureDC, CaptureBitmap, 0, WindowHeight, BitmapPixels.GetData(), &BitmapInfo, DIB_RGB_COLORS);
			if (ActualHeight > 0)
			{
				Algo::Transform(BitmapPixels, Pixels, [](const RGBQUAD& Pixel) { return FColor(Pixel.rgbRed, Pixel.rgbGreen, Pixel.rgbBlue); });
