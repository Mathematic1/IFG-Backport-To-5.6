...
void UPhysicalMaterial::PostLoad()
{
	...
	if (GetLinkerCustomVersion(FFortniteMainBranchObjectVersion::GUID) < FFortniteMainBranchObjectVersion::DebugColorForPhysicalMaterials)
	{
		// We don't have a color yet, generate one based on a stable hash generated by the asset's path : 
		DebugColor = FLinearColor::MakeRandomSeededColor(GetTypeHash(GetPathName()));
	}
}
