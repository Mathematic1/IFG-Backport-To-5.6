...
inline bool IsDeviceUMA(ID3D11Device* device)
{
	...
	if (SUCCEEDED(device->QueryInterface(__uuidof(ID3D11Device3), (void**)&pDevice3)))
	{
		...
		{
			pDevice3->Release();
			return (bool)(FeatureData.UnifiedMemoryArchitecture);
		}
		pDevice3->Release();
	}
	return false;
 ...
 */
static bool SafeTestD3D11CreateDevice(IDXGIAdapter* Adapter, D3D_FEATURE_LEVEL MinFeatureLevel,D3D_FEATURE_LEVEL MaxFeatureLevel, FD3D11DeviceBasicInfo& OutInfo)
{
	...
	__try
	{
		...
		if (SUCCEEDED(Result))
		{
			OutInfo.MaxFeatureLevel = DeviceFeatureLevel;
			OutInfo.bUMA = IsDeviceUMA(D3DDevice);
...
void FD3D11DynamicRHIModule::FindAdapter()
{
	...
	for(uint32 AdapterIndex = 0; LocalEnumAdapters(AdapterIndex,TempAdapter.GetInitReference()) != DXGI_ERROR_NOT_FOUND; ++AdapterIndex)
	{
		...
		if(TempAdapter)
		{
			...
			FD3D11DeviceBasicInfo DeviceInfo;
			if(SafeTestD3D11CreateDevice(TempAdapter, MinAllowedFeatureLevel, MaxAllowedFeatureLevel, DeviceInfo))
			{
				...
				UE_LOG(LogD3D11RHI, Log,
					TEXT("      %u/%u/%u MB DedicatedVideo/DedicatedSystem/SharedSystem, Outputs:%d, VendorId:0x%x UMA:%s"),
				...
				FD3D11Adapter CurrentAdapter(TempAdapter, bIsWARP, DeviceInfo);
	...
	GRHIDeviceIsIntegrated = ChosenAdapter.bUMA;
	UE_LOG(LogD3D11RHI, Log, TEXT("Integrated GPU (iGPU): %s"), GRHIDeviceIsIntegrated ? TEXT("true") : TEXT("false"));
}
...
FDynamicRHI* FD3D11DynamicRHIModule::CreateRHI(ERHIFeatureLevel::Type RequestedFeatureLevel)
{
	...
	FGenericCrashContext::SetEngineData(TEXT("RHI.IntegratedGPU"), ChosenAdapter.bUMA ? TEXT("true") : TEXT("false"));
...
#if WITH_DX_PERF
		Direct3DDeviceIMContext->QueryInterface<ID3DUserDefinedAnnotation>(Direct3DDeviceIMAnnotation.GetInitReference());
#endif
