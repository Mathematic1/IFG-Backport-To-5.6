...
FInstallBundleCacheStats FInstallBundleCache::GetStats(EInstallBundleCacheStatsFlags Flags /*= EInstallBundleCacheStatsFlags::None*/, bool bVerbose /*= false*/) const
{
	FInstallBundleCacheStats Stats;
	Stats.CacheName = CacheName;
	Stats.MaxSize = TotalSize;

	const bool bDumpToLog = EnumHasAnyFlags(Flags, EInstallBundleCacheStatsFlags::DumpToLog);
	const bool bDumpToResults = EnumHasAnyFlags(Flags, EInstallBundleCacheStatsFlags::DumpToResults);
	...
	auto AddStats = [&Stats, bDumpToResults](FName BundleName, const FBundleCacheInfo& Info)
	{
		...
		if (bDumpToResults)
		{
			Stats.BundleStats.Emplace(FInstallBundleCacheBundleStats
			{
				.BundleName = BundleName,
				.FullInstallSize = Info.FullInstallSize,
				.InstallOverheadSize = Info.InstallOverheadSize,
				.CurrentInstallSize = Info.CurrentInstallSize,
				.TimeStamp = Info.TimeStamp,
				.AgeScalar = Info.AgeScalar,
				.bReserved = Info.State == ECacheState::Reserved,
			});
		}
	};
...
#define INSTALLBUNDLECACHE_CSV_LOG(Verbosity) \
	...
	if (!bDumpToLog)
	{
		for (const TPair<FName, FBundleCacheInfo>& CachePair : CacheInfo)
		{
			...
			AddStats(CachePair.Key, Info);
		}
	}
	else if (EnumHasAnyFlags(Flags, EInstallBundleCacheStatsFlags::CSVFormat))
	{
		if (bVerbose)
		{
