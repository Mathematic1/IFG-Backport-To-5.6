...
void FRHIMemoryPool::Destroy()
{
	...
	for (FRHIPoolAllocationData* FreeBlock : FreeBlocks)
	{
		ReleaseAllocationData(FreeBlock);
	}
	FreeBlocks.Empty();
	
	for (FRHIPoolAllocationData* AllocationData : AllocationDataPool)
	{
		delete AllocationData;
	}
	AllocationDataPool.Empty();
}
...
bool FRHIPoolAllocator::TryAllocateInternal(uint32 InSizeInBytes, uint32 InAllocationAlignment, ERHIPoolResourceTypes InAllocationResourceType, FRHIPoolAllocationData& AllocationData)
{
	...
	uint32 AlignedSize = FRHIMemoryPool::GetAlignedSize(InSizeInBytes, PoolAlignment, InAllocationAlignment);
	if (NewPoolIndex >= 0)
	{
		Pools[NewPoolIndex] = CreateNewPool(NewPoolIndex, AlignedSize, InAllocationResourceType);
	}
	else
	{
		NewPoolIndex = Pools.Num();		
		Pools.Add(CreateNewPool(NewPoolIndex, AlignedSize, InAllocationResourceType));
