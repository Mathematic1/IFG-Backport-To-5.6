...
static TAutoConsoleVariable<bool> CVarDynamicMeshComponent_AllowCollisionAsyncBuildInEditor(
	TEXT("geometry.DynamicMesh.AllowAsyncCollisionBuildInEditor"),
	false,
	TEXT("If enabled, we allow async build of dynamic mesh component collision in Editor. Otherwise, async collision build will only happen at runtime.")
);
...
void UDynamicMeshComponent::RebuildPhysicsData()
{
	...
	const bool bUseAsyncCook = bUseAsyncCooking
		...
		// We experimentally allow asynchronous builds in editor via a cvar, to support cases where in-editor dynamic meshes
		//  may otherwise hitch during editing.
		&& World && (World->IsGameWorld() || 
			(World->IsEditorWorld() && CVarDynamicMeshComponent_AllowCollisionAsyncBuildInEditor.GetValueOnAnyThread())
			);
