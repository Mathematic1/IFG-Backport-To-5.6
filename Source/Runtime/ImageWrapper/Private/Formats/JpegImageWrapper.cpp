...
void FJpegImageWrapper::Compress(int32 Quality)
{
	...
	if (CompressedData.Num() == 0)
	{
		...
		// jpge::compress_image_to_jpeg_file_in_memory recommends at least 1kb buffer size since small images
		// have some overhead and certain configurations can expand in size (e.g. high quality)
		int64_t OutBufferSize = std::max<int64>(RawData.Num(), 1024);
		
		CompressedData.Reset(OutBufferSize);
		CompressedData.AddUninitialized(OutBufferSize);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FJpegImageWrapper::CompressTurbo);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FJpegImageWrapper::UncompressTurbo);
