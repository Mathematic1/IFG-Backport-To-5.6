...
namespace UE
{
	namespace Interchange
	{
		...
		const FString& FFactoryBaseNodeStaticData::AttributesAppliedThroughDelegatesBaseKey()
		{
			static FString AttributesAppliedThroughDelegates_BaseKey = TEXT("__AttributeAppliedThroughDelegate__");
			return AttributesAppliedThroughDelegates_BaseKey;
		}
...
UInterchangeFactoryBaseNode::UInterchangeFactoryBaseNode()
{
	...
	AttributesAppliedThroughDelegates.Initialize(Attributes, UE::Interchange::FFactoryBaseNodeStaticData::AttributesAppliedThroughDelegatesBaseKey());
...
{
	const static FString FactoryPrefix = TEXT("Factory_");

	if (TranslatedNodeUid.StartsWith(FactoryPrefix))
	{
		return TranslatedNodeUid;
	}

	return FactoryPrefix + TranslatedNodeUid;
}

FString UInterchangeFactoryBaseNode::BuildTranslatedNodeUid(const FString& InFactoryNodeUid)
{
	const static FString FactoryPrefix = TEXT("Factory_");

	FStringView View{InFactoryNodeUid};
	if (View.StartsWith(FactoryPrefix))
	{
		View.RemovePrefix(FactoryPrefix.Len());
		return FString{View};
	}

	return InFactoryNodeUid;
}
...
void UInterchangeFactoryBaseNode::RestoreAllCustomAttributeDelegates()
{
	TArray<FString> AttributeKeys;
	AttributesAppliedThroughDelegates.GetItems(AttributeKeys);
	for (const FString& Key : AttributeKeys)
	{
		AttributesAppliedThroughDelegatesKeySet.Add(Key);
	}

	// The same attributes would be set again when the actual reset happens.
	AttributesAppliedThroughDelegates.RemoveAllItems();
	OnRestoreAllCustomAttributeDelegates();
}
...
void UInterchangeFactoryBaseNode::RemoveAllCustomAttributeDelegates()
{
	ApplyCustomAttributeDelegates.Empty();
	FillCustomAttributeDelegates.Empty();
}
...
void UInterchangeFactoryBaseNode::OnRestoreAllCustomAttributeDelegates()
{
	// Should be filled with appropriate attributes in the child classes.
}
