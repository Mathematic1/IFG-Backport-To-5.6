...
public:
	...
	*/
	...
	SANDBOXFILE_API virtual IAsyncReadFileHandle*	OpenAsyncRead(const TCHAR* Filename, bool bAllowWrite = false) override;
	SANDBOXFILE_API virtual void					SetAsyncMinimumPriority(EAsyncIOPriorityAndFlags Priority) override;

	UE_DEPRECATED(5.6, "Please use overloaded value that returns error values properly")
	SANDBOXFILE_API virtual IMappedFileHandle*		OpenMapped(const TCHAR* Filename) override;
	SANDBOXFILE_API virtual FOpenMappedResult		OpenMappedEx(const TCHAR* Filename, EOpenReadFlags OpenOptions = EOpenReadFlags::None, int64 MaximumSize = 0) override;
