...
public:
	 ...
	 * @param Value the out value to read the data into
	 */
	JSON_API virtual void Serialize(FStringView Name, FUtf8String& Value) override;
	/**
	 * If the underlying json object has the field, it is read into the value
	 *
	 * @param Name the name of the field to read
	...
	*/
	...
	JSON_API virtual void WriteIdentifierPrefix(FStringView Name) override;

	JSON_API virtual void WriteRawJSONValue(FStringView Value) override;

	JSON_API virtual void WriteRawJSONValue(FUtf8StringView Value) override;
private:
