...
FCrashUploadToDataRouter::FCrashUploadToDataRouter(const FString& InDataRouterUrl, const FString& InOptionalDataRouterUnixSocket)
	: DataRouterUrl(InDataRouterUrl)
	, DataRouterUnixSocket(InOptionalDataRouterUnixSocket)
	, Timer(Duration)
{
...
{
	ErrorReport = PlatformErrorReport;
	PendingFiles = FPlatformErrorReport(ErrorReport.GetReportDirectory()).GetOptionalAttachmentsToUpload();
	UE_LOG(CrashReportCoreLog, Log, TEXT("Got %d pending optional attachment files to upload"), PendingFiles.Num());

	PauseState = EUploadState::Finished;
	// We should only upload optional attachments when the base report was successfully uploaded (EUploadState::Finished).
	if (State == EUploadState::Finished)
	{
		SetCurrentState(EUploadState::CompressAndSendData);
		CompressAndSendData();
	}
...
void FCrashUploadToDataRouter::CompressAndSendData()
{
	...
	if (!DataRouterUnixSocket.IsEmpty())
	{
		Request->SetOption(HttpRequestOptions::UnixSocketPath, DataRouterUnixSocket);
	}
#endif
	Request->SetContent(CompressedData.Data);
	UE_LOG(CrashReportCoreLog, Log, TEXT("Sending HTTP request: %s, Payload size: %d"), *Request->GetURL(), CompressedData.Data.Num());
