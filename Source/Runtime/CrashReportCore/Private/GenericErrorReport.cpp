...
FGenericErrorReport::FGenericErrorReport(const FString& Directory)
	...
	, OptionalAttachmentsDirectory(FPaths::Combine(Directory, FGenericCrashContext::CrashReportOptionalAttachmentsDirName))
	, bValidCallstack(true)
{
	...
	auto OptionalFilenamesVisitor = MakeDirectoryVisitor([this](const TCHAR* FilenameOrDirectory, bool bIsDirectory) {
		if (!bIsDirectory)
		{
			OptionalAttachmentsFilenames.Push(FPaths::GetCleanFilename(FilenameOrDirectory));
		}
		return true;
	});

	FPlatformFileManager::Get().GetPlatformFile().IterateDirectory(*ReportDirectory, FilenamesVisitor);
	FPlatformFileManager::Get().GetPlatformFile().IterateDirectory(*OptionalAttachmentsDirectory, OptionalFilenamesVisitor);
}
...
void FGenericErrorReport::SetPrimaryCrashProperties( FPrimaryCrashProperties& out_PrimaryCrashProperties )
{
	...
	if (Helper && bValidCallstack)
	{
		...
		out_PrimaryCrashProperties.Threads = Helper->CrashInfo.Threads;
...
TArray<FString> FGenericErrorReport::GetOptionalAttachmentsToUpload() const
{
	...
	for (const auto& Filename : OptionalAttachmentsFilenames)
	{
		FilesToUpload.Push(OptionalAttachmentsDirectory / Filename);
	}
	return FilesToUpload;
}
