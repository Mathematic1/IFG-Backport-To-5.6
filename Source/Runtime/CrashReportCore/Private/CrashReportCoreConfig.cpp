	...
	, SectionName( TEXT( "CrashReportClient" ) )
{
	...
	if (!GConfig->GetBool(*SectionName, TEXT("bHideAllowToBeContactedOption"), bHideAllowToBeContactedOption, GEngineIni))
	{
	...
	if (!GConfig->GetBool(*SectionName, TEXT("bIsAllowedToSendWithoutDetailedInfo"), bIsAllowedToSendWithoutDetailedInfo, GEngineIni))
	{
...
void FCrashReportCoreConfig::SetProjectConfigOverrides(const FConfigFile& InConfigFile)
{
	...
	InConfigFile.GetBool(*SectionName, TEXT("bHideAllowToBeContactedOption"), bHideAllowToBeContactedOption);
	...
	InConfigFile.GetBool(*SectionName, TEXT("bIsAllowedToSendWithoutDetailedInfo"), bIsAllowedToSendWithoutDetailedInfo);
	...
	bool bUnixSocketEnabled = false;
	InConfigFile.GetBool(*SectionName, TEXT("bUnixSocketEnabled"), bUnixSocketEnabled); 
	if (bUnixSocketEnabled)
	{
		FString UnixSocketPath;
		InConfigFile.GetString(*SectionName, TEXT("DataRouterUnixSocketsPath"), UnixSocketPath);
		FString UnixSocketName;
		InConfigFile.GetString(*SectionName, TEXT("DataRouterUnixSocket"), UnixSocketName);
		DataRouterUnixSocket = UnixSocketPath / UnixSocketName;
	}
}
