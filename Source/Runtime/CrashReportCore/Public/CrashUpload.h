...
public:
	...
	{
		return State == EUploadState::Finished;
	}
...
public:
	 ...
	 * @param ServerAddress Host IP of the crash report server
	 */
	explicit FCrashUploadToDataRouter(const FString& InDataRouterUrl, const FString& InOptionalDataRouterUnixSocket);
	...
	/**
	 * Compresses all optional attachments files and sends one compressed file.
	 * Requires the base report to have been already sent (via BeginUpload).
	 */
...
private:
	...
	/** Local Unix Socket for data router */
