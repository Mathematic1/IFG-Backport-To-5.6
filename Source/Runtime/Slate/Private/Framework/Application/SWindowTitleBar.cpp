...
FReply SWindowTitleBar::MaximizeRestoreButton_OnClicked( )
{
	...
	if (OwnerWindow.IsValid())
	{
		...
		{
			if (NativeWindow->GetWindowMode() == EWindowMode::WindowedFullscreen)
			{
				OwnerWindow->SetWindowMode(EWindowMode::Windowed);
			}
...
const FSlateBrush* SWindowTitleBar::GetMaximizeRestoreImage( ) const
{
	...
	if (NativeWindow.IsValid())
	{
		if (NativeWindow->GetWindowMode() == EWindowMode::WindowedFullscreen)
		{
			if (MaximizeRestoreButton->IsPressed())
			{
				return &Style->ExitFullscreenButtonStyle.Pressed;
			}
			if (MaximizeRestoreButton->IsHovered())
			{
				return &Style->ExitFullscreenButtonStyle.Hovered;
			}
			return &Style->ExitFullscreenButtonStyle.Normal;
		}
...
FText SWindowTitleBar::GetMaximizeButtonToolTip() const
{
	...
	if (OwnerWindow.IsValid())
	{
		...
		if (NativeWindow.IsValid())
		{
			...
			if (NativeWindow->GetWindowMode() == EWindowMode::WindowedFullscreen)
			{
				return NSLOCTEXT("WindowTitleBar", "MaximizeExitWindowedFullscreen", "Exit Full Screen");
			}
		}
	}
