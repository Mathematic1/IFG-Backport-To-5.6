...
void FPersistentNetDebugNames::ResetNameIds()
{
	FPersistentNetDebugNames& Singleton = Get();
	{
		UE::TScopeLock Lock(Singleton.CriticalSection);

		for (const auto& Iter : Singleton.PersistentNames)
		{
			Iter.Value->DebugNameId = 0;
		}
	}
}
...
{
	FPersistentNetDebugNames::Get().ResetNameIds();
}
