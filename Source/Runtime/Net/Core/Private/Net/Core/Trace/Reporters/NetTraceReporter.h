...
struct FNetTraceReporter
{
	...
	static void ReportObjectExists(uint32 GameInstanceId, uint64 NetObjectId, UE::Net::FNetDebugNameId NameId, uint64 TypeIdentifier, uint32 OwnerId);
	...
	static void ReportObjectPolled(uint32 GameInstanceId, uint64 NetObjectId, uint64 Cycles, bool bIsWaste);
	static void ReportObjectQuantized(uint32 GameInstanceId, uint64 NetObjectId, uint64 Cycles);
	static void ReportObjectWritten(uint32 GameInstanceId, uint64 NetObjectId, uint64 Cycles);
	static void ReportSubObject(uint32 GameInstanceId, uint64 ParentObjectId, uint64 SubObjectId);
};
