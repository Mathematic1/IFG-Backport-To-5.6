...
struct FTraceHandle
{
	...
	FTraceHandle(uint32 InFrameNumber, uint32 InIndex, bool bInTransactional = false)
	{
		...
		_Data.Index = InIndex;
		bTransactional = bInTransactional;
	}
...
struct FTransactionalAsyncTraceBuffer
{
...
struct AsyncTraceData : FNoncopyable
{
	...
	/**
	 * Data for Transactional async queries. This is effectively the same as above,
	 * however it must be stored separately as we cannot run any async queries until a transaction is closed.
	 */
