...
public:
	...
	ENGINE_API void SetInitialRuntimeState(EDataLayerRuntimeState InInitialRuntimeState);
	...
	EOverrideBlockOnSlowStreaming GetOverrideBlockOnSlowStreaming() const { return OverrideBlockOnSlowStreaming; }
	int32 GetStreamingPriority() const { return StreamingPriority; }
	...
	virtual bool IsLocked() const { FText Reason; return IsLocked(&Reason); }
...
protected:
	...
	/** Determines the streaming priority (lower value means higher priority), relative to target grid cells. Priority 0 means the same priority as target grid cells. */
	UPROPERTY(Category = "Runtime|Advanced", EditAnywhere)
