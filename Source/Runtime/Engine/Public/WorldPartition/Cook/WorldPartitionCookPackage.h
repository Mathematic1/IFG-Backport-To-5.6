...
using FWorldPartitionPackageHash = FBlake3Hash;
using FWorldPartitionPackageHashBuilder = FBlake3;
...
struct FWorldPartitionCookPackage
{
	...
	FWorldPartitionCookPackage(const FString& InRoot, const FString& InRelativePath, EType InType, const FWorldPartitionPackageHash& InGenerationHash)
		...
		Type(InType),
		GenerationHash(InGenerationHash)
	{
	}
	...
	const EType Type;
	const FWorldPartitionPackageHash GenerationHash;
#endif
