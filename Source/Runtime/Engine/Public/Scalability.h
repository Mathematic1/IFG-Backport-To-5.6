...
{ 
...
#if WITH_EDITOR
	ENGINE_API void ApplyCachedQualityLevels();
	/** Gets the default quality level from the Device Profile or system defaults */
	ENGINE_API FQualityLevels GetDefaultQualityLevels();
	/** Resets the Quality Levels to the Device Profile or system defaults */
	ENGINE_API void ResetQualityLevelsToDefault();
#endif
	...
	*/
	 ...
	 */
	ENGINE_API void LoadState(const FString& IniName, const TCHAR* Section);
	...
	/** 
	 * @param IniName e.g. GEditorPerProjectIni or GGameUserSettingsIni 
	 * @param Section the section to save into
	 */
	ENGINE_API void SaveState(const FString& IniName, const TCHAR* Section);
	...
	inline constexpr float MinResolutionScale = 0.0f;

	/** Maximum single axis scale for render resolution */
	inline constexpr float MaxResolutionScale = 100.0f;
