    ...
    virtual void InitRHI(FRHICommandListBase& RHICmdList) override
    {
		const static FLazyName DuplicatedVerticesBuffeName(TEXT("FDuplicatedVerticesBuffer"));
        {
			...
			const FRHIBufferCreateDesc Desc =
				FRHIBufferCreateDesc::CreateVertex(TEXT("DuplicatedVerticesIndexBuffer"), ResourceArray->GetResourceDataSize())
				.AddUsage(BUF_Static | BUF_ShaderResource)
				.SetClassName(DuplicatedVerticesBuffeName)
				.SetOwnerName(GetOwnerName())
				.SetInitActionResourceArray(ResourceArray)
				.DetermineInitialState();

			DuplicatedVerticesIndexBuffer.VertexBufferRHI = RHICmdList.CreateBuffer(Desc);
            DuplicatedVerticesIndexBuffer.VertexBufferSRV = RHICmdList.CreateShaderResourceView(
				DuplicatedVerticesIndexBuffer.VertexBufferRHI,
...

        {
			...
			const FRHIBufferCreateDesc Desc =
				FRHIBufferCreateDesc::CreateVertex(TEXT("LengthAndIndexDuplicatedVerticesIndexBuffer"), ResourceArray->GetResourceDataSize())
				.AddUsage(BUF_Static | BUF_ShaderResource)
				.SetClassName(DuplicatedVerticesBuffeName)
				.SetOwnerName(GetOwnerName())
				.SetInitActionResourceArray(ResourceArray)
            ...
            LengthAndIndexDuplicatedVerticesIndexBuffer.VertexBufferRHI = RHICmdList.CreateBuffer(Desc);
            LengthAndIndexDuplicatedVerticesIndexBuffer.VertexBufferSRV = RHICmdList.CreateShaderResourceView(
				LengthAndIndexDuplicatedVerticesIndexBuffer.VertexBufferRHI,
				FRHIViewDesc::CreateBufferSRV()
					.SetType(FRHIViewDesc::EBufferType::Typed)
					.SetFormat(PF_R32_UINT));
        }
