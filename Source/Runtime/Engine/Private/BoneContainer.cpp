...
void FBoneContainer::InitializeTo(const TArrayView<const FBoneIndexType>& InRequiredBoneIndexArray, const FCurveEvaluationOption& CurveEvalOption, const UObject& InAsset)
{
	const UE::Anim::FCurveFilterSettings CurveFilterSettings(CurveEvalOption.bAllowCurveEvaluation ? UE::Anim::ECurveFilterMode::DisallowFiltered : UE::Anim::ECurveFilterMode::DisallowAll, CurveEvalOption.DisallowedList, CurveEvalOption.LODIndex);
	InitializeTo(InRequiredBoneIndexArray, CurveFilterSettings, InAsset);
}

void FBoneContainer::InitializeTo(const TArrayView<const FBoneIndexType>& InRequiredBoneIndexArray, const UE::Anim::FCurveFilterSettings& CurveFilterSettings, const UObject& InAsset)
{
	LLM_SCOPE_BYNAME(TEXT("Animation/BoneContainer"));
	BoneIndicesArray = InRequiredBoneIndexArray;
	Asset = const_cast<UObject*>(&InAsset);
	...
	const TArray<FTransform>& InRetargetTransforms) const
{
	...
	if (!RetargetSourceCachedData)
	{
		...
		for (int32 CompactBoneIndex = 0; CompactBoneIndex < CompactPoseNumBones; CompactBoneIndex++)
		{
			...
			const bool bUseSourceRetargetModes = AssetSkeleton.GetEvenIfUnreachable()->GetUseRetargetModesFromCompatibleSkeleton();
			const EBoneTranslationRetargetingMode::Type RetargetMode = FAnimationRuntime::GetBoneTranslationRetargetingMode(
				...
				SourceSkeleton,
				AssetSkeleton.GetEvenIfUnreachable(),
				bDisableRetargeting);
