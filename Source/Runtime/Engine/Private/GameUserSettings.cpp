...
static TAutoConsoleVariable<int32> CVar_GameUserSettings_WindowPositionsMaxSize(
	TEXT("GameUserSettings.WindowPositionsMaxSize"),
	32,
	TEXT("Maximum number of saved window positions for the -SaveWinPos command-line option."));
		...
		if (!bAttemptedSettingsMigration)
		{
			...
			if (LowIntegrityPath.StartsWith(FWindowsPlatformProcess::UserSettingsDir())) // Confirm that assumptions are valid.
			{
				...
				if (FPaths::FileExists(MediumIntegrityPath))
				{
					...
					{
						UE_LOG(LogGameUserSettings, Log, TEXT("Migrating config file to low integrity. %s to %s"), *MediumIntegrityPath, *LowIntegrityPath);
						IFileManager::Get().Copy(*LowIntegrityPath, *MediumIntegrityPath, true);
					}
					else
					{
						UE_LOG(LogGameUserSettings, Log, TEXT("Skipping config file migration to low integrity. %s is newer than %s"), *LowIntegrityPath, *MediumIntegrityPath);
					}
				}
			}
		}
...
void UGameUserSettings::SetWindowPosition(int32 WinX, int32 WinY, int32 SaveSlotIndex)
{
	if (SaveSlotIndex >= 0 && SaveSlotIndex < CVar_GameUserSettings_WindowPositionsMaxSize.GetValueOnAnyThread())
	{
		const int32 NumToAdd = FMath::Max(0, SaveSlotIndex - WindowPositions.Num() + 1);
		for (int32 i = 0; i < NumToAdd; ++i)
		{
			WindowPositions.Add(GetDefaultWindowPosition());
		}
		WindowPositions[SaveSlotIndex].X = WinX;
		WindowPositions[SaveSlotIndex].Y = WinY;
	}
	else
	{
		UE_LOG(LogGameUserSettings, Warning, TEXT("UGameUserSettings::SetWindowPosition attempted to save window position in invalid slot %d. Maximum slot index is %d."),
			SaveSlotIndex, CVar_GameUserSettings_WindowPositionsMaxSize.GetValueOnAnyThread() - 1);
	}
}

void UGameUserSettings::SetWindowPosition(int32 WinX, int32 WinY)
{
	SetWindowPosition(WinX, WinY, 0);
}

FIntPoint UGameUserSettings::GetWindowPosition(int32 SaveSlotIndex)
{
	if (WindowPositions.IsValidIndex(SaveSlotIndex))
	{
		return FIntPoint(WindowPositions[SaveSlotIndex].X, WindowPositions[SaveSlotIndex].Y);
	}
...
void UGameUserSettings::ReloadWindowPositions()
{
	FProperty* WindowPositionsProperty = GetClass()->FindPropertyByName(GET_MEMBER_NAME_CHECKED(UGameUserSettings, WindowPositions));
	if (WindowPositionsProperty != nullptr)
	{
		LoadConfigIni(false);
		ReloadConfig(GetClass(), *GGameUserSettingsIni, UE::LCPF_None, WindowPositionsProperty);
	}
}

int32 UGameUserSettings::GetMaxNumWindowPositionSaveSlots() const
{
	return CVar_GameUserSettings_WindowPositionsMaxSize.GetValueOnAnyThread();
}
...
void UGameUserSettings::EnableHDRDisplayOutputInternal(bool bEnable, int32 DisplayNits, bool bFromUserSettings)
{
	...
	if (CVarHDROutputEnabled)
	{
		...
		{
			UE_LOG(LogGameUserSettings, Display, TEXT("Tried to enable HDR display output but unsupported or disallowed, forcing off."));
			bEnable = false;
		}
