...
{
	static const FName SkeletalMeshAttributeVertexBufferName(TEXT("FSkeletalMeshVertexAttributeRenderData.FSkeletalMeshAttributeVertexBuffer"));
...
void FSkeletalMeshVertexAttributeRenderData::Serialize(FArchive& Ar)
{
	if (Ar.IsLoading())
	{
		...
		for (int32 Index = 0; Index < Count && !Ar.IsError(); Index++)
		{
			...
			Buffer->SetResourceName(SkeletalMeshResourceNames::SkeletalMeshAttributeVertexBufferName);
		}
	}
	else
	{
