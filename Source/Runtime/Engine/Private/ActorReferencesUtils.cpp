...
public:
		...
		, bRecursive(bInRecursive)
	{
		...
		SubObjectsState.Add(Root, EReferenceState::Runtime);
	...
	virtual FArchive& operator<<(UObject*& Obj) override
	{
		...
		{
			EReferenceState& ExistingReferenceState = SubObjectsState.FindOrAdd(Obj, EReferenceState::Unset);
			const EReferenceState ReferenceState = EditorOnlyStack > 0 ? EReferenceState::EditorOnly : EReferenceState::Runtime;

			if (ExistingReferenceState < ReferenceState)
			{
				ExistingReferenceState = ReferenceState;
...
private:
	...
	TMap<UObject*, EReferenceState> SubObjectsState;
