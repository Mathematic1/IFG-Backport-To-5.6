...
UTextureLODSettings* UDeviceProfile::GetTextureLODSettings() const
{
	...
	UTextureLODSettings* PreviewTextureGroup = GetPreviewTextureGroupRedirect();
	if (PreviewTextureGroup != nullptr)
	{
		return PreviewTextureGroup;
	}
#endif
...
void UDeviceProfile::ForEachConsolidatedCVar(const TFunctionRef<bool(const FString&, const FString&)>& FuncRef) const
{
	FScopeLock Lock(&CVarsLock);
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	const TMap<FString, FString>& ConsolidatedCvars = GetConsolidatedCVars();
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	for (const TPair<FString, FString>& Pair : ConsolidatedCvars)
	{
		if (!FuncRef(Pair.Key, Pair.Value))
		{
			return;
		}
	}	
