...
TMap<FString, FString> UDeviceProfileManager::PreviewDeviceProfileScalabilityCVars;
...
static int32 CVarSetMode()
{
	static int32 Mode = -1;
	if (Mode == -1)
	{
		Mode = 0;
		// set the CVarSetMode from commandline
		FParse::Value(FCommandLine::Get(), TEXT("DPApplyMode="), Mode);
	}
	return Mode;
}
...
UDeviceProfileManager& UDeviceProfileManager::Get(bool bFromPostCDOContruct)
{
	if (DeviceProfileManagerSingleton == nullptr)
	{
		...
		UE::DynamicConfig::UpdateCVarsAndDeviceProfiles.AddStatic(UpdateCVarsAfterDynamicConfig);
	}
	return *DeviceProfileManagerSingleton;
}

TArray<TTuple<FName, FString>> UDeviceProfileManager::GetDPChain(const FString& DeviceProfileName, FConfigCacheIni* ConfigSystem)
{
	static FString DPSectionSuffix = *FString::Printf(TEXT(" %s"), *UDeviceProfile::StaticClass()->GetName());
	static FString FragmentSectionSuffix = *FString::Printf(TEXT(" %s"), *UDeviceProfileFragment::StaticClass()->GetName());

	// @todo fragments, before and after
	...
	{
		FString CurrentSectionName = BaseDeviceProfileName + DPSectionSuffix;

		ParentChain.Add(BaseDeviceProfileName);
		...
		if (CurrentSection != nullptr)
		{
			...
			ConfigSystem->GetString(*CurrentSectionName, TEXT("BaseProfileName"), NextBaseDeviceProfileName, GDeviceProfilesIni);
	...
	TArray<TTuple<FName, FString>> DPChain;

	// now walk up the chain, and insert fragments for each level
	for (int Index = ParentChain.Num() - 1; Index >= 0; Index--)
	{
		const FString& DPName = ParentChain[Index];
		FString CurrentSectionName = DPName + DPSectionSuffix;

		// includes of this DP
		TArray<FString> PreFragmentIncludes, PostFragmentIncludes;
		ConfigSystem->GetArray(*CurrentSectionName, TEXT("FragmentIncludes"), PreFragmentIncludes, GDeviceProfilesIni);
		ConfigSystem->GetArray(*CurrentSectionName, TEXT("OverrideFragmentIncludes"), PostFragmentIncludes, GDeviceProfilesIni);

		for (const FString& PreFragmentName : PreFragmentIncludes)
		{
			DPChain.Emplace(*PreFragmentName, PreFragmentName + FragmentSectionSuffix);
		}
		DPChain.Emplace(*DPName, CurrentSectionName);
		for (const FString& PostFragmentName : PostFragmentIncludes)
		{
			DPChain.Emplace(*PostFragmentName, PostFragmentName + FragmentSectionSuffix);
		}
	}

	// get fragments then append in reverse order
	TArray<FSelectedFragmentProperties> DynamicFragments = FindMatchingFragments(DeviceProfileName, ConfigSystem);
	for (int Index = DynamicFragments.Num() - 1; Index >= 0; Index--)
	{
		// @todo check benabled?
		DPChain.Emplace(*DynamicFragments[Index].Fragment, DynamicFragments[Index].Fragment + FragmentSectionSuffix);
	}
...
void UpdateCVarsAfterDynamicConfig(const FConfigModificationTracker* ChangeTracker)
{
	...
	for (const auto& SectionPair : ChangeTracker->CVars)
	{
		...
		for (const auto& TagPair : CVars.CVarEntriesPerBranchPerTag)
		{
			for (const auto& BranchPair : TagPair.Value)
			{
				...
				{
					FString KeyString = CVarPair.Key.ToString();

					if (KeyString.StartsWith(TEXT("sg.")))
					{
						UE::ConfigUtilities::ApplyCVarSettingsFromIni(*FString::Printf(TEXT("%s@%s"), *KeyString.Mid(3), *CVarPair.Value.GetValue()), *GScalabilityIni, Priority, false, TagPair.Key);
					}
					else
					{
						UE::ConfigUtilities::OnSetCVarFromIniEntry(TEXT("DynamicLayer"), *CVarPair.Key.ToString(), *CVarPair.Value.GetValue(), Priority, false, false, TagPair.Key);
					}
				}
			}
		}
	...
	const TSet<FString>* DeviceProfileSections = ChangeTracker->ModifiedSectionsPerBranch.Find(*GDeviceProfilesIni);
	if (DeviceProfileSections != nullptr)
	{
		if (CVarSetMode() > 0)
		{
			UDeviceProfileManager& DPManager = UDeviceProfileManager::Get();
			IConsoleManager& ConsoleManager = IConsoleManager::Get();

			// get the set of sections the active DP cares about
			TArray<TTuple<FName, FString>> DPChain = UDeviceProfileManager::Get().GetDPChain(DPManager.GetActiveDeviceProfileName(), GConfig);

			for (const auto& DP : DPChain)
			{
				// if we updated this DP, then refresh it
				const FName& DPName = DP.Get<0>();
				const FString& DPSectionName = DP.Get<1>();
				if (DeviceProfileSections->Contains(DPSectionName))
				{
					// mode 1 we wipe everything then start over instead of updating in place
					if (CVarSetMode() == 1)
					{
						ConsoleManager.UnsetAllConsoleVariablesWithTag(DP.Get<0>());
					}

					const bool bIsFragment = DPSectionName.EndsWith(UDeviceProfileFragment::StaticClass()->GetName());
					const UDeviceProfileManager::EDeviceProfileMode DeviceProfileMode =
						bIsFragment ? UDeviceProfileManager::EDeviceProfileMode::DPM_SetCVarsForSingleFragment : UDeviceProfileManager::EDeviceProfileMode::DPM_SetCVarsForSingleDP;

					// get just this DP's cvars, and then update in place
					TMap<FName, FString> DeviceProfileCVars = DPManager.GatherDeviceProfileCVars(DPName.ToString(), DeviceProfileMode);
					ConsoleManager.BatchUpdateTag(DPName, DeviceProfileCVars);
				}
			}
		}
		else
		{
			...
			for (const FString& Section : *DeviceProfileSections)
			{
	...
	// if we modify a scalability group section (ie EffectsQuality@2) that matches current value, re-set the group to apply the modified group
	const TSet<FString>* ScalabilitySections = ChangeTracker->ModifiedSectionsPerBranch.Find(*GScalabilityIni);
	if (ScalabilitySections != nullptr)
	{
		for (const FString& SectionName : *ScalabilitySections)
		{
			FString Group, Level;
			if (SectionName.Split(TEXT("@"), &Group, &Level))
			{
				FString SGName = FString::Printf(TEXT("sg.%s"), *Group);
				IConsoleVariable* SGVar = IConsoleManager::Get().FindConsoleVariable(*SGName);
				if (SGVar != nullptr)
				{
					if (SGVar->GetString() == Level)
					{
						SGVar->SetWithCurrentPriority(*Level);
					}
				}
			}
		}
...
// Read the requested fragment from within the +FragmentIncludes= array of a DP. When bForAppending is set to true, we'll use OverrideFragmentIncludes instead.
static void GetCVarsFromDPFragmentIncludes(const FString& CurrentSectionName, const FString& CVarArrayName, TArray<FString>& FragmentCVarsINOUT, FConfigCacheIni* ConfigSystem, bool bForAppending)
{
	const FString FragmentIncludes = bForAppending ? TEXT("OverrideFragmentIncludes") : TEXT("FragmentIncludes");
...
TMap<FName, FString> UDeviceProfileManager::GatherDeviceProfileCVars(const FString& DeviceProfileName, EDeviceProfileMode GatherMode)
{
	...
	bool bSingleSectionMode = GatherMode == EDeviceProfileMode::DPM_SetCVarsForSingleDP || GatherMode == EDeviceProfileMode::DPM_SetCVarsForSingleFragment;
	...
	// no fragments when doing single at a time
	else if (!bSingleSectionMode)
	{
	...
	if (!bSingleSectionMode)
	{
		...
		for (int i = FragmentsSelected.Num() - 1; i >= 0; i--)
		{
			...
			if (SelectedFragment.bEnabled)
			{
				...
				for (int j = FragmentCVars.Num() - 1; j >= 0; j--)
				{
					const FString& FragCVar = FragmentCVars[j];
	...
	const TCHAR* SectionSuffix = GatherMode == EDeviceProfileMode::DPM_SetCVarsForSingleFragment ? TEXT(" DeviceProfileFragment") : TEXT(" DeviceProfile");
	...
	while (bReachedEndOfTree == false)
	{
		...
		if (CurrentSection != nullptr)
		{
			...
			for (int Pass = 0; Pass < 2; Pass++)
			{
				...
				TArray< FString > CurrentProfilesCVars, FragmentCVars, OverrideFragmentCVars;
				if (!bSingleSectionMode)
				{
					GetCVarsFromDPFragmentIncludes(*CurrentSectionName, *ArrayName, FragmentCVars, ConfigSystem, false);
					GetCVarsFromDPFragmentIncludes(*CurrentSectionName, *ArrayName, OverrideFragmentCVars, ConfigSystem, true);
				}
				...
				CurrentProfilesCVars += OverrideFragmentCVars;
				...
				// Iterate over the profile and make sure we do not have duplicate CVars
				{
					...
					for (TArray< FString >::TConstIterator CVarIt(CurrentProfilesCVars); CVarIt; ++CVarIt)
					{
						...
						{
							UE_LOG(LogDeviceProfileManager, VeryVerbose, TEXT("ValidCVars: %s=%s"), *CVarKey, *CVarValue);
					...
					{
						UE_LOG(LogDeviceProfileManager, VeryVerbose, TEXT("CurrentProfilesCVars: %s=%s"), *ProcessedCVarIt.Key(), *ProcessedCVarIt.Value());
				...
				for (TArray< FString >::TConstIterator CVarIt(CurrentProfilesCVars); CVarIt; ++CVarIt)
				{
					...
					if ((*CVarIt).Split(TEXT("="), &CVarKey, &CVarValue))
					{
						...
						{
							UE_LOG(LogDeviceProfileManager, VeryVerbose, TEXT("DeviceProfileCVars: %s=%s"), *CVarKey, *CVarValue);
			...
			// Get the next device profile name, to look for CVars in, along the tree
			FString NextBaseDeviceProfileName;
			if (!bSingleSectionMode && ConfigSystem->GetString(*CurrentSectionName, TEXT("BaseProfileName"), NextBaseDeviceProfileName, GDeviceProfilesIni))
			{
				BaseDeviceProfileName = NextBaseDeviceProfileName;
		...
		// Check if we have inevitably reached the end of the device profile tree.
		bReachedEndOfTree = bSingleSectionMode || CurrentSection == nullptr || BaseDeviceProfileName.IsEmpty();
	}
...
void UDeviceProfileManager::SetDeviceProfileCVars(const FString& DeviceProfileName)
{
	...
	{
		FConfigCacheIni* ConfigSystem = GConfig;
		TArray<TTuple<FName, FString>> DPChain = UDeviceProfileManager::Get().GetDPChain(DeviceProfileName, ConfigSystem);

		// now that we have the ordered chain, walk it and set cvars
		for (const TTuple<FName, FString>& DP : DPChain)
		{
			FName DPName = DP.Get<0>();
			const FString& SectionName = DP.Get<1>();
			EDeviceProfileMode GatherMode = EDeviceProfileMode::DPM_SetCVarsForSingleDP;
			if (SectionName.EndsWith(UDeviceProfileFragment::StaticClass()->GetName()))
			{
				GatherMode = EDeviceProfileMode::DPM_SetCVarsForSingleFragment;
			}

			// get the cvars for the DP or the Fragment
			TMap<FName, FString> DeviceProfileCVars = GatherDeviceProfileCVars(DPName.ToString(), GatherMode);

			for (const TPair<FName, FString>& Pair : DeviceProfileCVars)
			{
				const FString CVarKey = Pair.Key.ToString();
				...
				UE::ConfigUtilities::OnSetCVarFromIniEntry(*GDeviceProfilesIni, *CVarKey, *CVarValue, CVarPriority, false, true, DPName);
			}
		}
	}
	else
...
static void RestorePushedState(TMap<FString, FPushedCVarSetting>& PushedSettings)
{
	...
	{
		IConsoleManager& ConsoleManager = IConsoleManager::Get();
		TArray<TTuple<FName, FString>> DPChain = UDeviceProfileManager::Get().GetDPChain(UDeviceProfileManager::Get().GetActiveDeviceProfileName(), GConfig);
		for (const auto& DP : DPChain)
		{
			ConsoleManager.UnsetAllConsoleVariablesWithTag(DP.Get<0>());
		}
...
void UDeviceProfileManager::ReapplyDeviceProfile(bool bForceReload, TSet<FString>* ModifiedCVars)
{	
	...
	PreReapplyDeviceProfilesDelegate.Broadcast();

	if (ModifiedCVars != nullptr)
	{
		// make sure any modified cvar is in the pushed state because we are going to set cvars without Popping first
		if (OverrideProfile)
		{
			for (const FString& CVarName : *ModifiedCVars)
			{
				if (!PushedSettings.Contains(CVarName))
				{
					IConsoleVariable* CVar = IConsoleManager::Get().FindConsoleVariable(*CVarName);
					if (CVar)
					{
						FPushedCVarSetting OldSetting = FPushedCVarSetting(CVar->GetString(), CVar->GetFlags());
						PushedSettings.Add(CVarName, OldSetting);
					}
				}
			}
			SetDeviceProfileCVars(DeviceProfileManagerSingleton->GetActiveProfile()->GetName());
		}
	}
	...
	PostReapplyDeviceProfilesDelegate.Broadcast();
}
...
FWillReapplyDeviceProfiles& UDeviceProfileManager::WillReapplyDeviceProfiles()
{
	return PreReapplyDeviceProfilesDelegate;
}

FOnReapplyDeviceProfiles& UDeviceProfileManager::OnReapplyDeviceProfiles()
{
	return PostReapplyDeviceProfilesDelegate;
}
	...
	if( !HasAnyFlags( RF_ClassDefaultObject ) )
	{
		...
		if (!FPlatformProperties::RequiresCookedData())
		{
			...
			for (int32 PlatformIndex = 0; PlatformIndex < TargetPlatforms.Num(); ++PlatformIndex)
			{
				...
				UTextureLODSettings* TextureLODSettingsObj = FindProfile(Platform->CookingDeviceProfileName(), false);
				checkf(TextureLODSettingsObj, TEXT("No TextureLODSettings found for %s"), *Platform->CookingDeviceProfileName());

#if ALLOW_OTHER_PLATFORM_CONFIG
				// disable redirect if we are using this for cooking.
				TextureLODSettingsObj->SetPreviewTextureGroupRedirect(nullptr);
#endif
...
void UDeviceProfileManager::SetPreviewDeviceProfile(UDeviceProfile* DeviceProfile, FName PreviewModeTag)
{
	...
	ActiveDeviceProfile->SetPreviewTextureGroupRedirect(PreviewDeviceProfile);
	...
	// Gathering the cvars the device profile inherits and contains
	TMap<FName, FString> DeviceProfileCVars = GatherDeviceProfileCVars(DeviceProfile->GetName(), EDeviceProfileMode::DPM_CacheValues);
	for (TPair<FName, FString>& Pair : DeviceProfileCVars)
	{
		FString CVarKey = Pair.Key.ToString();
		const FString& CVarValue = Pair.Value;
		// Cache any scalability related cvars so that the active profile scalability group can be overriden
		const bool bIsScalabilityBucket = CVarKey.StartsWith(TEXT("sg."));
		if (bIsScalabilityBucket && CVarAllowScalabilityGroupsToChangeAtRuntime.GetValueOnGameThread() > 0)
		{
			PreviewDeviceProfileScalabilityCVars.Add(*CVarKey, *CVarValue);
		}
	}
	...
	PreviewDeviceProfileChangedDelegate.Broadcast();
}
...
void UDeviceProfileManager::RestorePreviewDeviceProfile(FName PreviewModeTag)
{
	...
	ActiveDeviceProfile->SetPreviewTextureGroupRedirect(nullptr);

	PreviewDeviceProfileScalabilityCVars.Empty();
...
{
	if (const FString* CVarValue = PreviewDeviceProfileScalabilityCVars.Find(CVarName))
	{
