...
{
	FPhysicsActorHandle ActorRef1 = Body1 ? Body1->GetPhysicsActor() : FPhysicsActorHandle();
	FPhysicsActorHandle ActorRef2 = Body2 ? Body2->GetPhysicsActor() : FPhysicsActorHandle();
...
void FConstraintInstance::SerializeState(Chaos::FSerializedDataBuffer& OutSerializedState, Chaos::ESerializedDataContext DataContext)
{
	...
	FPhysicsCommand::ExecuteRead(ConstraintHandle, [&OutSerializedState, DataContext](const FPhysicsConstraintHandle& Constraint)
	{
		FChaosEngineInterface::SerializeConstraintStateToBuffer(Constraint, OutSerializedState, DataContext);
	}, LockContext);
}

void FConstraintInstance::ApplySerializedState(Chaos::FSerializedDataBufferPtr&& InSerializedState, Chaos::ESerializedDataContext DataContext)
{
	const EPhysicsInterfaceScopedThreadContext LockContext = DataContext == Chaos::ESerializedDataContext::Internal ? EPhysicsInterfaceScopedThreadContext::Internal : EPhysicsInterfaceScopedThreadContext::External;
	FPhysicsCommand::ExecuteWrite(ConstraintHandle, [&InSerializedState, this, DataContext](const FPhysicsConstraintHandle& Constraint)
	{
		FChaosEngineInterface::ApplySerializedStateToConstraint(Constraint, MoveTemp(InSerializedState), DataContext);
