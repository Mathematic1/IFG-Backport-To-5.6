...
void FKTaperedCapsuleElem::DrawTaperedCapsuleSides(FPrimitiveDrawInterface* PDI, const FTransform& ElemTM, const FVector& InCenter0, const FVector& InCenter1, float InRadius0, float InRadius1, const FColor& Color, bool bInSplitTaperedCylinder)
{
	const FColor HalfTaperedColor = bInSplitTaperedCylinder ? (FLinearColor(Color) * .2f).ToFColor(true) : Color;
	...
	FVector StartVertex = CapsuleOrientation.RotateVector(FVector(0, ScaleXY, OffsetZ));
	FVector VertexPrevious = StartVertex;
	for(float Angle = AngleIncrementCircle; Angle <= 180.0f; Angle += AngleIncrementCircle)  // iterate over unit circle about capsule's major axis (which is orientation.AxisZ)
	{
		FVector VertexCurrent = CapsuleOrientation.RotateVector(FVector(FMath::Sin(FMath::DegreesToRadians(Angle)) * ScaleXY, FMath::Cos(FMath::DegreesToRadians(Angle))*ScaleXY, OffsetZ));
		PDI->DrawTranslucentLine(InCenter0 + VertexPrevious * InRadius0, InCenter0 + VertexCurrent * InRadius0, Color, SDPG_World);  // cap-circle segment on sphere S0
		PDI->DrawTranslucentLine(InCenter1 + VertexPrevious * InRadius1, InCenter1 + VertexCurrent * InRadius1, Color, SDPG_World);  // cap-circle segment on sphere S1
		VertexPrevious = VertexCurrent;
	}

	for (float Angle = 180.f + AngleIncrementCircle; Angle <= 360.0f; Angle += AngleIncrementCircle)  // iterate over unit circle about capsule's major axis (which is orientation.AxisZ)
	{
		FVector VertexCurrent = CapsuleOrientation.RotateVector(FVector(FMath::Sin(FMath::DegreesToRadians(Angle)) * ScaleXY, FMath::Cos(FMath::DegreesToRadians(Angle)) * ScaleXY, OffsetZ));
		PDI->DrawTranslucentLine(InCenter0 + VertexPrevious * InRadius0, InCenter0 + VertexCurrent * InRadius0, HalfTaperedColor, SDPG_World);  // cap-circle segment on sphere S0
		PDI->DrawTranslucentLine(InCenter1 + VertexPrevious * InRadius1, InCenter1 + VertexCurrent * InRadius1, HalfTaperedColor, SDPG_World);  // cap-circle segment on sphere S1
		VertexPrevious = VertexCurrent;
	}

	for (float Angle = 0; Angle <= 180.f; Angle += AngleIncrementSides)  // iterate over unit circle about capsule's major axis (which is orientation.AxisZ)
	{
		FVector VertexCurrent = CapsuleOrientation.RotateVector(FVector(FMath::Sin(FMath::DegreesToRadians(Angle)) * ScaleXY, FMath::Cos(FMath::DegreesToRadians(Angle)) * ScaleXY, OffsetZ));
		PDI->DrawTranslucentLine(InCenter0 + VertexCurrent * InRadius0, InCenter1 + VertexCurrent * InRadius1, Color, SDPG_World);  // capsule side segment between spheres
	}
	for(float Angle = 180.f + AngleIncrementSides; Angle < 360.0f; Angle += AngleIncrementSides)  // iterate over unit circle about capsule's major axis (which is orientation.AxisZ)
	{
		FVector VertexCurrent = CapsuleOrientation.RotateVector(FVector(FMath::Sin(FMath::DegreesToRadians(Angle)) * ScaleXY, FMath::Cos(FMath::DegreesToRadians(Angle)) * ScaleXY, OffsetZ));
		PDI->DrawTranslucentLine(InCenter0 + VertexCurrent  * InRadius0, InCenter1 + VertexCurrent * InRadius1, HalfTaperedColor, SDPG_World);  // capsule side segment between spheres
	}
}
...
{
	const FColor HalfTaperedColor = bOneSidedCollision ? ( FLinearColor(Color) * .2f).ToFColor(true) : Color;
	const FVector Origin = ElemTM.GetLocation();
	const FVector XAxis = ElemTM.GetScaledAxis(EAxis::X);
	const FVector YAxis = ElemTM.GetScaledAxis(EAxis::Y);
	const FVector ZAxis = ElemTM.GetScaledAxis(EAxis::Z);
	...
	if(Separation.IsNearlyZero() || Distance <= FMath::Abs(ScaledRadius0 - ScaledRadius1))
	{
		...
		{
			DrawArc(PDI, TopEnd, XAxis, YAxis, -90.f, 90.f, ScaledRadius0, DrawCollisionSides, Color, SDPG_World);
			DrawArc(PDI, TopEnd, XAxis, ZAxis, -90.f, 90.f, ScaledRadius0, DrawCollisionSides, Color, SDPG_World);

			DrawArc(PDI, TopEnd, XAxis, YAxis, 90.f, 270.f, ScaledRadius0, DrawCollisionSides, HalfTaperedColor, SDPG_World);
			DrawArc(PDI, TopEnd, XAxis, ZAxis, 90.f, 270.f, ScaledRadius0, DrawCollisionSides, HalfTaperedColor, SDPG_World);
			DrawCircle(PDI, TopEnd, YAxis, ZAxis, Color, ScaledRadius0, DrawCollisionSides, SDPG_World);
		}
		else
		{
			DrawArc(PDI, BottomEnd, XAxis, YAxis, -90.f, 90.f, ScaledRadius1, DrawCollisionSides, Color, SDPG_World);
			DrawArc(PDI, BottomEnd, XAxis, ZAxis, -90.f, 90.f, ScaledRadius1, DrawCollisionSides, Color, SDPG_World);

			DrawArc(PDI, BottomEnd, XAxis, YAxis, 90.f, 270.f, ScaledRadius1, DrawCollisionSides, HalfTaperedColor, SDPG_World);
			DrawArc(PDI, BottomEnd, XAxis, ZAxis, 90.f, 270.f, ScaledRadius1, DrawCollisionSides, HalfTaperedColor, SDPG_World);
	...
	else
	{
		...
		DrawArc(PDI, TopEnd, NegZAxis, XAxis, TopLimit, 180.f, ScaledRadius0, DrawCollisionSides, Color, SDPG_World);
		DrawArc(PDI, TopEnd, NegZAxis, XAxis, 180.f,360.0f - TopLimit , ScaledRadius0, DrawCollisionSides, HalfTaperedColor, SDPG_World);
		...
		DrawArc(PDI, BottomEnd, ZAxis, XAxis, BottomLimit, 180.f, ScaledRadius1, DrawCollisionSides, Color, SDPG_World);
		DrawArc(PDI, BottomEnd, ZAxis, XAxis, 180.f, 360.0f - BottomLimit, ScaledRadius1, DrawCollisionSides, HalfTaperedColor, SDPG_World);

		// Draw connecty lines
		DrawTaperedCapsuleSides(PDI, ElemTM, TopEnd, BottomEnd, ScaledRadius0, ScaledRadius1, Color, bOneSidedCollision);
	}
}
...
void FKMLLevelSetElem::DrawElemWire(class FPrimitiveDrawInterface* PDI, const FTransform& ElemTM, float Scale, const FColor Color) const
{
	TArray<FBox> Boxes;
...
void FKMLLevelSetElem::DrawElemSolid(class FPrimitiveDrawInterface* PDI, const FTransform& ElemTM, float Scale, const FMaterialRenderProxy* MaterialRenderProxy) const
{
	TArray<FVector3f> Vertices;
	TArray<FIntVector> Tris;
	GetZeroIsosurfaceGridCellFaces(Vertices, Tris);
...
void FKMLLevelSetElem::GetElemSolid(const FTransform& ElemTM, const FVector& Scale3D, const FMaterialRenderProxy* MaterialRenderProxy, int32 ViewIndex, class FMeshElementCollector& Collector) const
{
	TArray<FVector3f> Vertices;
	TArray<FIntVector> Tris;
	GetZeroIsosurfaceGridCellFaces(Vertices, Tris);
...
/////////////////////////////////////////////////////////////////////////////////////
// FKSkinnedTriangleMesh
/////////////////////////////////////////////////////////////////////////////////////
void FKSkinnedTriangleMeshElem::DrawElemWire(class FPrimitiveDrawInterface* PDI, const FTransform& ElemTM, float Scale, const FColor Color) const
{
	if (SkinnedTriangleMesh.IsValid())
	{
		const TArray<Chaos::TVec3<int32>>& Elements = SkinnedTriangleMesh->GetTriangleMesh().GetElements();
		const TConstArrayView<Chaos::Softs::FSolverVec3>& Positions = SkinnedTriangleMesh->GetLocalPositions();
		PDI->AddReserveLines(SDPG_World, Elements.Num() * 3);
		for (const Chaos::TVec3<int32>& Element : Elements)
		{
			const FVector P0 = ElemTM.TransformPosition(FVector(Positions[Element[0]]));
			const FVector P1 = ElemTM.TransformPosition(FVector(Positions[Element[1]]));
			const FVector P2 = ElemTM.TransformPosition(FVector(Positions[Element[2]]));
...
void FKSkinnedTriangleMeshElem::DrawElemSolid(class FPrimitiveDrawInterface* PDI, const FTransform& ElemTM, const float Scale, const FMaterialRenderProxy* MaterialRenderProxy) const
{
	if (SkinnedTriangleMesh.IsValid())
	{
		const TArray<Chaos::TVec3<int32>>& Elements = SkinnedTriangleMesh->GetTriangleMesh().GetElements();
		...
		FDynamicMeshBuilder MeshBuilder(PDI->View->GetFeatureLevel());
		for (const Chaos::Softs::FSolverVec3& Position : Positions)
		{
			MeshBuilder.AddVertex(FDynamicMeshVertex(FVector3f(Position)));
		}
		for (const Chaos::TVec3<int32>& Element : Elements)
		{
...
void FKSkinnedTriangleMeshElem::GetElemSolid(const FTransform& ElemTM, const FVector& Scale3D, const FMaterialRenderProxy* MaterialRenderProxy, int32 ViewIndex, class FMeshElementCollector& Collector) const
{
	if (SkinnedTriangleMesh.IsValid())
	{
		const TArray<Chaos::TVec3<int32>>& Elements = SkinnedTriangleMesh->GetTriangleMesh().GetElements();
...
void FKAggregateGeom::GetAggGeom(const FTransform& Transform, const FColor Color, const FMaterialRenderProxy* MatInst, bool bPerHullColor, bool bDrawSolid, bool bOutputVelocity, int32 ViewIndex, FMeshElementCollector& Collector) const
{
	...
	for (int32 i = 0; i < MLLevelSetElems.Num(); i++)
	{
		if (bDrawSolid)
		{
			MLLevelSetElems[i].GetElemSolid(Transform, Scale3D, MatInst, ViewIndex, Collector);
		}
		else
		{
			MLLevelSetElems[i].DrawElemWire(Collector.GetPDI(ViewIndex), Transform, 1.f, Color);
		}
	}

	for (int32 i = 0; i < SkinnedTriangleMeshElems.Num(); i++)
	{
		if (bDrawSolid)
		{
			SkinnedTriangleMeshElems[i].GetElemSolid(Transform, Scale3D, MatInst, ViewIndex, Collector);
		}
		else
		{
			SkinnedTriangleMeshElems[i].DrawElemWire(Collector.GetPDI(ViewIndex), Transform, 1.f, Color);
		}
	}
}
...
void UPhysicsAsset::GetCollisionMesh(int32 ViewIndex, FMeshElementCollector& Collector, const FReferenceSkeleton& RefSkeleton, const TArray<FTransform>& SpaceBases, const FTransform& LocalToWorld, const FVector& Scale3D) const
{
...
void UPhysicsAsset::DrawConstraints(int32 ViewIndex, FMeshElementCollector& Collector, const FReferenceSkeleton& RefSkeleton, const TArray<FTransform>& SpaceBases, const FTransform& LocalToWorld, float Scale) const
{
	for (int32 i = 0; i < ConstraintSetup.Num(); i++)
	{
		FConstraintInstance& Instance = ConstraintSetup[i]->DefaultInstance;
