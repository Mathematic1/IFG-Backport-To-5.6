...
void UBodySetup::AddCollisionFrom(const FKAggregateGeom& FromAggGeom)
{
	...
	AggGeom.MLLevelSetElems.Append(FromAggGeom.MLLevelSetElems);
}
...
{
	ClearPhysicsMeshes();
	// We use the BodySetupGuid as a change indicator for DDC. It would be better to create it deterministically
	// from the physics data but that is not yet implemented. As a workaround we assign it an arbitrary unique value
	// every time the physics data changes.
...
void UBodySetup::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsLoading() && !BodySetupGuid.IsValid())
	{
		...
		UE_LOG(LogPhysics, Log, TEXT("FIX GUID FOR: %s"), *PathName);
		// For the autocreated guid for an old version, we construct it to a constant value; this is important
		// for determinism during cooking if a legacy BodySetup is encountered during cooking. The BodySetupGuid is used
		// as a change-indicator for the BodySetup's DDC key, so this constant value would ordinarily be a problem, but
		// it is okay in this legacy-version handling because the physics data stored in the package will always be the
		// same, unless the package is resaved. And when the package is resaved, the new code will calculate a unique
		// BodySetupGuid for it and store that BodySetupGuid in the serialized data.
		BodySetupGuid = FGuid::NewDeterministicGuid(PathName);
	}
...
UBodySetup::FBodySetupTryCookResult UBodySetup::TryGetCookedData(FName Format)
{
	...
	const bool bIsRuntime = IsRuntime(this);

#if WITH_EDITOR
	if(!UseCookedData->Contains(Format))
	{
		...
		// Note: We also do this *before* the below scope lock, to avoid deadlock 
		// (because ContainsPhysicsTriMeshData can call GetRenderData() for static mesh, which will wait for async build to run, 
		//  which will try to take this same lock on a separate thread when the async mesh build calls this same method)
		if ((CDP == nullptr || CDP->ContainsPhysicsTriMeshData(bMeshCollideAll) == false) && AggGeom.ConvexElems.Num() == 0)
		{
			return { nullptr, CookDataResult };
		}

		// Only one thread should build the physics data. Below calling GetFormat and TryGetDDCBuiltData will
		// both mutate shared state so we protect this operation with a lock.
		FScopeLock ScopeLock(&CookedFormatDataMutex);

		// Check the lock condition again, we only want the first thread in here to actually build the data,
		// while it's building other threads should wait and proceed after the build completes.
		// Double-checked locking avoids always taking the lock - we only do it for unbuilt data
		if(!UseCookedData->Contains(Format))
		{
		...
		{
			// We were a waiting thread while another thread built the data, just take it from the container now
			Result = &UseCookedData->GetFormat(Format);
		}
	}
	else
...
void UBodySetup::PostInitProperties()
{
	...
	if (!IsTemplate())
	{
		// When constructing a BodySetup, we initialize its guid to a constant value; this is important
		// for determinism during cooking if a new BodySetup is created during cooking. The BodySetupGuid is used
		// as a change-indicator for the BodySetup's DDC key, so it is not possible to use a constant value for it
		// for changes to the BodySetup, but since a new BodySetup is created into an initial state that never varies,
		// making the initial value of BodySetupGuid a constant is okay.
		BodySetupGuid = FGuid::NewDeterministicGuid(GetPathName());
	}
}
...
void FKMLLevelSetElem::CloneElem(const FKMLLevelSetElem& Other)
{
	Super::CloneElem(Other);
	MLLevelSet = Other.MLLevelSet;
	NNESignedDistanceModelData = Other.NNESignedDistanceModelData;
	NNEIncorrectZoneModelData = Other.NNEIncorrectZoneModelData;
}

void FKSkinnedTriangleMeshElem::CloneElem(const FKSkinnedTriangleMeshElem& Other)
{
	Super::CloneElem(Other);
	SkinnedTriangleMesh = Other.SkinnedTriangleMesh;
}
