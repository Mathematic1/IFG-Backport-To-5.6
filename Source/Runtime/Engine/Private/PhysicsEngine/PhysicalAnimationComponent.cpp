		...
		FPhysicsCommand::ExecuteWrite(SkeletalMeshComponent, [&]()
		{
			...
			for (int32 DataIdx = 0; DataIdx < DriveData.Num(); ++DataIdx)
			{
				...
				if(bNewConstraint)
				{
					...
					{
						if (FPhysicsActorHandle ActorHandle = ChildBody->GetPhysicsActor())
						{
							...
							ConstraintInstance->InitConstraint_AssumesLocked(ChildBody->GetPhysicsActor(), InstanceData.TargetActor, 1.f);
						}
					}
				}
			}
