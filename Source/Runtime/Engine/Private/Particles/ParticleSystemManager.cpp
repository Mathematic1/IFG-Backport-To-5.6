...
{
	static constexpr ETickingGroup FirstTickGroup = TG_PrePhysics;
	static constexpr ETickingGroup LastTickGroup = TG_LastDemotable;
	static constexpr int NumTickGroups = LastTickGroup - FirstTickGroup + 1;

	static int32 GFirstHighPriTickGroup = 0;
	static FAutoConsoleVariableRef CVarFirstHighPriTickGroup(
		TEXT("fx.ParticleManagerFirstHighPriTickGroup"),
		...
		FConsoleVariableDelegate::CreateLambda(
			[](IConsoleVariable*)
			{
				FParticleSystemWorldManager::SetFirstHighPriTickGroup(GFirstHighPriTickGroup);
			}
		),
		ECVF_Default
	);

	static bool GetParticleSystemTickGroupPriority(int ParticleSystemTickGroup)
	{
		return ParticleSystemTickGroup >= GFirstHighPriTickGroup;
	}
}
	...
	: World(InWorld)
{
	...
	for (int32 TickGroup = 0; TickGroup < TickFunctions.Num(); ++TickGroup)
	{
		...
		TickFunc.bHighPriority = ParticleSystemWorldManagerInternal::GetParticleSystemTickGroupPriority(TickGroup);
		TickFunc.Owner = this;
...
void FParticleSystemWorldManager::SetTickGroupPriority(const int32 ParticleSystemTickGroupIndex, const bool bHighPriority)
{
	check((ParticleSystemTickGroupIndex >= 0) && (ParticleSystemTickGroupIndex < ParticleSystemWorldManagerInternal::NumTickGroups));
	TickFunctions[ParticleSystemTickGroupIndex].SetPriorityIncludingPrerequisites(bHighPriority);
}

void FParticleSystemWorldManager::SetFirstHighPriTickGroup(int32 FirstHighPriTickGroupIndex)
{
	for (auto& Pair : WorldManagers)
	{
		FParticleSystemWorldManager* WorldManager = Pair.Value;
		if (WorldManager)
		{
			for (int32 i = 0; i < ParticleSystemWorldManagerInternal::NumTickGroups; i++)
			{
				bool HighPriority = i >= FirstHighPriTickGroupIndex;
				WorldManager->SetTickGroupPriority(i, HighPriority);
			}
		}
	}
}
