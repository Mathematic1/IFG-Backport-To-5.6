...
// 		}
		...
		LastTickTime = Component.GetWorldTimeSeconds();
...
void FParticleTrailsEmitterInstance_Base::UpdateBoundingBox(float DeltaTime)
{
	SCOPE_CYCLE_COUNTER(STAT_ParticleUpdateBounds);
	{
		...
		if (bUpdateBox)
		{
			...
			{
				ParticleBoundingBox.Max = Component.GetComponentTransform().GetLocation();
				ParticleBoundingBox.Min = Component.GetComponentTransform().GetLocation();
			}
			else
			{
...
void FParticleTrailsEmitterInstance_Base::KillParticles()
{
	if (ActiveParticles > 0)
	{
		float CurrentTickTime = Component.GetWorldTimeSeconds();
...
FParticleRibbonEmitterInstance::FParticleRibbonEmitterInstance(IParticleEmitterInstanceOwner& InComponent) :
	  FParticleTrailsEmitterInstance_Base(InComponent)
	, TrailTypeData(NULL)
...
{
	FParticleTrailsEmitterInstance_Base::InitParameters(InTemplate);
...
bool FParticleRibbonEmitterInstance::GetSpawnPerUnitAmount(float DeltaTime, int32 InTrailIdx, int32& OutCount, float& OutRate)
{
	...
	if (SpawnPerUnitModule && SpawnPerUnitModule->bEnabled)
	{
		...
		float ParticlesPerUnit = SpawnPerUnitModule->SpawnPerUnit.GetValue(EmitterTime, Component.GetDistributionData()) / SpawnPerUnitModule->UnitScalar;
...
void FParticleRibbonEmitterInstance::GetParticleLifetimeAndSize(int32 InTrailIdx, const FBaseParticle* InParticle, bool bInNoLivingParticles, float& OutOneOverMaxLifetime, float& OutSize)
{
	if (bInNoLivingParticles == true)
	{
		...
		for (int32 ModuleIdx = 0; ModuleIdx < LODLevel->SpawnModules.Num(); ModuleIdx++)
		{
			...
			{
				float MaxLifetime = LifetimeModule->GetLifetimeValue({*this}, EmitterTime, Component.GetDistributionData());
				if (CurrLifetime > 0.f)
				{
...
float FParticleRibbonEmitterInstance::Spawn(float DeltaTime)
{
	...
	if (bProcessSpawnRate)
	{
		...
		SpawnRate += LODLevel->SpawnModule->Rate.GetValue(EmitterTime, Component.GetDistributionData()) * FMath::Clamp<float>(QualityMult, 0.0f, 1.0);
	}
		...
		for (int32 SpawnIdx = 0; SpawnIdx < SpawnNumber; SpawnIdx++)
		{
			...
			if (bAddedParticle)
			{
				...
				if ((int32)ActiveParticles > LocalMaxParticleInTrailCount)
				{
					...
					{
						const FString TemplateName = Component.GetTemplate() ? *Component.GetTemplate()->GetName() : TEXT("No template");
		...
		if (MovementSpawnCount > 0)
		{
			...
			for (int32 SpawnIdx = 0; SpawnIdx < MovementSpawnCount; SpawnIdx++, CurrTimeStep += InvCount)
			{
				...
				for (int32 SpawnModuleIdx = 0; SpawnModuleIdx < LODLevel->SpawnModules.Num(); SpawnModuleIdx++)
				{
					...
					SpawnModule->Spawn({ *this, (int32)GetModuleDataOffset(SpawnModule), SpawnTime, Particle });
				}
					...
					if ((int32)ActiveParticles > LocalMaxParticleInTrailCount)
					{
						if (Component.HasWorld())
						{
							FString ErrorMessage = 
								...
								Component.GetTemplate() ? *(Component.GetTemplate()->GetName()) : TEXT("No template"));
							UE_LOG(LogParticles, Log, TEXT("%s"), *ErrorMessage);
						}
					}
#endif	//#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
...
void FParticleRibbonEmitterInstance::ResolveSource()
{
	if (SourceModule && SourceModule->SourceName != NAME_None)
	{
		...
		case PET2SRCM_Actor:
			if (SourceActor == NULL)
			{
				TArrayView<const FParticleSysParam> AsyncInstanceParameters = Component.GetAsyncInstanceParameters();
		...
		case PET2SRCM_Particle:
			if (SourceEmitter == NULL)
			{
				...
				{
					FParticleEmitterInstance* EmitInst = EmitterInstances[EmitterIdx];
	...
	FVector& OutPosition, FQuat& OutRotation, FVector& OutUp, FVector& OutTangent, float& OutTangentStrength)
{
		...
		case PET2SRCM_Particle:
			{
				...
				if (SourceEmitter && SourceEmitter->ParticleIndices && SourceEmitter->ActiveParticles > 0)
				{
					...
					if (SourceParticle != nullptr)
					{
						...
						IParticleEmitterInstanceOwner& Comp = SourceEmitter->Component;
					...
					else
					{
						...
						OutPosition = SourceEmitter->Component.GetComponentTransform().GetLocation();
						OutTangent = Component.GetPartSysVelocity();
	...
	{
		OutPosition = Component.GetComponentTransform().GetLocation();
		...
		OutRotation = Component.GetComponentTransform().GetRotation();
		OutTangent = Component.GetPartSysVelocity();
		OutTangentStrength = OutTangent.SizeSquared();
...
bool FParticleRibbonEmitterInstance::FillReplayData(FDynamicEmitterReplayDataBase& OutData)
{
	...
	if (IndexCount > MAX_TRAIL_INDICES)
	{
		...
		UE_LOG(LogParticles, Warning, TEXT("\t%s"), Component.GetTemplate() ? 
			*(Component.GetTemplate()->GetPathName()) : *(Component.GetName()));
#endif
		return false;
	}
...
//
/** Constructor	*/
FParticleAnimTrailEmitterInstance::FParticleAnimTrailEmitterInstance(IParticleEmitterInstanceOwner& InComponent) :
	  FParticleTrailsEmitterInstance_Base(InComponent)
	, TrailTypeData(NULL)
	, SpawnPerUnitModule(NULL)
...
void FParticleAnimTrailEmitterInstance::InitParameters(UParticleEmitter* InTemplate)
{
	FParticleTrailsEmitterInstance_Base::InitParameters(InTemplate);
...
//	else
	{
		UMeshComponent* MeshComp = Cast<UMeshComponent>(Component.AsComponent()->GetAttachParent());
		check(TrailTypeData);
		check(MeshComp);
...
//	for (int32 TrailIdx = 0; TrailIdx < MaxTrailCount; TrailIdx++)
	...
	if (bProcessSpawnRate)
	{
		...
		SpawnRate += LODLevel->SpawnModule->Rate.GetValue(EmitterTime, Component.GetDistributionData()) * FMath::Clamp<float>(RateScale, 0.0f, RateScale);
	}
...
void FParticleTrailsEmitterInstance_Base::DumpCircularTrailsSpam()
{
	...
	{
		UE_LOG(LogParticles, Warning, TEXT("Circular links in Anim Trail discovered. \nPSys: %s\nEmitter: %s"), *Component.GetTemplate()->GetFullName(), *SpriteTemplate->GetEmitterName().ToString());
