	...
	{
		{
			const FRHIBufferCreateDesc CreateDesc =
				FRHIBufferCreateDesc::CreateVertex<uint32>(TEXT("PartialSortKeyBuffer"), BufferSize)
				.AddUsage(EBufferUsageFlags::Static | EBufferUsageFlags::ShaderResource | EBufferUsageFlags::UnorderedAccess)
				.DetermineInitialState();

			KeyBuffers[BufferIndex] = RHICmdList.CreateBuffer(CreateDesc);
		...
		{
			const FRHIBufferCreateDesc CreateDesc =
				FRHIBufferCreateDesc::CreateVertex<uint32>(TEXT("PartialSortVertexBuffer"), BufferSize)
				.AddUsage(EBufferUsageFlags::Static | EBufferUsageFlags::ShaderResource | EBufferUsageFlags::UnorderedAccess)
				.DetermineInitialState();

			VertexBuffers[BufferIndex] = RHICmdList.CreateBuffer(CreateDesc);
			VertexBufferSortSRVs[BufferIndex] = RHICmdList.CreateShaderResourceView(
				VertexBuffers[BufferIndex],
				FRHIViewDesc::CreateBufferSRV()
					.SetType(FRHIViewDesc::EBufferType::Typed)
					.SetFormat(PF_R32_UINT));
			VertexBufferSortUAVs[BufferIndex] = RHICmdList.CreateUnorderedAccessView(
				VertexBuffers[BufferIndex],
				FRHIViewDesc::CreateBufferUAV()
					.SetType(FRHIViewDesc::EBufferType::Typed)
					.SetFormat(PF_R32_UINT));
