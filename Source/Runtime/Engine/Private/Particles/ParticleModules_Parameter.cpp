...
void UParticleModuleParameterDynamic::Spawn(const FSpawnContext& Context)
{
	SpawnEx(Context, &GetRandomStream(Context));
}

void UParticleModuleParameterDynamic::SpawnEx(const FSpawnContext& Context, struct FRandomStream* InRandomStream)
{
	...
	SPAWN_INIT;
	{
		...
		{
			DynamicPayload.DynamicParameterValue[0] = GetParameterValue(Context, DynamicParams[0], Particle, Owner, InRandomStream);
		}
		if (DynamicParams[1].ValueMethod != EDPV_AutoSet)
		{
			DynamicPayload.DynamicParameterValue[1] = GetParameterValue(Context, DynamicParams[1], Particle, Owner, InRandomStream);
		}
		if (DynamicParams[2].ValueMethod != EDPV_AutoSet)
		{
			DynamicPayload.DynamicParameterValue[2] = GetParameterValue(Context, DynamicParams[2], Particle, Owner, InRandomStream);
		}
		if (DynamicParams[3].ValueMethod != EDPV_AutoSet)
		{
			DynamicPayload.DynamicParameterValue[3] = GetParameterValue(Context, DynamicParams[3], Particle, Owner, InRandomStream);
		}
	}
}

void UParticleModuleParameterDynamic::Update(const FUpdateContext& Context)
{
	...
	case EDPU_UPDATE_3:
		{
			...
			if (bUsesVelocity == false)
			{
				BEGIN_UPDATE_LOOP;
				{
					...
					DynamicPayload.DynamicParameterValue[ParameterIndex] = GetParameterValue_UserSet(Context, DynParam, Particle, Owner, &RandomStream);
				}
				END_UPDATE_LOOP;
			}
			else
			{
				BEGIN_UPDATE_LOOP;
				{
					...
					FPlatformMisc::Prefetch(ParticleData, (ParticleIndices[i+1] * ParticleStride));
					FPlatformMisc::Prefetch(ParticleData, (ParticleIndices[i+1] * ParticleStride) + PLATFORM_CACHE_LINE_SIZE);
					DynamicPayload.DynamicParameterValue[ParameterIndex] = GetParameterValue(Context, DynParam, Particle, Owner, &RandomStream);
				}
				END_UPDATE_LOOP;
			}
		}
	...
	case EDPU_UPDATE_01:
		{
			...
			if (bUsesVelocity == false)
			{
				BEGIN_UPDATE_LOOP;
				{
					...
					FPlatformMisc::Prefetch(ParticleData, (ParticleIndices[i+1] * ParticleStride));
					FPlatformMisc::Prefetch(ParticleData, (ParticleIndices[i+1] * ParticleStride) + PLATFORM_CACHE_LINE_SIZE);
					DynamicPayload.DynamicParameterValue[0] = GetParameterValue_UserSet(Context, DynParam0, Particle, Owner, &RandomStream);
					DynamicPayload.DynamicParameterValue[1] = GetParameterValue_UserSet(Context, DynParam1, Particle, Owner, &RandomStream);
				}
				END_UPDATE_LOOP;
			}
			else
			{
				BEGIN_UPDATE_LOOP;
				{
					...
					FPlatformMisc::Prefetch(ParticleData, (ParticleIndices[i+1] * ParticleStride));
					FPlatformMisc::Prefetch(ParticleData, (ParticleIndices[i+1] * ParticleStride) + PLATFORM_CACHE_LINE_SIZE);
					DynamicPayload.DynamicParameterValue[0] = GetParameterValue(Context, DynParam0, Particle, Owner, &RandomStream);
					DynamicPayload.DynamicParameterValue[1] = GetParameterValue(Context, DynParam1, Particle, Owner, &RandomStream);
				}
				END_UPDATE_LOOP;
			}
		}
	...
	case EDPU_UPDATE_012:
		{
			...
			if (bUsesVelocity == false)
			{
				BEGIN_UPDATE_LOOP;
				{
					...
					DynamicPayload.DynamicParameterValue[0] = GetParameterValue_UserSet(Context, DynParam0, Particle, Owner, &RandomStream);
					DynamicPayload.DynamicParameterValue[1] = GetParameterValue_UserSet(Context, DynParam1, Particle, Owner, &RandomStream);
					DynamicPayload.DynamicParameterValue[2] = GetParameterValue_UserSet(Context, DynParam2, Particle, Owner, &RandomStream);
				}
				END_UPDATE_LOOP;
			}
			else
			{
				BEGIN_UPDATE_LOOP;
				{
					...
					DynamicPayload.DynamicParameterValue[0] = GetParameterValue(Context, DynParam0, Particle, Owner, &RandomStream);
					DynamicPayload.DynamicParameterValue[1] = GetParameterValue(Context, DynParam1, Particle, Owner, &RandomStream);
					DynamicPayload.DynamicParameterValue[2] = GetParameterValue(Context, DynParam2, Particle, Owner, &RandomStream);
				}
				END_UPDATE_LOOP;
			}
		}
	...
	case EDPU_UPDATE_ALL:
		{
			...
			if (bUsesVelocity == false)
			{
				BEGIN_UPDATE_LOOP;
				{
					...
					DynamicPayload.DynamicParameterValue[1] = GetParameterValue_UserSet(Context, DynParam1, Particle, Owner, &RandomStream);
					DynamicPayload.DynamicParameterValue[2] = GetParameterValue_UserSet(Context, DynParam2, Particle, Owner, &RandomStream);
					DynamicPayload.DynamicParameterValue[3] = GetParameterValue_UserSet(Context, DynParam3, Particle, Owner, &RandomStream);
				}
				END_UPDATE_LOOP;
			}
			else
			{
				BEGIN_UPDATE_LOOP;
				{
					...
					DynamicPayload.DynamicParameterValue[1] = GetParameterValue(Context, DynParam1, Particle, Owner, &RandomStream);
					DynamicPayload.DynamicParameterValue[2] = GetParameterValue(Context, DynParam2, Particle, Owner, &RandomStream);
					DynamicPayload.DynamicParameterValue[3] = GetParameterValue(Context, DynParam3, Particle, Owner, &RandomStream);
				}
				END_UPDATE_LOOP;
			}
		}
	...
	default:
		{
			...
			if (bUsesVelocity == false)
			{
				BEGIN_UPDATE_LOOP;
				{
					...
					FPlatformMisc::Prefetch(ParticleData, (ParticleIndices[i+1] * ParticleStride));
					FPlatformMisc::Prefetch(ParticleData, (ParticleIndices[i+1] * ParticleStride) + PLATFORM_CACHE_LINE_SIZE);
					DynamicPayload.DynamicParameterValue[0] = (UpdateFlags & EDPU_UPDATE_0) ? GetParameterValue_UserSet(Context, DynParam0, Particle, Owner, &RandomStream) : DynamicPayload.DynamicParameterValue[0];
					DynamicPayload.DynamicParameterValue[1] = (UpdateFlags & EDPU_UPDATE_1) ? GetParameterValue_UserSet(Context, DynParam1, Particle, Owner, &RandomStream) : DynamicPayload.DynamicParameterValue[1];
					DynamicPayload.DynamicParameterValue[2] = (UpdateFlags & EDPU_UPDATE_2) ? GetParameterValue_UserSet(Context, DynParam2, Particle, Owner, &RandomStream) : DynamicPayload.DynamicParameterValue[2];
					DynamicPayload.DynamicParameterValue[3] = (UpdateFlags & EDPU_UPDATE_3) ? GetParameterValue_UserSet(Context, DynParam3, Particle, Owner, &RandomStream) : DynamicPayload.DynamicParameterValue[3];
				}
				END_UPDATE_LOOP;
			}
			else
			{
				BEGIN_UPDATE_LOOP;
				{
					...
					FPlatformMisc::Prefetch(ParticleData, (ParticleIndices[i+1] * ParticleStride));
					FPlatformMisc::Prefetch(ParticleData, (ParticleIndices[i+1] * ParticleStride) + PLATFORM_CACHE_LINE_SIZE);
					DynamicPayload.DynamicParameterValue[0] = (UpdateFlags & EDPU_UPDATE_0) ? GetParameterValue(Context, DynParam0, Particle, Owner, &RandomStream) : DynamicPayload.DynamicParameterValue[0];
					DynamicPayload.DynamicParameterValue[1] = (UpdateFlags & EDPU_UPDATE_1) ? GetParameterValue(Context, DynParam1, Particle, Owner, &RandomStream) : DynamicPayload.DynamicParameterValue[1];
					DynamicPayload.DynamicParameterValue[2] = (UpdateFlags & EDPU_UPDATE_2) ? GetParameterValue(Context, DynParam2, Particle, Owner, &RandomStream) : DynamicPayload.DynamicParameterValue[2];
					DynamicPayload.DynamicParameterValue[3] = (UpdateFlags & EDPU_UPDATE_3) ? GetParameterValue(Context, DynParam3, Particle, Owner, &RandomStream) : DynamicPayload.DynamicParameterValue[3];
				}
				END_UPDATE_LOOP;
			}
		}
