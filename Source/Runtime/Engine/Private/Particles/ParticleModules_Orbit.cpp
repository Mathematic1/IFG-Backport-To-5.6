...
void UParticleModuleOrbit::Spawn(const FSpawnContext& Context)
{
	...
	SPAWN_INIT;
	{
		...
		if (OffsetOptions.bProcessDuringSpawn == true)
		{
			...
			{
				LocalOffset = OffsetAmount.GetValue(Particle.RelativeTime, Context.GetDistributionData());
			}
			else
			{
				LocalOffset = OffsetAmount.GetValue(Owner->EmitterTime, Context.GetDistributionData());
			}
		...
		if (RotationOptions.bProcessDuringSpawn == true)
		{
			...
			{
				LocalRotation = RotationAmount.GetValue(Particle.RelativeTime, Context.GetDistributionData());
			}
			else
			{
				LocalRotation = RotationAmount.GetValue(Owner->EmitterTime, Context.GetDistributionData());
			}
		...
		if (RotationRateOptions.bProcessDuringSpawn == true)
		{
			...
			{
				LocalRotationRate = RotationRateAmount.GetValue(Particle.RelativeTime, Context.GetDistributionData());
			}
			else
			{
				LocalRotationRate = RotationRateAmount.GetValue(Owner->EmitterTime, Context.GetDistributionData());
			}
...
void UParticleModuleOrbit::Update(const FUpdateContext& Context)
{
	FParticleEmitterInstance* Owner = &Context.Owner;
	BEGIN_UPDATE_LOOP;
	{
