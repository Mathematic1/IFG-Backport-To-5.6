...
const TArray<FAssetData> UAudioWidgetSubsystem::GetBlueprintAssetData(UClass* InInterfaceClass)
{
	...
	Filter.ClassPaths.Add(FTopLevelAssetPath(TEXT("/Script/Blutility"), TEXT("EditorUtilityWidget")));
...
{
	UClass* NativeClass = UAssetRegistryHelpers::FindAssetNativeClass(InAssetData);
	// Native interfaces can't use the BP tag system
	if (InAssetData.GetClass()->ImplementsInterface(InInterfaceClass))
	{
		return true;
	}
