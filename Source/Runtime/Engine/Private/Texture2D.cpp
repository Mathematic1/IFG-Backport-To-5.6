...
void FVirtualTexture2DResource::InitializeEditorResources(IVirtualTexture* InVirtualTexture)
{
	...
	if (GIsEditor && !IsRunningCommandlet())
	{
		...
		const uint32 MaxTargetSize = FMath::Min(1024u, GetMax2DTextureDimension());
		const uint32 TargetSize = FMath::Clamp(128u, GetTileSize(), MaxTargetSize);
		const uint32 MaxMipLevel = VTData->GetNumMips() - 1u;
		
		uint32 MipLevel = 0;
		uint32 MipWidth = GetSizeX();
		uint32 MipHeight = GetSizeY();
		while (((MipWidth > TargetSize && MipHeight > TargetSize) || MipWidth > MaxTargetSize || MipHeight > MaxTargetSize) && MipLevel < MaxMipLevel)
		{
			++MipLevel;
		...
		else
		{
			...
			if (Texture2DRHI == nullptr)
			{
				const FRHITextureCreateDesc Desc =
					FRHITextureCreateDesc::Create2D(TEXT("VTEditorResource"), MipWidthInTiles * TileSizeInPixels, MipHeightInTiles * TileSizeInPixels, PixelFormat)
					.SetFlags(ETextureCreateFlags::ShaderResource);

				Texture2DRHI = RHICreateTexture(Desc);
			}
			
			TRefCountPtr<IPooledRenderTarget> PooledRenderTarget = CreateRenderTarget(Texture2DRHI, TEXT("VTEditorResource"));
