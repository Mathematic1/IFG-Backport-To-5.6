...
static_assert(sizeof(uint16) == sizeof(FBoneIndexType));

bool FSoftSkinVertex::GetRigidWeightBone(uint16& OutBoneIndex) const
{
	bool bIsRigid = false;
...
void FSkeletalMeshLODModel::GetMeshDescription(const USkeletalMesh *InSkeletalMesh, const int32 InLODIndex, FMeshDescription& OutMeshDescription) const
{
	...
	OutMeshDescription.SetNumUVChannels(static_cast<int32>(NumTexCoords));
	...
	for (TPair<FName, const FMorphTargetLODModel*>& MorphSource: MorphTargets)
	{
		...
		{
			if (SourceToTargetVertexMap.IsValidIndex(Delta.SourceIdx))
			{
	...
	for (const TPair<FName, FImportedSkinWeightProfileData>& SkinWeightProfileInfo: SkinWeightProfiles)
	{
...
#if 0
		// This version reads the copied import data influences. This is less precise than the ground truth version below
		// which is what will actually be used at render time.
		...
		for (int32 SectionIndex = 0; SectionIndex < Sections.Num(); SectionIndex++)
		{
			...
			{
				FRawSkinWeight RawSkinWeights = SkinWeightProfileData.SkinWeights[Index];
				const int32 TargetVertexIndex = SourceToTargetVertexMap[Index];
				const FVertexID VertexID = VertexIDs[TargetVertexIndex];

				for (int32 WeightIndex = 0; WeightIndex < MAX_TOTAL_INFLUENCES; WeightIndex++)
				{
					const uint16 BoneWeight = RawSkinWeights.InfluenceWeights[WeightIndex];
					FBoneIndexType& BoneIndex = RawSkinWeights.InfluenceBones[WeightIndex];
					if (BoneWeight != 0 && ensure(BoneMap.IsValidIndex(BoneIndex)))
					{
