...
{
	return SourceFilename.StartsWith(GeneratedByEnginePrefix);
}

void FMorphTargetImportedSourceFileInfo::SetGeneratedByEngine(bool bInGeneratedByEngine)
{
	FString NewSourceFilename = GeneratedByEnginePrefix;
	NewSourceFilename += FGuid::NewGuid().ToString(EGuidFormats::Base36Encoded);
