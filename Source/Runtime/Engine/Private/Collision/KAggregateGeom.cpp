...
static FAutoConsoleVariableRef CVarShowExperimentalImportMLLevelSetButton(TEXT("p.Experimental.EnableMLLevelSet"), bEnableMLLevelSet, TEXT("Shows Import MLLevelSet button. [def: false]"));
...
FBox FKAggregateGeom::CalcAABB(const FTransform& Transform) const
{
	...
	{
		Box += MLLevelSetElems[i].CalcAABB(BoneTM, (FVector)Scale3D);
	}

	for (int32 i = 0; i < SkinnedTriangleMeshElems.Num(); i++)
	{
		Box += SkinnedTriangleMeshElems[i].CalcAABB(BoneTM, (FVector)Scale3D);
	}
...
{
	WeightedLatticeLevelSet = MoveTemp(InWeightedLevelSet);
}
...
FBox FKMLLevelSetElem::CalcAABB(const FTransform& BoneTM, const FVector& Scale3D) const
{
	FBox Box(ForceInit);

	if (MLLevelSet.IsValid())
	{
		Box = FBox(MLLevelSet->BoundingBox().Min(), MLLevelSet->BoundingBox().Max());
		const FTransform LocalToWorld = FTransform(FQuat::Identity, FVector::ZeroVector, Scale3D) * BoneTM;
		return Box.TransformBy(LocalToWorld);
	}
...
FIntVector3 FKMLLevelSetElem::GridResolution() const
{
	if (MLLevelSet.IsValid())
	{
		const Chaos::TVec3<int32>& Dim = MLLevelSet->GetGrid().Counts();
		return FIntVector3(Dim[0], Dim[1], Dim[2]);
	}
...
{
	NNESignedDistanceModelData = LoadObject<UNNEModelData>(GetTransientPackage(), *MLLevelSetImportData.NNEModelDataArr[0].NNEModelPath);
	NNEIncorrectZoneModelData = LoadObject<UNNEModelData>(GetTransientPackage(), *MLLevelSetImportData.NNEModelDataArr[1].NNEModelPath);
		
	MLLevelSetImportData.NNEModelDataArr[0].NNEModelData = NNESignedDistanceModelData;
	MLLevelSetImportData.NNEModelDataArr[1].NNEModelData = NNEIncorrectZoneModelData;

	MLLevelSet = TSharedPtr<Chaos::FMLLevelSet>(new Chaos::FMLLevelSet(MoveTemp(MLLevelSetImportData)));
}

void FKMLLevelSetElem::GetInteriorGridCells(TArray<FBox>& CellBoxes, double InteriorThreshold) const
{
	if (MLLevelSet.IsValid())
	{
		const Chaos::TUniformGrid<Chaos::FReal, 3>& Grid = MLLevelSet->GetGrid();
		const Chaos::TVector<int32, 3> Cells = Grid.Counts();
		for (int i = 0; i < Cells.X; ++i)
		{
			for (int j = 0; j < Cells.Y; ++j)
			{
				for (int k = 0; k < Cells.Z; ++k)
				{
					const double Value = MLLevelSet->GetPhiArray()(i, j, k);
...
void FKMLLevelSetElem::GetZeroIsosurfaceGridCellFaces(TArray<FVector3f>& Vertices, TArray<FIntVector>& Tris) const
{
	if (MLLevelSet.IsValid())
	{
...
///////////////////////////////////////
FBox FKSkinnedTriangleMeshElem::CalcAABB(const FTransform& BoneTM, const FVector& Scale3D) const
{
	FBox Box(ForceInit);
	if (SkinnedTriangleMesh.IsValid())
	{
		Box = FBox(SkinnedTriangleMesh->BoundingBox().Min(), SkinnedTriangleMesh->BoundingBox().Max());
		const FTransform LocalToWorld = FTransform(FQuat::Identity, FVector::ZeroVector, Scale3D) * BoneTM;
		return Box.TransformBy(LocalToWorld);
	}
...
{
	SkinnedTriangleMesh = MoveTemp(InSkinnedTriangleMesh);
}
...
{
	Ar << NNESignedDistanceModelData;
	Ar << NNEIncorrectZoneModelData;

	if (Ar.IsLoading())
	{
		MLLevelSet = TSharedPtr<Chaos::FMLLevelSet>(new Chaos::FMLLevelSet(NNESignedDistanceModelData, NNEIncorrectZoneModelData));
	}

	if (MLLevelSet.IsValid())
	{
		MLLevelSet->Serialize(Ar);
	}
...
bool FKSkinnedTriangleMeshElem::Serialize(FArchive& Ar)
{
	if (Ar.IsLoading())
	{
		SkinnedTriangleMesh = TRefCountPtr<Chaos::FSkinnedTriangleMesh>(new Chaos::FSkinnedTriangleMesh());
	}

	if (SkinnedTriangleMesh.IsValid())
	{
		Chaos::FChaosArchive ChaosAr(Ar);
		SkinnedTriangleMesh->Serialize(ChaosAr);
	}
