...
void FCollisionQueryParams::Internal_AddIgnoredSourceObject(const FWeakObjectPtr& WeakPtr)
{
	if (WeakPtr.IsValid())
	{
#if UE_WITH_REMOTE_OBJECT_HANDLE
		// We only want to add the object to the ignore list if it's owned by the current server. 
		// Even if it's owned, it might currently be remote which would cause a migration upon dereference. 
		// We can grab the object directly, even if it's stale, to get the unique id and avoid a migration. 
		// This unique id will always be valid to use.
		if (UE::RemoteObject::Handle::IsOwned(WeakPtr.GetRemoteId()))
		{
			constexpr bool bEvenIfGarbage = true;
			constexpr bool bOnGameThread = true;
			check(WeakPtr.IsValid(bEvenIfGarbage, bOnGameThread)); // this should always be true if the object is owned

			// This is the UObject pointer, even if it's stale (e.g. contains some invalid data)
			const UObject* DoNotMutateLocalCopy = StaticFindObjectFastInternal(WeakPtr.GetRemoteId());

			// This is the array index in GUObjectArray which is the same as its UniqueId.
			const int32 UniqueId = GUObjectArray.ObjectToIndex(DoNotMutateLocalCopy);
			AddIgnoredSourceObject(UniqueId);
		}
		IgnoreSourceObjectReferences.Add(FRemoteObjectReference(WeakPtr));
#else
		UObject * Obj = WeakPtr.Get();
		// On rare occasions, this is null even with the above `IsValid` check
		if (Obj != nullptr)
		{
			AddIgnoredSourceObject(Obj->GetUniqueID());
		}
#endif
	}
}
...
void FCollisionQueryParams::AddIgnoredActors(const TArray<TWeakObjectPtr<const AActor> >& InIgnoreActors)
{
	for (int32 Idx = 0; Idx < InIgnoreActors.Num(); ++Idx)
	{
		Internal_AddIgnoredSourceObject(static_cast<FWeakObjectPtr>(InIgnoreActors[Idx]));
	}
}
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
	UObject* Obj = (UObject*)GUObjectArray.IndexToObject(InIgnoreActorID)->GetObject();
	IgnoreSourceObjectReferences.Add(FRemoteObjectReference(Obj));
#endif
}
...
void FCollisionQueryParams::AddIgnoredSourceObject(const UObject* InIgnoreObject)
{
	if (InIgnoreObject)
	{
		AddIgnoredSourceObject(InIgnoreObject->GetUniqueID());
#if UE_WITH_REMOTE_OBJECT_HANDLE
		IgnoreSourceObjectReferences.Add(FRemoteObjectReference(InIgnoreObject));
#endif
	}
}
...
{
	Internal_AddIgnoredSourceObject(static_cast<FWeakObjectPtr>(InIgnoreObject));
}

void FCollisionQueryParams::AddIgnoredSourceObject(const FRemoteObjectReference& ObjectRef)
{
	Internal_AddIgnoredSourceObject(ObjectRef.ToWeakPtr());
}
...
FORCEINLINE_DEBUGGABLE void FCollisionQueryParams::Internal_AddIgnoredComponent(const UPrimitiveComponent* InIgnoreComponent)
{
	...
	{
		Internal_AddIgnoredComponent(InIgnoreComponent->GetUniqueID());
	}
}
...
void FCollisionQueryParams::AddIgnoredComponent(const UPrimitiveComponent* InIgnoreComponent)
{
	...
	{
		IgnoreComponentReferences.Add(FRemoteObjectReference(InIgnoreComponent));
	}
#endif
}
...
{
	if (InIgnoreComponent.IsValid())
	{
		bool bIsValidIgnore = true;

#if UE_WITH_REMOTE_OBJECT_HANDLE
		FRemoteObjectReference RemoteObjectRef(InIgnoreComponent);
		IgnoreComponentReferences.Add(RemoteObjectRef);
		// Only ignore objects that belong to this server's physics world (only true if owned).
		bIsValidIgnore = UE::RemoteObject::Handle::IsOwned(RemoteObjectRef.GetRemoteId());
#endif

		if (bIsValidIgnore)
		{
			Internal_AddIgnoredComponent(InIgnoreComponent.Get());
		}
	}
}
...
{
	FWeakObjectPtr WeakPtr = InComponentRef.ToWeakPtr();
	if (WeakPtr.IsValid())
	{
		bool bIsValidIgnore = true;
#if UE_WITH_REMOTE_OBJECT_HANDLE
		IgnoreComponentReferences.Add(InComponentRef);
		// Only ignore objects that belong to this server's physics world (only true if owned).
		bIsValidIgnore = UE::RemoteObject::Handle::IsOwned(InComponentRef.GetRemoteId());
#endif
		if (bIsValidIgnore)
		{
			UObject* Obj = WeakPtr.Get();
			...
			if (Obj != nullptr)
			{
				UPrimitiveComponent* PrimitiveComponent = Cast<UPrimitiveComponent>(Obj);
				check(PrimitiveComponent);
				Internal_AddIgnoredComponent(PrimitiveComponent);
			}
		}
	}
}
