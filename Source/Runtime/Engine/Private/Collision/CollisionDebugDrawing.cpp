...
void DrawGeomOverlaps(const UWorld* InWorld, const Chaos::FImplicitObject& Geom, const FTransform& GeomPose, TArray<struct FOverlapResult>& Overlaps, float Lifetime)
{
	...
	else if (const auto Sphere = Geom.template GetObject<Chaos::FSphere>())
	{
		DrawSphereOverlap(InWorld, Pos, Sphere->GetRadiusf(), Overlaps, Lifetime);
	}
	else if (const auto Capsule = Geom.template GetObject<FCapsule>())
	{
		...
		DrawCapsuleOverlap(InWorld, Pos, Capsule->GetHeightf() * 0.5f + Capsule->GetRadiusf(), Capsule->GetRadiusf(), Rot, Overlaps, Lifetime);
	}
}
...
void DrawGeomSweeps(const UWorld* InWorld, const FVector& Start, const FVector& End, const Chaos::FImplicitObject& Geom, const FQuat& Rotation, const TArray<FHitResult>& Hits, float Lifetime)
{
	...
	else if (const Chaos::FSphere* Sphere = Geom.template GetObject<Chaos::FSphere>())
	{
		DrawSphereSweeps(InWorld, Start, End, Sphere->GetRadiusf(), Hits, Lifetime);
	}
	else if (const auto Capsule = Geom.template GetObject<FCapsule>())
	{
		// Convert here from Chaos to unreal definition of capsule height
		DrawCapsuleSweeps(InWorld, Start, End, Capsule->GetHeightf() * 0.5f + Capsule->GetRadiusf(), Capsule->GetRadiusf(), Rotation, Hits, Lifetime);
	}
}
