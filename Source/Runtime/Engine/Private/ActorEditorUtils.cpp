...
namespace FActorEditorUtils
{
	...
	{
		return ValidateActorLabel(FStringView(InName.ToString()), OutErrorMessage);
	}
	...
	bool ValidateActorLabel(FStringView InName, FText& OutErrorMessage)
	{
		...
		if (InName.Len() == 0)
		{
			OutErrorMessage = LOCTEXT("ValidateActorNameString_Empty", "Actor names cannot be empty");
			return false;
		}

		if (InName.Len() >= NAME_SIZE)
		{
			OutErrorMessage = FText::Format(LOCTEXT("ValidateActorNameString_TooLong", "Names must be less than {0} characters long."), NAME_SIZE);
			return false;
		}

		if (InName.Compare("None", ESearchCase::IgnoreCase) == 0)
		{
			OutErrorMessage = LOCTEXT("ValidateActorNameString_ReservedNameNone", "\"None\" is a reserved term and cannot be used for actor names");
			return false;
		}
