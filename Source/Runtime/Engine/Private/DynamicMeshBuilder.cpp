...
public:
	...
	FBufferRHIRef CreateResource(FRHICommandListBase& RHICmdList, FGlobalDynamicMeshPoolPolicy::CreationArguments Args)
	{
		...
		const FRHIBufferCreateDesc CreateDesc =
			FRHIBufferCreateDesc::CreateIndex(TEXT("FGlobalDynamicMeshIndexPolicy"), BufferSize, sizeof(DynamicMeshIndexType))
...
public:
	...
	FBufferRHIRef CreateResource(FRHICommandListBase& RHICmdList, FGlobalDynamicMeshPoolPolicy::CreationArguments Args)
	{
		...
		const FRHIBufferCreateDesc CreateDesc =
			FRHIBufferCreateDesc::CreateVertex(TEXT("FGlobalDynamicMeshVertexPolicy"), BufferSize)
...
FDynamicMeshIndexBuffer32::FDynamicMeshIndexBuffer32() = default;
FDynamicMeshIndexBuffer32::~FDynamicMeshIndexBuffer32() = default;

void FDynamicMeshIndexBuffer32::InitRHI(FRHICommandListBase& RHICmdList)
{
	IndexBufferRHI = UE::RHIResourceUtils::CreateIndexBufferFromArray(RHICmdList, TEXT("FDynamicMeshIndexBuffer32"), UsageFlags, MakeConstArrayView(Indices));
}

FDynamicMeshIndexBuffer16::FDynamicMeshIndexBuffer16() = default;
FDynamicMeshIndexBuffer16::~FDynamicMeshIndexBuffer16() = default;

void FDynamicMeshIndexBuffer16::InitRHI(FRHICommandListBase& RHICmdList)
{
	IndexBufferRHI = UE::RHIResourceUtils::CreateIndexBufferFromArray(RHICmdList, TEXT("FDynamicMeshIndexBuffer16"), UsageFlags, MakeConstArrayView(Indices));
}
...
FBufferRHIRef FDynamicMeshBufferAllocator::AllocIndexBuffer(FRHICommandListBase& RHICmdList, uint32 NumElements)
{
	...
	const FRHIBufferCreateDesc CreateDesc =
		FRHIBufferCreateDesc::CreateIndex(TEXT("FDynamicMeshBufferAllocator"), SizeInBytes, sizeof(DynamicMeshIndexType))
...
FBufferRHIRef FDynamicMeshBufferAllocator::AllocVertexBuffer(FRHICommandListBase& RHICmdList, uint32 Stride, uint32 NumElements)
{
	...
	const FRHIBufferCreateDesc CreateDesc =
		FRHIBufferCreateDesc::CreateVertex(TEXT("FDynamicMeshBufferAllocator"), SizeInBytes)
		.AddUsage(EBufferUsageFlags::Volatile | EBufferUsageFlags::ShaderResource)
		.DetermineInitialState();
...
class FPooledDynamicMeshBufferAllocator : public FDynamicMeshBufferAllocator
{
	...
	virtual void ReleaseIndexBuffer(FBufferRHIRef& IndexBufferRHI)
	{
		if (IsValidRef(IndexBufferRHI))
		{
			...
			{
				GDynamicMeshIndexPool.ReleasePooledResource(MoveTemp(IndexBufferRHI));
			}
	...
	virtual void ReleaseVertexBuffer(FBufferRHIRef& VertexBufferRHI)
	{
		if (IsValidRef(VertexBufferRHI))
		{
			...
			{
				GDynamicMeshVertexPool.ReleasePooledResource(MoveTemp(VertexBufferRHI));
			}
		...
		if (VertexBuffer || IndexBuffer)
		{
			...
			FPrimitiveUniformShaderParameters PrimitiveParams = FPrimitiveUniformShaderParametersBuilder{}
				.Defaults()
					...
					.OutputVelocity(true)
					.IsFirstPerson(Settings.bIsFirstPerson)
				.Build();
