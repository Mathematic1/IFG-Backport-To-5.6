		...
		else
		{
			...
			if (const TCHAR* NewBuffer1 = FPropertyHelpers::ReadToken(Buffer, Token))
			{
				...
				if (Buffer[0] == TEXT(':'))
				{
					...
					OnlineSubsystemName = FName(*Token);
					if (const TCHAR* NewBuffer2 = FPropertyHelpers::ReadToken(Buffer, IdContents))
					{
						Buffer = NewBuffer2;
					}
				}
				...
				if (IdContents != InvalidUniqueNetIdStr)
				{
					// Log the subsystem name used once
					if (!UOnlineEngineInterface::Get()->IsLoaded(OnlineSubsystemName))
					{
						static bool bLogged = false;
						UE_CLOG(!bLogged, LogNet, Log, TEXT("Attempted to ImportText to FUniqueNetIdRepl while OSS is not loaded. OSS:%s Parent:%s"), *OnlineSubsystemName.ToString(), *GetPathNameSafe(Parent));
						bLogged = true;
					}

					UniqueIdFromString(OnlineSubsystemName, IdContents);
				}
				else
				{
					bShouldWarn = false;
