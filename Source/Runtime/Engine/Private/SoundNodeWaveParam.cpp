	...
	if (USoundWave* NewWave = Cast<USoundWave>(ParamValue.ObjectParam))
	{
		...
		if (PrevWave != NewWave)
		{
			if (FSoundCueParameterTransmitter* SoundCueTransmitter = static_cast<FSoundCueParameterTransmitter*>(ActiveSound.GetTransmitter()))
			{
				...
				SoundCueTransmitter->RemoveChildTransmitter(GetNodeWaveInstanceHash(NodeWaveInstanceHash, (UPTRINT)PrevWave, 0));
				...
				SoundCueTransmitter->CreateChildTransmitter(WaveHash, *NewWave, MoveTemp(Params));
			}
