...
static FAutoConsoleVariableRef CVarShowRuntimeHashSetDebugDisplayMode(
	...
	TEXT("Used to choose what mode to display when showing runtime partitions (0=Level Streaming State, 1=Data Layers, 2=Content Bundles, 3=Level Streaming State Without Data Layers/Content Bundles)."));
...
static bool IsDebugCellShown(UWorldPartitionRuntimeCell* Cell)
{
	UWorldPartitionRuntimeCellData* RuntimeCellData = Cell->RuntimeCellData;
	return ((RuntimeCellData->HierarchicalLevel >= GShowRuntimeHashSetDebugDisplayLevel) &&
		(RuntimeCellData->HierarchicalLevel < (GShowRuntimeHashSetDebugDisplayLevel + GShowRuntimeHashSetDebugDisplayLevelCount)) &&
		(GShowRuntimeHashSetDebugDisplayMode != 3 || (!Cell->GetDataLayers().Num() && !Cell->GetContentBundleID().IsValid())) &&
		Cell->IsDebugShown());
}
...
void UWorldPartitionRuntimeHashSet::Draw3D(const TArray<FWorldPartitionStreamingSource>& Sources) const
{
	...
	auto DebugDrawCell3D = [WorldPartitionTransform, OwningWorld, ContentBundleManager, VisualizeMode, &DataLayerDebugColors](UWorldPartitionRuntimeCell* Cell)
	{
		if (!IsDebugCellShown(Cell))
		{
			return;
		...
		DrawDebugBox(OwningWorld, CellPos, Box.GetExtent(), WorldPartitionTransform.GetRotation(), BoxColor.WithAlpha(255), false, -1.f, 0, 20.f);
	};

	ForEachStreamingData([this, &Sources, VisualizeMode, &DataLayerDebugColors, ContentBundleManager, OwningWorld, &WorldPartitionTransform, &DebugDrawCell3D](const FRuntimePartitionStreamingData& StreamingData)
	{
		if (StreamingData.SpatialIndex)
		{
			for (const FWorldPartitionStreamingSource& Source : Sources)
			{
				Source.ForEachShape(StreamingData.GetLoadingRange(), StreamingData.Name, false, [this, &StreamingData, VisualizeMode, &DataLayerDebugColors, ContentBundleManager, OwningWorld, &WorldPartitionTransform, &DebugDrawCell3D](const FSphericalSector& Shape)
				{
					auto ForEachIntersectingElement = [&]<typename ShapeType>(const ShapeType& InShape)
					{
						StreamingData.SpatialIndex.Get()->ForEachIntersectingElement(InShape, [this, VisualizeMode, &DataLayerDebugColors, ContentBundleManager, OwningWorld, &WorldPartitionTransform, &DebugDrawCell3D](UWorldPartitionRuntimeCell* Cell)
						{
							DebugDrawCell3D(Cell);
						});
					};
		...
		if (StreamingData.SpatialIndex2D.IsValid())
		{
			for (const FWorldPartitionStreamingSource& Source : Sources)
			{
				const bool bCalcBoundsIn2D = true;
				const FBox SourceBounds = Source.CalcBounds(StreamingData.GetLoadingRange(), StreamingData.Name, bCalcBoundsIn2D);
				const FBox2D SourceBounds2D(FVector2D(SourceBounds.Min.X, SourceBounds.Min.Y), FVector2D(SourceBounds.Max.X, SourceBounds.Max.Y));
				StreamingData.SpatialIndex2D->ForEachIntersectingElement(SourceBounds2D, [&DebugDrawCell3D](UWorldPartitionRuntimeCell* Cell)
				{
					DebugDrawCell3D(Cell);
				});
			}
