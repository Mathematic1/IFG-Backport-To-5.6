...
namespace
{
	...
	static FWorldPartitionHLODDestructionTag ResolveHLODDestructionTagForActor(AActor* InActor)
	{
		...
		if (RuntimeCell)
		{
			for (IWorldPartitionHLODObject* HLODObject : InActor->GetWorld()->GetSubsystem<UWorldPartitionHLODRuntimeSubsystem>()->GetHLODObjectsForCell(RuntimeCell))
			{
				// This is the only use of IWorldPartitionHLODObject::GetUObject() for now... Once we solve destruction for FastGeo HLODs, we will be able to remove it.
				if (AWorldPartitionHLOD* HLODActor = Cast<AWorldPartitionHLOD>(HLODObject->GetUObject()))
				{
