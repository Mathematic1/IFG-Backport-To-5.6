...
FString UWorldPartitionLevelStreamingPolicy::GetCellPackagePath(const FName& InCellName, const UWorld* InWorld)
{
	if (InWorld->IsGameWorld())
	{
		if (InWorld->IsPlayInEditor() || InWorld->IsPlayInPreview())
		{
			...
			return FString::Printf(TEXT("/Memory/%s_%s"), *FPackageName::GetShortName(InWorld->GetPackage()), *InCellName.ToString());
		}
...
void UWorldPartitionLevelStreamingPolicy::PrepareActorToCellRemapping()
{
	...
	//	- Spatially loaded or Datalayer Actors from the main World Partition map that get moved into
	//		a Streaming Cell. In this case, an actor path like '/Game/SomePath/WorldName.WorldName:PersistentLevel.ActorA'
	//		would be mapped to a cell name like 'WorldName_MainGrid_L0_X5_Y-4'.
	//	- Always loaded Actors from the main World:
	//		- In PIE they get remapped to the top level Cell 'WorldName_MainGrid_L{MAX}_X0_Y0'.
	//		- In Cook they don't need remapping as the top level Cell is
	//			the PersistentLevel (Cell->NeedsActorToCellRemapping() returns false).
	WorldPartition->RuntimeHash->ForEachStreamingCells([this, &SourceWorldPath](const UWorldPartitionRuntimeCell* Cell)
	{
		...
		if (Cell->NeedsActorToCellRemapping())
		{
			...
			{
				// Avoid remapping from any editor only references of actors that the cells may have. 
				if (!CellObjectMap.bIsEditorOnly)
				{
...
bool UWorldPartitionLevelStreamingPolicy::ConvertEditorPathToRuntimePath(const FSoftObjectPath& InPath, FSoftObjectPath& OutPath) const
{
	...
	if (SrcObjectPath.GetSubPathString().Split(TEXT("."), &SubObjectContext, &SubObjectString))
	{
		if (SubObjectContext == TEXT("PersistentLevel"))
		{
			...
			UObject* OutCellOuter = nullptr;
			const FName* CellName = FindCellNameForSubObject(SubObjectString, true, OutSubObjectString, OutLevelMountPointContext, OutCellOuter);
...
UObject* UWorldPartitionLevelStreamingPolicy::GetSubObject(const TCHAR* SubObjectPath)
{
	...
	const FString SrcPath = UWorld::RemovePIEPrefix(SubObjectPath);

	FString OutSubObjectString;
	const UObject* OutLevelMountPointContext = nullptr;
	UObject* OutCellOuter = nullptr;
	if (const FName* CellName = FindCellNameForSubObject(SrcPath, /*bInResolveContainers=*/true, OutSubObjectString, OutLevelMountPointContext, OutCellOuter))
	{
		if (const UWorldPartitionRuntimeLevelStreamingCell* Cell = (UWorldPartitionRuntimeLevelStreamingCell*)StaticFindObject(UWorldPartitionRuntimeLevelStreamingCell::StaticClass(), OutCellOuter, *(CellName->ToString())))
		{
			if (UWorldPartitionLevelStreamingDynamic* LevelStreaming = Cell->GetLevelStreaming())
			{
				if (LevelStreaming->GetLoadedLevel())
				{
					return StaticFindObject(UObject::StaticClass(), LevelStreaming->GetLoadedLevel(), *OutSubObjectString);
				}
			}
		}
	}
...
bool UWorldPartitionLevelStreamingPolicy::RemoveExternalStreamingObject(URuntimeHashExternalStreamingObjectBase* ExternalStreamingObject)
{
	...
	for (const TPair<FName, FName> Pair : ExternalStreamingObject->SubObjectsToCellRemapping)
	{
		if (TWeakObjectPtr<URuntimeHashExternalStreamingObjectBase>* RemovedExternalStreamingObjectPtr = SubObjectsToExternalStreamingObjectsRemapping.Find(Pair.Key))
		{
			TWeakObjectPtr<URuntimeHashExternalStreamingObjectBase> RemovedExternalStreamingObject = *RemovedExternalStreamingObjectPtr;
			UE_CLOG(RemovedExternalStreamingObject != ExternalStreamingObject, LogWorldPartition, Log, TEXT("Mismatch in SubObjectsToExternalStreamingObjectsRemapping map. Found: %s. Expected: %s"),
				(RemovedExternalStreamingObject.IsValid() && RemovedExternalStreamingObject->GetPackage()) ? *RemovedExternalStreamingObject->GetPackage()->GetName() : TEXT("<Invalid>"),
				ExternalStreamingObject->GetPackage() ? *ExternalStreamingObject->GetPackage()->GetName() : TEXT("<Invalid>"));
			SubObjectsToExternalStreamingObjectsRemapping.Remove(Pair.Key);
		}
		else
		{
			UE_LOG(LogWorldPartition, Log, TEXT("Mismatch in SubObjectsToExternalStreamingObjectsRemapping map. Subobject %s not found"), *Pair.Key.ToString());
		}
	}
...
const FName* UWorldPartitionLevelStreamingPolicy::FindCellNameForSubObject(const FString& InSubObjectString, bool bInResolveContainers, FString& OutSubPathString, const UObject*& OutLevelMountPointContext, UObject*& OutCellOuter) const
{
	OutLevelMountPointContext = nullptr;
	OutCellOuter = nullptr;
	...
	{
		OutCellOuter = GetOuterUWorldPartition()->RuntimeHash;
		return CellName;
	}
	...
	if (const TWeakObjectPtr<URuntimeHashExternalStreamingObjectBase>* ExternalStreamingObject = SubObjectsToExternalStreamingObjectsRemapping.Find(*SubObjectContext); ExternalStreamingObject && ExternalStreamingObject->IsValid())
	{
		...
		if (const FName* CellName = ExternalStreamingObjectPtr->SubObjectsToCellRemapping.Find(*SubObjectContext))
		{
			...
			OutCellOuter = ExternalStreamingObject->Get();
			return CellName;
		}
	}
	...
	if (bInResolveContainers)
	{
		...
		{
			return FindCellNameForSubObject(OutResolvedSubPathString, false, OutSubPathString, OutLevelMountPointContext, OutCellOuter);
		}
