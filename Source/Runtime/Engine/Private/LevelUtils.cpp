...
void FLevelUtils::SetEditorTransform(ULevelStreaming* StreamingLevel, const FTransform& Transform, bool bDoPostEditMove )
{
	...
	bool bNeedsToRebuildLevelBSP = false;
	if (ULevel* LoadedLevel = StreamingLevel->GetLoadedLevel())
	{
		for (AActor* Actor : LoadedLevel->Actors)
		{
			...
			// Iterate over all actors in the level to see if we need to rebuild the Level BSP(i.e., moving any Brush actor affecting BSP)
			if (!bNeedsToRebuildLevelBSP && GEditor)
			{
				if (ABrush* Brush = Cast<ABrush>(Actor))
				{
					bNeedsToRebuildLevelBSP = Brush->CanEverAffectBSP();
				}
			}
		}
	}
	...
	if (bNeedsToRebuildLevelBSP)
	{
		check(GEditor);
		check(StreamingLevel->GetLoadedLevel());

		// RebuildAlteredBSP() only tracks the levels of currently selected actors if they are brushes
		// which is not applicable when we are moving the whole level
		// So we need to build the specific level BSP
		GEditor->RebuildLevel(*StreamingLevel->GetLoadedLevel());
	}
