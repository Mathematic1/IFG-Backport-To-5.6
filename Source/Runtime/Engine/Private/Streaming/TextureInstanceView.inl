...
void FBounds4::OffsetBounds(int32 Index, const FVector& Offset)
{
	check(Index >= 0 && Index < 4);
...
void FBounds4::UpdateLastRenderTime(int32 Index, float InLastRenderTime)
{
	check(Index >= 0 && Index < 4);
...
void FBounds4::UpdateMaxDrawDistanceSquared(int32 Index, float InMaxRangeSq)
{
	check(Index >= 0 && Index < 4);
