...
{
	const int32 MipOffset = !!(MipAllocator->GetIntermediateTextureRHI()->GetFlags() & TexCreate_Virtual) ? 0 : PendingFirstLODIdx;
	const int32 NumMips = CurrentFirstLODIdx - PendingFirstLODIdx;

	for (TUniquePtr<FTextureMipDataProvider>& MipDataProvider : MipDataProviders)
	{
		check(MipDataProvider);
		if (MipDataProvider->GetNextTickState() == FTextureMipDataProvider::ETickState::PostprocessGPUMipData
			&& IsSameThread(MipDataProvider->GetNextTickThread(), Context.CurrentThread))
		{
			if (!MipDataProvider->PostprocessGPUMipData(Context, MipAllocator->GetIntermediateTextureRHI(), PendingFirstLODIdx - MipOffset, NumMips, SyncOptions))
			{
				return false;
			}
		}
...
void FTextureStreamIn::PollMipData(const FContext& Context)
{
	...
	else
	{
		...
		if (!bCancelled)
		{
			...
			else // Allocator does not specificy an UploadMips thread. This is a required operation before finalizing the texture.
			{
				MarkAsCancelled();
				bCancelled = true;
			}
...
{
	DECLARE_SCOPE_CYCLE_COUNTER(TEXT("FTextureStreamIn::UploadNewMips"), STAT_TextureStreamIn_UploadNewMips, STATGROUP_StreamingDetails);
	...
	else if (!IsCancelled())
	{
		...
		NextThread = GetMipDataProviderThread(FTextureMipDataProvider::ETickState::PostprocessGPUMipData);
		// All mips must be handled before moving to next stage.
		if (NextThread != TT_None)
		{
			bIsPostprocessingGPUMipData = true;
			PushTask(Context, NextThread, SRA_UPDATE_CALLBACK(PostprocessGPUMipData), GetCancelThread(), SRA_UPDATE_CALLBACK(Cancel));
		}
		else // Otherwise nothing to execute but schedule next phase (edge case).
		{
			PostprocessGPUMipData(Context);
		}
	}
	else
	{
...
{
	DECLARE_SCOPE_CYCLE_COUNTER(TEXT("FTextureStreamIn::PostprocessGPUMipData"), STAT_TextureStreamIn_PostprocessGPUMipData, STATGROUP_StreamingDetails);

	// Execute
	if (!DoPostprocessGPUMipData(Context))
	{
		MarkAsCancelled();
	}

	// Schedule the next update step.
	EThreadType NextThread = GetMipDataProviderThread(FTextureMipDataProvider::ETickState::PostprocessGPUMipData);
	...
	{
		bIsPostprocessingGPUMipData = false;
		// The finalize task should always fire on the render thread
...
void FTextureStreamIn::FinalizeNewMips(const FContext& Context)
{
	...
	{
		// Mark as successfully finished even if canceled to keep the CachedSRRState in sync with the RHI texture.
...
void FTextureStreamIn::Abort()
{
	...
	if (bIsPollingMipData || bIsPostprocessingGPUMipData)
	{
		...
		(new FAsyncAbortPollMipsTask(this, bIsPollingMipData, bIsPostprocessingGPUMipData))->StartBackgroundTask();
	}
}
...
void FTextureStreamIn::FAbortPollMipsTask::DoWork()
{
	...
	for (TUniquePtr<FTextureMipDataProvider>& MipDataProvider : PendingUpdate->MipDataProviders)
	{
		...
		else if (bIsPostprocessingGPUMipData)
		{
			MipDataProvider->AbortPostprocessGPUMipData();
		}
	}
