...
EAddComponentResult FStaticRenderAssetInstanceManager::Add(const TArray<FRenderAssetInstanceState::FPreAddComponentPayload>& Payloads, TFunctionRef<void(const FRenderAssetInstanceState::FPreAddComponentPayload&, EAddComponentResult)> OnAddFailedFunc)
{
	if (!AsyncView)
	{
		FRenderAssetInstanceState* State = StateSync.SyncAndGetState();
		return State->AddComponents(Payloads, OnAddFailedFunc);
	}
