...
public:
	...
	// Route FTextureMipAllocator::UploadMips()
	void UploadNewMips(const FContext& Context);

	// Route FTextureMipDataProvider::PostprocessGPUMipData
...
private:
	...
	{
	public:
		FAbortPollMipsTask(FTextureStreamIn* InPendingUpdate, bool bInIsPollingMipData, bool bInIsPostprocessingGPUMipData) 
			: PendingUpdate(InPendingUpdate)
			, bIsPollingMipData(bInIsPollingMipData)
			, bIsPostprocessingGPUMipData(bInIsPostprocessingGPUMipData)
		{
		}
		void DoWork();
	...
	protected:
		...
		bool bIsPollingMipData;
		bool bIsPostprocessingGPUMipData;
	};

	bool bIsPollingMipData = false; // Used to prevent indirecting any pointer in the abort task.
	bool bIsPostprocessingGPUMipData = false; // Used to prevent indirecting any pointer in the abort task.
