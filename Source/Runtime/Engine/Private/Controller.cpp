...
void AController::OnPossess(APawn* InPawn)
{
	...
	{
		UE_CLOG(InPawn->GetController() == this, LogController, Warning, TEXT("Asking %s to possess pawn %s more than once; pawn will be restarted! Should call Unpossess first."), *GetNameSafe(this), *GetNameSafe(InPawn));
