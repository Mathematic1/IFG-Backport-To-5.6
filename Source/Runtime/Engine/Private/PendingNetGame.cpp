...
void UPendingNetGame::InitNetDriver(UNetDriver* InNetDriver)
{
	...
	if (!GDisallowNetworkTravel)
	{
		...
		{
			ensureMsgf(InNetDriver->ServerConnection == nullptr, TEXT("UPendingNetGame: Provided NetDriver is already connected/connecting to a remote server."));
			ensureMsgf(!InNetDriver->ConnectionlessHandler.IsValid(), TEXT("UPendingNetGame: Provided NetDriver is already listening for incoming connections."));
...
void UPendingNetGame::FinishDestroy()
{
	ensureMsgf(NetDriver == nullptr, TEXT("PendingNetGame's NetDriver was not shutdown before destruction."));
...
{
	SendJoinWithFlags(UE::Net::EJoinFlags::None);
}

void UPendingNetGame::SendJoinWithFlags(UE::Net::EJoinFlags Flags)
{
	bSentJoinRequest = true;

	if (EnumHasAllFlags(Flags, UE::Net::EJoinFlags::NoPawn))
	{
		FNetControlMessage<NMT_JoinNoPawn>::Send(NetDriver->ServerConnection);
...
ULocalPlayer* UPendingNetGame::GetFirstGamePlayer()
{
	return GEngine->GetFirstGamePlayer(this);
}