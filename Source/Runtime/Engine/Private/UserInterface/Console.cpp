...
void UConsole::UpdateCompleteIndices()
{
	...
	else if (!TypedStr.IsEmpty())
	{
		...
		AutoComplete.Sort([this](const FAutoCompleteCommand& A, const FAutoCompleteCommand& B)
			{
				// Give preference to commands that actually begin with what the user's typed so far
				if (A.Command.StartsWith(TypedStr))
				{
					if (!B.Command.StartsWith(TypedStr))
					{
						return true;
					}
				}
...
void UConsole::OutputTextLine(const FString& Text)
{
	...
	{
		Scrollback.RemoveAt(0, Scrollback.Num() - ConsoleSettings->MaxScrollbackSize + 1);
		SBHead = ConsoleSettings->MaxScrollbackSize;
	}
	else
	{
		SBHead++;
...
void UConsole::PostRender_Console_Open(UCanvas* Canvas)
{
	...
	PostRender_InputLine(Canvas, FIntPoint(PosLeft, PosTop + Height + 6 + TextH));
}
...
void UConsole::BeginState_Open(FName PreviousStateName)
{
	...
	PreviousMouseCaptureMode = GetOuterUGameViewportClient()->GetMouseCaptureMode();
...
{
	// Restore mouse capture for non-open states (only if nothing else modified it while the console was open otherwise, let's keep the new mode)
	if (GetOuterUGameViewportClient()->GetMouseCaptureMode() == EMouseCaptureMode::CaptureDuringMouseDown)
	{
		GetOuterUGameViewportClient()->SetMouseCaptureMode(PreviousMouseCaptureMode);
	}
}
