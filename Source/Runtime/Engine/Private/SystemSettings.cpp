...
static inline const TCHAR* GetSectionName(bool bIsEditor)
{
	...
	if (FParse::Value(FCommandLine::Get(), TEXT("-SystemSettings="), OverrideSubName))
	{
		...
		{
			FCString::Strncpy(ReturnedOverrideName + SystemSettingsStrLen, (*OverrideSubName + SystemSettingsStrLen),
				UE_ARRAY_COUNT(ReturnedOverrideName) - SystemSettingsStrLen);
		}
		else
		{
			FCString::Strncpy(ReturnedOverrideName + SystemSettingsStrLen, *OverrideSubName,
				UE_ARRAY_COUNT(ReturnedOverrideName) - SystemSettingsStrLen);
		}
