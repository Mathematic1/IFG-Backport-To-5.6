...
void FGestureRecognizer::HandleGesture(UPlayerInput* PlayerInput, FKey Gesture, bool bStarted, bool bEnded)
{
	...
	if (Value)
	{
		const ULocalPlayer* OwningLP = PlayerInput->GetOwningLocalPlayer();
		const FInputDeviceId DefaultDevice = IPlatformInputDeviceMapper::Get().GetPrimaryInputDeviceForUser(OwningLP ? OwningLP->GetPlatformUserId() : PLATFORMUSERID_NONE);
		...
		PlayerInput->InputGesture(DefaultDevice, Gesture, GestureEvent, *Value, CurrentTimestamp);
