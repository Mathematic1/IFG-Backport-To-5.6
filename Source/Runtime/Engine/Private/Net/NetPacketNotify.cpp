...
namespace 
{
	struct FPackedHeader
	{
		...
		static_assert(FNetPacketNotify::SequenceHistoryT::WordCount <= (1 << HistoryWordCountBits), "Sequence history word count (minus one) must fit within HistoryWordCountBits. Check FNetPacketNotify::MaxSequenceHistoryLength.");
...
bool FNetPacketNotify::WriteHeader(FBitWriter& Writer, bool bRefresh)
{
	...
	UE_LOG_PACKET_NOTIFY(TEXT("FNetPacketNotify::WriteHeader - Seq %u, AckedSeq %u bReFresh %u HistorySizeInWords %" SIZE_T_FMT), Seq, AckedSeq, bRefresh ? 1u : 0u, WrittenHistoryWordCount);
...
bool FNetPacketNotify::ReadHeader(FNotificationHeader& Data, FBitReader& Reader) const
{
	...
	UE_LOG_PACKET_NOTIFY(TEXT("FNetPacketNotify::ReadHeader - Seq %u, AckedSeq %u HistorySizeInWords %" SIZE_T_FMT), Data.Seq.Get(), Data.AckedSeq.Get(), Data.HistoryWordCount);
