...
static bool GbSortDebugOffsets = false;
static bool GbSortDebug = false;
static FAutoConsoleVariableRef CVarDebugOffsets(TEXT("GPUSort.DebugOffsets"), GbSortDebugOffsets, TEXT("Debug GPU sort offsets."));
static FAutoConsoleVariableRef CVarDebugSort(TEXT("GPUSort.DebugSort"), GbSortDebug, TEXT("Debug GPU sorting."));
...
public:
	...
	virtual void InitRHI(FRHICommandListBase& RHICmdList) override
	{
		...
		const FRHIBufferCreateDesc CreateDesc =
			FRHIBufferCreateDesc::CreateVertex(TEXT("SortOffset"), OffsetsBufferSize)
		...
		{
			Buffers[BufferIndex] = RHICmdList.CreateBuffer(CreateDesc);
...
public:
	...
	{
		const FRHIBufferCreateDesc CreateDesc =
			FRHIBufferCreateDesc::CreateVertex(TEXT("FRadixSortParametersBuffer"), sizeof(FRadixSortParameters))
			.AddUsage(EBufferUsageFlags::Volatile | EBufferUsageFlags::ShaderResource)
			.DetermineInitialState();

		SortParametersBufferRHI = RHICmdList.CreateBuffer(CreateDesc);
		SortParametersBufferSRV = RHICmdList.CreateShaderResourceView(
				SortParametersBufferRHI,
				FRHIViewDesc::CreateBufferSRV()
					.SetType(FRHIViewDesc::EBufferType::Typed)
					.SetFormat(PF_R32_UINT));
	}
...
int32 SortGPUBuffers(FRHICommandList& RHICmdList, FGPUSortBuffers SortBuffers, int32 BufferIndex, uint32 KeyMask, int32 Count, ERHIFeatureLevel::Type FeatureLevel)
{
	...
	const bool bDebugOffsets = GbSortDebugOffsets;
	const bool bDebugSort = GbSortDebug;
...
static bool RunGPUSortTest(FRHICommandListImmediate& RHICmdList, int32 TestSize, ERHIFeatureLevel::Type FeatureLevel)
{
	...
	for ( int32 i=0; i < UE_ARRAY_COUNT(KeysBufferRHI); ++i )
	{
		const FRHIBufferCreateDesc KeysCreateDesc =
			FRHIBufferCreateDesc::CreateVertex(TEXT("KeysBuffer"), BufferSize)
			.AddUsage(EBufferUsageFlags::Static | EBufferUsageFlags::ShaderResource | EBufferUsageFlags::UnorderedAccess)
			.SetInitActionInitializer()
			.DetermineInitialState();

		FRHIBufferInitializer Initializer = RHICmdList.CreateBufferInitializer(KeysCreateDesc);
		Initializer.WriteData(Keys.GetData(), BufferSize);
		KeysBufferRHI[i] = Initializer.Finalize();
	}

	for (int32 i = 0; i < UE_ARRAY_COUNT(ValuesBufferRHI); ++i)
	{
		const FRHIBufferCreateDesc ValuesCreateDesc =
			FRHIBufferCreateDesc::CreateVertex(TEXT("ValuesBuffer"), BufferSize)
			.AddUsage(EBufferUsageFlags::Static | EBufferUsageFlags::ShaderResource | EBufferUsageFlags::UnorderedAccess)
			.SetInitActionInitializer()
			.DetermineInitialState();

		FRHIBufferInitializer Initializer = RHICmdList.CreateBufferInitializer(ValuesCreateDesc);
		Initializer.WriteData(Keys.GetData(), BufferSize);
		ValuesBufferRHI[i] = Initializer.Finalize();
	}

	// Allocate GPU resources.
	for (int32 BufferIndex = 0; BufferIndex < 2; ++BufferIndex)
	{
		KeysBufferSRV[BufferIndex] = RHICmdList.CreateShaderResourceView(
			KeysBufferRHI[BufferIndex],
			FRHIViewDesc::CreateBufferSRV()
				.SetType(FRHIViewDesc::EBufferType::Typed)
				.SetFormat(PF_R32_UINT));
		KeysBufferUAV[BufferIndex] = RHICmdList.CreateUnorderedAccessView(
			KeysBufferRHI[BufferIndex], 
			FRHIViewDesc::CreateBufferUAV()
				.SetType(FRHIViewDesc::EBufferType::Typed)
				.SetFormat(PF_R32_UINT));

		ValuesBufferSRV[BufferIndex] = RHICmdList.CreateShaderResourceView(
			ValuesBufferRHI[BufferIndex],
			FRHIViewDesc::CreateBufferSRV()
				.SetType(FRHIViewDesc::EBufferType::Typed)
				.SetFormat(PF_R32_UINT));
		ValuesBufferUAV[BufferIndex] = RHICmdList.CreateUnorderedAccessView(
			ValuesBufferRHI[BufferIndex], 
			FRHIViewDesc::CreateBufferUAV()
				.SetType(FRHIViewDesc::EBufferType::Typed)
				.SetFormat(PF_R32_UINT));
...
{
	TGuardValue<bool> SortDebugOffsets(GbSortDebugOffsets, true);
	TGuardValue<bool> SortDebug(GbSortDebug, true);
