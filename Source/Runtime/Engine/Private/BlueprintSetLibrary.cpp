...
void UBlueprintSetLibrary::GenericSet_GetItemByIndex(void* TargetSet, const FSetProperty* SetProperty, int32 Index, void* Item)
{
	if (TargetSet)
	{
		FScriptSetHelper SetHelper(SetProperty, TargetSet);
		const FProperty* ElementProp = SetProperty->ElementProp;
		const TScriptContainerIterator<FScriptSetHelper> Iterator = SetHelper.CreateIterator(Index);

		if (Iterator)
		{
			ElementProp->CopySingleValueToScriptVM(Item, SetHelper.GetElementPtr(Iterator));
		}
		else
		{
			FFrame::KismetExecutionMessage(*FString::Printf(TEXT("Attempted to access invalid index %d from set '%s' of length %d in '%s'!"),
				Index,
				*SetProperty->GetName(),
				SetHelper.Num(),
...
int32 UBlueprintSetLibrary::GenericSet_GetLastIndex(const void* TargetSet, const FSetProperty* SetProperty)
{
	if (TargetSet)
	{
		FScriptSetHelper SetHelper(SetProperty, TargetSet);
