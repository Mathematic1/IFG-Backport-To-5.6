...
void UVolumeTexture::UpdateResourceWithParams(EUpdateResourceFlags InFlags)
{
#if WITH_EDITOR
...
void UVolumeTexture::SetDefaultSource2DTileSize()
{
	...
	if (Source2DTexture && Source2DTexture->Source.IsValid())
	{
		...
		if (SourceSizeX > 0 && SourceSizeY > 0)
		{
			...
			const int32 TileSize = FMath::Max(1, FMath::RoundToInt(FMath::Pow((float)NumPixels, 1.f / 3.f)));
			const int32 NumTilesBySide = FMath::Max(1, FMath::RoundToInt(FMath::Sqrt((float)(SourceSizeX / TileSize) * (SourceSizeY / TileSize))));
