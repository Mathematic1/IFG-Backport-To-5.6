...
float ULocalLightComponent::GetUnitsConversionFactor(ELightUnits SrcUnits, ELightUnits TargetUnits, float CosHalfConeAngle)
{
	...
	else
	{
		...
		if (SrcUnits == ELightUnits::Nits)
		{
			// no scale factor (it depends on the light's area)
		}
		else if (SrcUnits == ELightUnits::Candelas)
		{
		...
		if (TargetUnits == ELightUnits::Nits)
		{
			// no scale factor (it depends on the light's area)
		}
		else if (TargetUnits == ELightUnits::Candelas)
		{
