...
void UShapeComponent::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsMigratingRemoteObjects() && Ar.IsLoading())
	{
		// ShapeBodySetup exists outside of this hierarchy (it is a runtime instanced SubObject on the CDO)
		// So null it out here so we don't end up with that runtime CDO SubObject from another server.
		// If we migrated, it would show up as a RemoteObject and crash since its Outer is the CDO.
		// Note: We should ban CDO mutation on runtime objects. This is never safe due to delta serialization.
		if (ShapeBodySetup.IsRemote())
		{
...
void UShapeComponent::CreateShapeBodySetupIfNeeded()
{
	if (!IsValid(ShapeBodySetup))
	{
		...
		BodyInstance.BodySetup = ShapeBodySetup;
		{
			...
			{
				FPhysicsCommand::ExecuteWrite(BodyInstance.GetPhysicsActorWeldRoot(), [this](const FPhysicsActorHandle& Actor)
				{
