...
bool UTextureRenderTarget::UpdateTexture(UTexture* InTexture, EConstructTextureFlags InFlags, const TArray<uint8>* InAlphaOverride, FOnTextureChangingDelegate InOnTextureChangingDelegate, FText* OutErrorMessage)
{
	...
	if ( PixelFormat == PF_R16F || PixelFormat == PF_R32_FLOAT )
	{
		// single 16F instead of TC_HDR which is RGBA16F
		CompressionSettingsForTexture = TC_HalfFloat;
	}
	#if 0
	// @todo Oodle : this seems to work fine but not sure it's desirable because of the way format affects usage in Unreal
	else if ( PixelFormat == PF_G8 || PixelFormat == PF_G16 )
	{
		CompressionSettingsForTexture = TC_Grayscale;
	}
	#endif
