...
void FSubsurfaceProfileTexture::CreateTexture(FRHICommandListImmediate& RHICmdList)
{
	...
	for (uint32 y = 0; y < Height; ++y)
	{
		...
		TextureRow[SSSS_BOUNDARY_COLOR_BLEED_OFFSET].A = 
			(Data.Implementation == ESubsurfaceImplementationTechniqueHint::SIH_AFIS) ? SSS_TYPE_BURLEY : SSS_TYPE_SSSS;
...
bool FSubsurfaceProfileTexture::GetEntryString(uint32 Index, FString& Out) const
{
	...
	Out = FString::Printf(TEXT(" %c. %p SurfaceAlbedo=%.1f %.1f %.1f, MeanFreePathColor=%.1f %.1f %.1f, MeanFreePathDistance=%.1f, WorldUnitScale=%.1f,\
								Tint=%.1f %.1f %.1f, BoundaryColorBleeding=%.1f %.1f %.1f, TransmissionTintColor=%.1f %.1f %.1f, %s"), 
		...
		ref.Implementation == ESubsurfaceImplementationTechniqueHint::SIH_AFIS?TEXT("AFIS") : TEXT("Separable"));
...
void FSubsurfaceProfileTexture::Dump()
{
	...
	for (int32 i = 0; i < SubsurfaceProfileEntries.Num(); ++i)
	{
...

		{
			...
			UE_LOG(LogSubsurfaceProfile, Log, TEXT("     %s"),
				SubsurfaceProfileEntries[i].Settings.Implementation == ESubsurfaceImplementationTechniqueHint::SIH_AFIS?TEXT("AFIS") : TEXT("Separable"));
		}
	}
