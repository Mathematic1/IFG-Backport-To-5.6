...
void FSkeletalMeshAttributeVertexBuffer::InitRHI(FRHICommandListBase& RHICmdList)
{
	...
	const FRHIBufferCreateDesc CreateDesc =
		FRHIBufferCreateDesc::CreateVertex(TEXT("SkeletalMeshAttributeVertexBuffer"), ResourceArray->GetResourceDataSize())
		.AddUsage(EBufferUsageFlags::Static | EBufferUsageFlags::ShaderResource)
		.SetInitActionResourceArray(ResourceArray)
		.DetermineInitialState();

	ValueBuffer.VertexBufferRHI = RHICmdList.CreateBuffer(CreateDesc);
	ValueBuffer.VertexBufferSRV = RHICmdList.CreateShaderResourceView(
		ValueBuffer.VertexBufferRHI,
		FRHIViewDesc::CreateBufferSRV()
			.SetType(FRHIViewDesc::EBufferType::Typed)
			.SetFormat(static_cast<EPixelFormat>(PixelFormat)));
}
