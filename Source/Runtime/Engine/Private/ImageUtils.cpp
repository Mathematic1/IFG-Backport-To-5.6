...
#ifdef LoadImage // Seems like some windows include is leaking through (LoadImage define can be set to LoadImageW)
#undef LoadImage
#endif
...
bool FImageUtils::ExportRenderTargetToDDS(TArray64<uint8> & OutData, UTextureRenderTarget * TexRT)
{
	...
	UE_LOG(LogImageUtils,Display,TEXT("Exporting DDS Dimension=%d SizeX=%" INT64_FMT " SizeY=%" INT64_FMT " SizeZ=%d NumMips=%d ArraySize=%d"),Dimension, SizeX,SizeY,SizeZ,NumMips,ArraySize);
...
// prefer FImageCore::ResizeImage
void FImageUtils::ImageResize(int32 SrcWidth, int32 SrcHeight, const TArray<FColor> &SrcData, int32 DstWidth, int32 DstHeight, TArray<FColor> &DstData, bool bResizeSRGBinLinearSpace, bool bForceOpaqueOutput)
{
	int64 Num = (int64)DstWidth*DstHeight;
	DstData.Empty(Num);
	DstData.AddZeroed(Num);

	ImageResize(SrcWidth, SrcHeight, TArrayView<const FColor>(SrcData), DstWidth, DstHeight, TArrayView<FColor>(DstData), bResizeSRGBinLinearSpace, bForceOpaqueOutput);
}

// prefer FImageCore::ResizeImage
void FImageUtils::ImageResize(int32 SrcWidth, int32 SrcHeight, const TArrayView<const FColor> &SrcData, int32 DstWidth, int32 DstHeight, const TArrayView<FColor> &DstData, bool bResizeSRGBinLinearSpace, bool bForceOpaqueOutput)
{
	check(SrcData.Num() >= (int64)SrcWidth * SrcHeight);
	check(DstData.Num() >= (int64)DstWidth * DstHeight);
	
	if ( SrcData.Num() == 0 || DstData.Num() == 0 )
	{
		return;
	}

	// bResizeSRGBinLinearSpace means convert SRGB colors to Linear before resizing
	//	if it's false, we tell resize that our U8's are linear so it does no transforms
	EGammaSpace Gamma = bResizeSRGBinLinearSpace ? EGammaSpace::sRGB : EGammaSpace::Linear; 

	FImageView SrcImage(&SrcData[0],SrcWidth,SrcHeight,Gamma);
	FImageView DstImage(&DstData[0],DstWidth,DstHeight,Gamma);
	
	FImageCore::ResizeImage(SrcImage,DstImage);

	if ( bForceOpaqueOutput )
	{
		FImageCore::SetAlphaOpaque(DstImage);
	}
}
...
void FImageUtils::ImageResize(int32 SrcWidth, int32 SrcHeight, const TArrayView64<const FLinearColor>& SrcData, int32 DstWidth, int32 DstHeight, const TArrayView64<FLinearColor>& DstData)
{
	...
	FImageView SrcImage(&SrcData[0],SrcWidth,SrcHeight);
	FImageView DstImage(&DstData[0],DstWidth,DstHeight);
...
UTexture * FImageUtils::CreateTexture(ETextureClass TextureClass, const FImageView & Image, UObject* Outer, const FString& Name, EObjectFlags Flags, bool DoPostEditChange )
{
	...
	if ( DoPostEditChange )
	{
		// note: Tex->CompressionSettings , SRGB, etc. have not been set yet
		//	calling PostEditChange usually doesn't make sense
		//	you should first set those things up, then call PostEditChange yourself outside
...
void FImageUtils::CropAndScaleImage( int32 SrcWidth, int32 SrcHeight, int32 DesiredWidth, int32 DesiredHeight, const TArray<FColor> &SrcData, TArray<FColor> &DstData  )
{
	...
	// Resize the image
	FImageUtils::ImageResize( MaxWidth, MaxHeight, CroppedData, DesiredWidth, DesiredHeight, DstData, true, true );
}
