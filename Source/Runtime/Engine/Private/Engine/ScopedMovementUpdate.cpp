...
, bIsGroupDeferedUpdate(ScopeBehavior == EScopedUpdate::DeferredGroupUpdates)
{
	if (IsValid(Component))
	{
		...
		// Do not allow for GROUP deferred updates if there is already an outer scope 
		// which is not part of the group.
		
		// Safe gaurd against other possible input scopes so that you can only use grouped updates on their own, not with others
		if (bIsGroupDeferedUpdate && OuterDeferredScope && !OuterDeferredScope->bIsGroupDeferedUpdate)
		{
			UE_LOG(LogScopedMovementUpdate, Warning, TEXT("FScopedMovementUpdate attempting to use group updates within an ungrouped scope, will use deferred updates instead."));
			bIsGroupDeferedUpdate = false;

			// Fallback to just using normal deferred updates 
			bDeferUpdates = true;
		}

		// Probably better to have a "Begin Grouped movement update" instead to keep them separate. 
		if (bDeferUpdates || bIsGroupDeferedUpdate)
		{
...
{
	if ((bDeferUpdates || bIsGroupDeferedUpdate) && IsValid(Owner))
	{
...
void FScopedMovementUpdate::RevertMove()
{
	...
	if (IsValid(Component))
	{
		...
		if (IsTransformDirty())
		{
			...
			Component->RelativeRotationCache.RotatorToQuat(InitialRelativeRotation);
			Component->WorldRotationCache.RotatorToQuat(InitialTransform.Rotator());
