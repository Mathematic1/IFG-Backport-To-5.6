...
void FGeomTools2D::GenerateConvexHullFromPoints(TArray<FVector2D>& OutConvexHull, TArray<FVector2D>& SourcePoints)
{
	...
	ConvexHull2D::ComputeConvexHull(SourcePoints, HullIndices);
	OutConvexHull.Empty(HullIndices.Num());

	for (int32 HullIdx : HullIndices)
	{
		OutConvexHull.Add(SourcePoints[HullIdx]);
	}
}
