...
void UAudioBus::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyName == GET_MEMBER_NAME_CHECKED(UAudioBus, AudioBusChannels))
	{
		...
		{
			DeviceManager->IterateOverAllDevices([BusId = GetUniqueID(), Name = GetPathName(), NumChannels = AudioBusChannels](Audio::FDeviceId, FAudioDevice* InDevice)
			{
				...
				AudioBusSubsystem->StartAudioBus(AudioBusKey, Name, (int32)NumChannels + 1, false /* bInIsAutomatic */);
			});
		}
	}
}
