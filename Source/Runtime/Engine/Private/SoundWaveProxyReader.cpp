	...
	, Settings(InSettings)
{
	...
	UE_CLOG(Audio::MatchesLogFilter(WaveProxy->GetFName()),LogAudioTiming, Verbose,
		TEXT("FSoundWaveProxyReader ctor: Name=%s,NumChannels=%d, Rate=%.2f, NumFrames=%d"), *InWaveProxy->GetFName().ToString(), NumChannels, SampleRate, NumFramesInWave);
...
bool FSoundWaveProxyReader::SeekToTime(float InSeconds)
{
	...
	UE_CLOG(Audio::MatchesLogFilter(WaveProxy->GetFName()), LogAudioTiming, Verbose,
		TEXT("FSoundWaveProxyReader SeekToTime: SeekTime=%.2f, Name=%s, NumChannels=%d, Rate=%.2f, NumFrames=%d"),
		InSeconds, *WaveProxy->GetFName().ToString(), NumChannels, SampleRate, NumFramesInWave);
...
bool FSoundWaveProxyReader::SeekToFrame(uint32 InFrameNum)
{
	...
	UE_CLOG(Audio::MatchesLogFilter(WaveProxy->GetFName()), LogAudioTiming, Verbose,
		TEXT("FSoundWaveProxyReader SeekToFrame: SeekFrame=%u, Name=%s, NumChannels=%d, Rate=%.2f, NumFrames=%d"),
		InFrameNum, *WaveProxy->GetFName().ToString(), NumChannels, SampleRate, NumFramesInWave);
...
int32 FSoundWaveProxyReader::PopAudio(Audio::FAlignedFloatBuffer& OutBuffer)
{
	...
	while ((NumSamplesUnset > 0) && bCanProduceMoreAudio)
	{
		...
		UE_CLOG(Audio::MatchesLogFilter(WaveProxy->GetFName()), LogAudioTiming, VeryVerbose,
				TEXT("FSoundWaveProxyReader PopAudio, NumSamplesCopiedThisLoop=%d, NumSamplesCopied=%d, NumSamplesUnset=%d, Name=%s, NumChannels=%d, Rate=%.2f, NumFrames=%d"),
						NumSamplesCopiedThisLoop, NumSamplesCopied, NumSamplesUnset,  *WaveProxy->GetFName().ToString(), NumChannels, SampleRate, NumFramesInWave)
		...
		if (Settings.bIsLooping)
		{
			...
			if (0 == DecoderOutput.Num())
			{
				...
				{
					UE_CLOG(Audio::MatchesLogFilter(WaveProxy->GetFName()), LogAudioTiming, Verbose,
								TEXT("FSoundWaveProxyReader PopAudio (looping-return to start) (((!bDecoderCanDecodeMoreData) || (CurrentFrameIndex >= LoopEndFrameIndex))), Name=%s, NumChannels=%d, Rate=%.2f, NumFrames=%d"),
									*WaveProxy->GetFName().ToString(), NumChannels, SampleRate, NumFramesInWave)
		...
		{
			UE_CLOG(Audio::MatchesLogFilter(WaveProxy->GetFName()),LogAudioTiming, Verbose,
				TEXT("FSoundWaveProxyReader PopAudio (break-loop, (bCanProduceMoreAudio && DecoderOutput.Num() == 0), Name=%s, NumChannels=%d, Rate=%.2f, NumFrames=%d"),
					*WaveProxy->GetFName().ToString(), NumChannels, SampleRate, NumFramesInWave);
	...
	UE_CLOG(Audio::MatchesLogFilter(WaveProxy->GetFName()), LogAudioTiming, Verbose,
		TEXT("FSoundWaveProxyReader PopAudio (loop-end), OutBufferView.Num()=%d, NumSamplesCopied=%d, Name=%s, NumChannels=%d, Rate=%.2f, NumFrames=%d"),
		OutBufferView.Num(), NumSamplesCopied,  *WaveProxy->GetFName().ToString(), NumChannels, SampleRate, NumFramesInWave);
