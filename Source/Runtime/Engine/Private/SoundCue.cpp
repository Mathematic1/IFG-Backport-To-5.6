...
void USoundCue::Parse(FAudioDevice* AudioDevice, const UPTRINT NodeWaveInstanceHash, FActiveSound& ActiveSound, const FSoundParseParameters& ParseParams, TArray<FWaveInstance*>& WaveInstances)
{
	...
	if (FSoundCueParameterTransmitter* Transmitter = static_cast<FSoundCueParameterTransmitter*>(ActiveSound.GetTransmitter()))
	{
		...
		if (WaveInstances.IsEmpty())
		{
			return;
		}

		auto UpdateChildParameters = [](IParameterTransmitter& InChildTransmitter, USoundBase* InSound, TArray<FAudioParameter> InParameters)
		{
			if (InSound)
			{
				InSound->InitParameters(InParameters);
			}

			if (!InParameters.IsEmpty())
			{
				InChildTransmitter.SetParameters(MoveTemp(InParameters));
			}
		};

		const FWaveInstance* LastInstance = WaveInstances.Last();
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
			{
				...
				if (ChildTransmitterPtr->IsValid())
				{
					...
					{
						// Avoid extra copy on last wave instance by moving parameters. 
						// This also avoids a copy on simple case where there is only one wave instance. 
						UpdateChildParameters(*ChildTransmitterPtr->Get(), Instance->WaveData, MoveTemp(ParameterUpdates));
					}
					else
					{
						UpdateChildParameters(*ChildTransmitterPtr->Get(), Instance->WaveData, ParameterUpdates);
					}
				}
			}
		}
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS // Disabling deprecation warnings for construction/destruction of deprecated member variables. 
FSoundCueParameterTransmitter::FSoundCueParameterTransmitter(Audio::FParameterTransmitterInitParams&& InParams)
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS // Disabling deprecation warnings for construction/destruction of deprecated member variables.
FSoundCueParameterTransmitter::~FSoundCueParameterTransmitter()
{
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS
...
{
	static bool bHasLoggedError = false;
	if (!bHasLoggedError)
	{
		UE_LOG(LogAudio, Error, TEXT("Do not call FSoundCueParameterTransmitter::GetReferencedObjects() as it is not thread safe."));
		bHasLoggedError = true;
	}
...
{
PRAGMA_DISABLE_DEPRECATION_WARNINGS // Disable deprecations because ParamsToSet is being moved to private
	AddReferencedObjectsFromParameters(InCollector, ParamsToSet);
PRAGMA_ENABLE_DEPRECATION_WARNINGS
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
		...
		return Super::SetParameters(MoveTemp(InParameters));
	}
	else
	{
		TArray<FAudioParameter> TempParams = InParameters;
...
void FSoundCueParameterTransmitter::CreateChildTransmitter(UPTRINT InWaveInstanceHash, USoundBase& InSound, Audio::FParameterTransmitterInitParams InTransmitterInitParams)
{
	check(IsInAudioThread());

	// Override any default parameters with the existing parameters. 
	TArray<FAudioParameter> CurrentAudioParameters;
	CopyParameters(CurrentAudioParameters);
	FAudioParameter::Merge(MoveTemp(CurrentAudioParameters), InTransmitterInitParams.DefaultParams);

	InSound.InitParameters(InTransmitterInitParams.DefaultParams);
	// Create child transmitter and store if valid. 
	TSharedPtr<Audio::IParameterTransmitter> ChildTransmitter = InSound.CreateParameterTransmitter(MoveTemp(InTransmitterInitParams));

	if (ChildTransmitter.IsValid())
	{
PRAGMA_DISABLE_DEPRECATION_WARNINGS // Disable deprecations because transmitters is being moved to private
		Transmitters.Add(InWaveInstanceHash, ChildTransmitter.ToSharedRef());
PRAGMA_ENABLE_DEPRECATION_WARNINGS 
	}
}
...
void FSoundCueParameterTransmitter::RemoveChildTransmitter(UPTRINT InWaveInstanceHash)
{
	check(IsInAudioThread());
PRAGMA_DISABLE_DEPRECATION_WARNINGS // Disable deprecations because transmitters is being moved to private
	Transmitters.Remove(InWaveInstanceHash);
PRAGMA_ENABLE_DEPRECATION_WARNINGS 
}
...
{
PRAGMA_DISABLE_DEPRECATION_WARNINGS // Disable deprecations because ParamsToSet is being moved to private
	TArray<FAudioParameter> AccumulatedParameters(MoveTemp(ParamsToSet));
	ParamsToSet.Reset();
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	return MoveTemp(AccumulatedParameters);
}
