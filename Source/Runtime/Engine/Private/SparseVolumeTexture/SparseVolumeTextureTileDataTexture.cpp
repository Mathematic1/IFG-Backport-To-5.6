...
DECLARE_MEMORY_STAT_POOL(TEXT("TileData GPU Memory"), STAT_SVTTileDataMemory, STATGROUP_SparseVolumeTextureMemory, FPlatformMemory::MCR_GPU);
...
int64 FTileDataTexture::GetMaxTileDataTextureResourceSize(int32 InVoxelMemSize)
{
	...
	if (ShouldUseReservedResources())
	{
		...
		if (GSVTStreamingReservedResourcesMemoryLimit > 0)
		{
			...
			const int64 ReservedResourcesMemoryLimit = FMath::Min((int64)GSVTStreamingReservedResourcesMemoryLimit, INT64_MAX / OneMiB) * OneMiB;
			MaxStreamingPoolResourceSize = FMath::Min(MaxStreamingPoolResourceSize, ReservedResourcesMemoryLimit);
		}
	}
...
#if STATS
		const int64 MemorySize = static_cast<int64>(RHICalcTexturePlatformSize(TileDataTextureA->GetRHI()->GetDesc()).Size);
		INC_MEMORY_STAT_BY(STAT_SVTTileDataMemory, MemorySize);
...
#if STATS
		const int64 MemorySize = static_cast<int64>(RHICalcTexturePlatformSize(TileDataTextureB->GetRHI()->GetDesc()).Size);
		INC_MEMORY_STAT_BY(STAT_SVTTileDataMemory, MemorySize);
		INC_MEMORY_STAT_BY(STAT_SVTGPUMemoryTotal, MemorySize);
#endif
	}
...
void FTileDataTexture::ReleaseRHI()
{
	...
	{
		const int64 MemorySize = static_cast<int64>(RHICalcTexturePlatformSize(TileDataTextureA->GetRHI()->GetDesc()).Size);
		DEC_MEMORY_STAT_BY(STAT_SVTTileDataMemory, MemorySize);
