...
static int32 GSVTEnabled = 1;
static FAutoConsoleVariableRef CVarSVTEnabled(
	TEXT("r.SparseVolumeTexture"),
	GSVTEnabled,
	TEXT("Whether SparseVolumeTextures are supported at all. Toggles support for streaming and rendering SVTs."),
...
void FStreamingManager::InitRHI(FRHICommandListBase& RHICmdList)
{
	...
	if (!UseSparseVolumeTextures(GMaxRHIShaderPlatform))
	{
		return;
	}
...
{
	if (!UseSparseVolumeTextures(GMaxRHIShaderPlatform) || !SparseVolumeTexture)
	{
		return;
	}
...
void FStreamingManager::BeginAsyncUpdate(FRDGBuilder& GraphBuilder, bool bUseAsyncThread)
{
	...
	if (!UseSparseVolumeTextures(GMaxRHIShaderPlatform) || StreamingInfo.IsEmpty())
	{
		return;
	}
...
void FStreamingManager::EndAsyncUpdate(FRDGBuilder& GraphBuilder)
{
	...
	SET_MEMORY_STAT(STAT_SVTCPUMemoryTotal, GetAllocatedSize());
}
...
#if STATS
				const int64 MemorySize = static_cast<int64>(RHICalcTexturePlatformSize(FrameInfo.PageTableTexture->GetRHI()->GetDesc()).Size);
				INC_MEMORY_STAT_BY(STAT_SVTPageTableMemory, MemorySize);
				INC_MEMORY_STAT_BY(STAT_SVTGPUMemoryTotal, MemorySize);
#endif
			}
...
#if STATS
			const int64 MemorySize = static_cast<int64>(RHICalcTexturePlatformSize(FrameInfo.PageTableTexture->GetRHI()->GetDesc()).Size);
			DEC_MEMORY_STAT_BY(STAT_SVTPageTableMemory, MemorySize);
			DEC_MEMORY_STAT_BY(STAT_SVTGPUMemoryTotal, MemorySize);
#endif
		...
		// Create request
		{
...
#endif
			{
...
#endif
				{
					...
					TRACE_IOSTORE_METADATA_SCOPE_TAG("SparseVolumeTexture");
...
#endif // WITH_EDITORONLY_DATA
		{
			...
			if (PendingRequest.Request.IsCompleted())
			{
				if (!PendingRequest.Request.IsOk())
				{
					...
					TRACE_IOSTORE_METADATA_SCOPE_TAG("SparseVolumeTexture (retry)");
					FBulkDataBatchRequest::FScatterGatherBuilder Batch = FBulkDataBatchRequest::ScatterGather(1);
...
SIZE_T FStreamingManager::GetAllocatedSize() const
{
	SIZE_T AllocatedSize = 0;
	AllocatedSize += SparseVolumeTextureToHandle.GetAllocatedSize();
	AllocatedSize += StreamingInfo.GetAllocatedSize();
	for (const auto& Info : StreamingInfo)
	{
		AllocatedSize += Info->GetAllocatedSize();
	}
	AllocatedSize += RequestsHashTable.GetAllocatedSize();
	AllocatedSize += PendingRequests.GetAllocatedSize();
...
SIZE_T FStreamingManager::FFrameInfo::GetAllocatedSize() const
{
	return TileAllocations.GetAllocatedSize()
		+ ResidentPages.GetAllocatedSize()
		+ InvalidatedPages.GetAllocatedSize()
		+ ResidentTiles.GetAllocatedSize()
		+ StreamingTiles.GetAllocatedSize()
		+ TileIndexToPendingRequestIndex.GetAllocatedSize();
}

SIZE_T FStreamingManager::FStreamingInfo::GetAllocatedSize() const
{
	SIZE_T AllocatedSize = 0;
	for (const FFrameInfo& FrameInfo : PerFrameInfo)
	{
		AllocatedSize += FrameInfo.GetAllocatedSize();
	}
	AllocatedSize += PerFrameInfo.GetAllocatedSize();
	AllocatedSize += StreamingInstances.GetAllocatedSize() + StreamingInstances.Num() * sizeof(*StreamingInstances[0]); // StreamingInstances only stores pointers, but each FStreamingInstance has the same size.
	AllocatedSize += MipLevelStreamingSize.GetAllocatedSize();
	AllocatedSize += TileAllocator.GetAllocatedSize();
	AllocatedSize += sizeof(*TileDataTexture);
	return AllocatedSize;
}
...
SIZE_T FTileAllocator::GetAllocatedSize() const
{
	// FBinaryHeap unfortunately does not support querying the allocated size, so we just compute it ourselves here.
	const SIZE_T HeapSize = FreeHeap.GetHeapSize() * sizeof(uint32) + FreeHeap.GetIndexSize() * (sizeof(uint64) + sizeof(uint32));
	return HeapSize + Allocations.GetAllocatedSize();
}
