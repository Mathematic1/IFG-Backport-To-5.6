...
struct FStatRenderGlobals
{
	...
	int32 AfterNameColumnOffset = 0;

	/** Same as above, but for stat groups that don't need to display as many characters in the name column. */
	int32 AfterNameColumnOffsetNarrow = 0;

	/** Rendering offsets for additional columns. */
	int32 InterColumnOffset = 0;

	/** Same as above, but for stat columns that don't need to display as many value characters. */
	int32 InterColumnOffsetNarrow = 0;

	/** Color for rendering stats. */
	FLinearColor StatColor = { 0.f, 1.f, 0.f };

	/** Color to use when rendering headings. */
	FLinearColor HeadingColor = { 1.f, 0.2f, 0.f };
	...
	EStatFontTypes StatFontType = EStatFontTypes::NumFonts;
	...
	void Initialize( const int32 InSizeX, const int32 InSizeY, const int32 InSafeSizeX, const bool bInIsStereo )
	{
		...
		if( bNeedRefresh )
		{
			...
			const FString STATNAME_COLUMN_WIDTH_NARROW = FString::ChrN(GetNumCharsForStatName() / 2, 'S');
			...
			const FString STATDATA_COLUMN_WIDTH_NARROW = FString::ChrN(StatFontType == EStatFontTypes::Small ? 5 : 4, 'W');
			...
			StringSize(StatFont, AfterNameColumnOffsetNarrow, StatColumnSpaceSizeY, *STATNAME_COLUMN_WIDTH_NARROW);
			...
			StringSize(StatFont, InterColumnOffsetNarrow, TimeColumnSpaceSizeY, *STATDATA_COLUMN_WIDTH_NARROW);
...
static void RenderCounterLabel(const FComplexStatMessage& All, class FCanvas* Canvas, const int32 X, const int32 Y)
{
	const FStatRenderGlobals& Globals = GetStatRenderGlobals();
...
static void RenderCounterValues(const FComplexStatMessage& All, class FCanvas* Canvas, int32& X, const int32 Y, int32 Spacing, float ZeroStatAlphaMult = 1.0f)
{
	const FStatRenderGlobals& Globals = GetStatRenderGlobals();

	auto RenderValue = [&](EComplexStatField::Type Field)
	{
		if (All.NameAndInfo.GetField<EStatDataType>() == EStatDataType::ST_double)
		{
			const FString ValueFormatted = FormatStatValueFloat(All.GetValue_double(Field));

			FLinearColor Color = Globals.StatColor;
			if (ValueFormatted == TEXT("0.00"))
			{
				Color.A *= ZeroStatAlphaMult;
			}

			RightJustify(Canvas, X, Y, *ValueFormatted, Color);
		}
		else if (All.NameAndInfo.GetField<EStatDataType>() == EStatDataType::ST_int64)
		{
			int64 Value = All.GetValue_int64(Field);
			const FString ValueFormatted = FormatStatValueInt64(Value);
	...
	if (All.NameAndInfo.GetFlag(EStatMetaFlags::ShouldClearEveryFrame))
	{
		// Append the average.
		RenderValue(EComplexStatField::IncAve);
	}
	X += Spacing;

	// Append the maximum.
	RenderValue(EComplexStatField::IncMax);
	X += Spacing;

	// Append the minimum.
	RenderValue(EComplexStatField::IncMin);
	X += Spacing;
}
...
static int32 RenderCounter( const FGameThreadStatsData& ViewData, const FComplexStatMessage& All, class FCanvas* Canvas, const int32 X, const int32 Y, const float Budget, const bool bIsBudgetIgnored )
{
	...
	// Draw the label
	int32 CurrX = X;
	RenderCounterLabel(All, Canvas, CurrX, Y);
	CurrX += Globals.AfterNameColumnOffset;
	RenderCounterValues(All, Canvas, CurrX, Y, Globals.InterColumnOffset);
...
struct FGpuStatMessage
{
	const FComplexStatMessage* Busy;
	const FComplexStatMessage* Wait;
	const FComplexStatMessage* Idle;
};

static int32 RenderGpuStatHeadings(class FCanvas* Canvas, const int32 X, const int32 Y)
{
	const FStatRenderGlobals& Globals = GetStatRenderGlobals();
	...
	// Draw headers
	int32 CurrX = X + Globals.AfterNameColumnOffsetNarrow + Globals.InterColumnOffsetNarrow;
	for (TCHAR const* Type : Types)
	{
		RightJustify(Canvas, CurrX, Y - Globals.GetFontHeight(), Type, Globals.HeadingColor);
		CurrX += Globals.InterColumnOffsetNarrow * 2 + Globals.InterColumnOffset;
	}

	// Draw column labels
	CurrX = X + Globals.AfterNameColumnOffsetNarrow;
	for (TCHAR const* Type : Types)
	{
		// Draw the average column label.
		RightJustify(Canvas, CurrX, Y, TEXT("Avg"), Globals.HeadingColor);
		CurrX += Globals.InterColumnOffsetNarrow;
		...
		// Use a larger space between the stat types
		CurrX += Globals.InterColumnOffset;
	}
...
void RenderArrayOfGpuStats(FCanvas* Canvas, const int32 X, int32& Y, const TMap<FName, FGpuStatMessage>& Aggregates, const FGameThreadStatsData& ViewData)
{
	const FStatRenderGlobals& Globals = GetStatRenderGlobals();
	const FTexture* BackgroundTexture = Globals.GetBackgroundTexture();

	int32 RowIndex = 0;
	auto DrawRowBackground = [&]()
	{
		if (BackgroundTexture != nullptr)
		{
			Canvas->DrawTile(
				X,
				Y + Globals.GetYOffset(),
				Globals.AfterNameColumnOffsetNarrow + Globals.InterColumnOffsetNarrow * 6 + Globals.InterColumnOffset * 3,
				Globals.GetFontHeight(),
				0, 0, 1, 1,
				Globals.BackgroundColors[RowIndex & 1],
				BackgroundTexture,
				true
			);
		}
	...
	// Render all counters.
	int32 MaxStatsPerGroup = CVarNumStatsPerGroup.GetValueOnGameThread();
	for (auto const& [Name, Stat] : Aggregates)
	{
		if (RowIndex >= MaxStatsPerGroup)
		{
			break;
		}
		...
		int32 CurrX = X;
		const FComplexStatMessage* StatForLabel = 
			Stat.Busy ? Stat.Busy :
			Stat.Wait ? Stat.Wait : 
			Stat.Idle ? Stat.Idle : nullptr;

		if (StatForLabel)
		{
			RenderCounterLabel(*StatForLabel, Canvas, CurrX, Y);
			CurrX += Globals.AfterNameColumnOffsetNarrow;

			// Render the Busy, Wait and Idle stats all on the same row
			auto RenderCol = [&](const FComplexStatMessage* Message)
			{
				if (Message)
				{
					RenderCounterValues(*Message, Canvas, CurrX, Y, Globals.InterColumnOffsetNarrow, 0.4f);
				}
				else
				{
					CurrX += Globals.InterColumnOffsetNarrow * 3;
				}

				// Adjust for a wider space between stat types
				CurrX -= Globals.InterColumnOffsetNarrow;
				CurrX += Globals.InterColumnOffset;
			};
		...
		// Stop drawing after we have gone off the bottom of the screen
		if (Y > (Globals.SizeXY.Y + Globals.GetFontHeight()))
		{
			break;
		}
...
static void RenderGroupedWithHierarchy(const FGameThreadStatsData& ViewData, FViewport* Viewport, class FCanvas* Canvas, const int32 X, int32& Y)
{
	...
	for( int32 GroupIndex = 0; GroupIndex < ViewData.ActiveStatGroups.Num(); ++GroupIndex )
	{
		...
		// Render GPU stats
	#if RHI_NEW_GPU_PROFILER
		if (StatGroup.GpuStatsAggregate.Num())
		{
			// Group together the individual GPU Busy, Wait and Idle stats 
			TMap<FName, FGpuStatMessage> GroupedStats;
			for (const FComplexStatMessage& Message : StatGroup.GpuStatsAggregate)
			{
				...
				FGpuStatMessage& Stat = GroupedStats.FindOrAdd(ShortName);
				switch (EType(Number))
				{
				default: checkNoEntry(); [[fallthrough]];
				case EType::Busy: Stat.Busy = &Message; break;
				case EType::Wait: Stat.Wait = &Message; break;
				case EType::Idle: Stat.Idle = &Message; break;
				}
			}

			Y += RenderGpuStatHeadings(Canvas, X, Y);
			RenderArrayOfGpuStats(Canvas, X, Y, GroupedStats, ViewData);
			Y += Globals.GetFontHeight();
		}
	#endif
	}
