...
#define SCOPE_LOCK_REF(X) UE::TScopeLock ScopeLock(X);
...
void FNetworkProfiler::SerializeReplicateProperty( uint32 NameTableIndex, uint16 NumBits, UNetConnection* Connection )
{
	SetCurrentConnection( Connection );
...
void FNetworkProfiler::TrackReplicateProperty( const FProperty* Property, uint16 NumBits, UNetConnection* Connection )
{
	if(bIsTrackingEnabled && !!!IgnorePropertyCount)
	{
		...
		{
			UE_AUTORTFM_ONCOMMIT(this, NameTableIndex, NumBits, Connection)
			{
				UE::TScopeLock ScopeLock2(CriticalSection);

				SerializeReplicateProperty(NameTableIndex, NumBits, Connection);
			};
		}
		else
		{
...
void FNetworkProfiler::SerializeWritePropertyHeader( uint32 NameTableIndex, uint16 NumBits, UNetConnection* Connection )
{
	SetCurrentConnection( Connection );
	...
	if( bIsTrackingEnabled )
	{
		...
		if (AutoRTFM::IsClosed())
		{
			UE_AUTORTFM_ONCOMMIT(this, NameTableIndex, NumBits, Connection)
			{
				...
				SerializeWritePropertyHeader(NameTableIndex, NumBits, Connection);
			};
		}
		else
		{
...
void FNetworkProfiler::TrackSessionChange( bool bShouldContinueTracking, const FURL& InURL )
{
	if ( bIsTrackingEnabled || bShouldTrackingBeEnabled )
	{
		...
		if( bShouldContinueTracking )
		{
			...
			FileWriter = new FTransactionallySafeArchiveWriter( TUniquePtr<FArchive>( IFileManager::Get().CreateFileWriter( *FinalFileName, FILEWRITE_EvenIfReadOnly )));
			check( FileWriter );
...
void FNetworkProfiler::SerializeBeginContentBlock( uint32 NameTableIndex, uint16 NumBits, UNetConnection* Connection )
{
	SetCurrentConnection( Connection );
	...
	if ( bIsTrackingEnabled )
	{
		...
		if (AutoRTFM::IsClosed())
		{
			UE_AUTORTFM_ONCOMMIT(this, NameTableIndex, NumBits, Connection)
			{
				...
				SerializeBeginContentBlock(NameTableIndex, NumBits, Connection);
			};
		}
		else
		{
...
void FNetworkProfiler::SerializeEndContentBlock( uint32 NameTableIndex, uint16 NumBits, UNetConnection* Connection )
{
	SetCurrentConnection( Connection );
	...
	if ( bIsTrackingEnabled )
	{
		...
		if (AutoRTFM::IsClosed())
		{
			UE_AUTORTFM_ONCOMMIT(this, NameTableIndex, NumBits, Connection)
			{
				...
				SerializeEndContentBlock(NameTableIndex, NumBits, Connection);
			};
		}
		else
		{
...
void FNetworkProfiler::SerializeWritePropertyHandle( uint16 NumBits, UNetConnection* Connection )
{
	SetCurrentConnection( Connection );
...
void FNetworkProfiler::TrackWritePropertyHandle( uint16 NumBits, UNetConnection* Connection )
{
	if (bIsTrackingEnabled && !!!IgnorePropertyCount)
	{
		UE_AUTORTFM_ONCOMMIT(this, NumBits, Connection)
		{
			SCOPE_LOCK_REF(CriticalSection);

			SerializeWritePropertyHandle(NumBits, Connection);
		};
	}
}
