...
#if WITH_EDITORONLY_DATA
	/** 
	 * The format container may be queried from multiple threads and mutated if a thread fails to find a format.
	 * This mutex protects the cook path in TryGetCookedData so we don't have multiple threads attempting to
	 * write to the format data
	 */
	FCriticalSection CookedFormatDataMutex;
#endif
