...
public:
	 ...
	 */
	UFUNCTION(BlueprintPure, CustomThunk, meta = (DisplayName = "Get Item By Index", CompactNodeTitle = "GET ITEM BY INDEX", SetParam = "TargetSet|Item", AutoCreateRefTerm = "Item", BlueprintThreadSafe), Category = "Utilities|Set")
	static void Set_GetItemByIndex(const TSet<int32>& TargetSet, int32 Index, int32& Item);
	 ...
	 */
	UFUNCTION(BlueprintPure, CustomThunk, meta=(DisplayName = "Last Index", CompactNodeTitle = "GET LAST INDEX", SetParam = "TargetSet", BlueprintThreadSafe), Category="Utilities|Set")
	...
	DECLARE_FUNCTION(execSet_GetItemByIndex)
	{
		...
		P_FINISH;
		P_NATIVE_BEGIN;
		GenericSet_GetItemByIndex(SetAddr, SetProperty, Index, ItemPtr);
		P_NATIVE_END;
		ElementProp->DestroyValue(StorageSpace);
	}
	...
	DECLARE_FUNCTION(execSet_GetLastIndex)
	{
		...
		P_FINISH;
		P_NATIVE_BEGIN;
		*(int32*)RESULT_PARAM = GenericSet_GetLastIndex(SetAddr, SetProperty);
		P_NATIVE_END
	}
	...
	static ENGINE_API void GenericSet_GetItemByIndex(void* TargetSet, const FSetProperty* SetProperty, int32 Index, void* Item);
	static ENGINE_API int32 GenericSet_GetLastIndex(const void* TargetSet, const FSetProperty* SetProperty);
