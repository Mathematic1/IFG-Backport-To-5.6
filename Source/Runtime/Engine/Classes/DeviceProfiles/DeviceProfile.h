...
public:
	...
	*/
	 ...
	 *  This is _not_ thread-safe (for thread-safety use ForEachConsolidatedCVar or GetConsolidatedCVarValue API instead)
	*/
	UE_DEPRECATED(5.6, "GetConsolidatedCVars is no longer supported due to being non-thread safe, use ForEachConsolidatedCvar or GetConsolidatedCVarValue instead")
	ENGINE_API const TMap<FString, FString>& GetConsolidatedCVars() const;

	/** 
	 * Iterates over all consolidated string key-value CVar pairs held in this device profile, or in any parent device profile.
	 * @param	FuncRef		Iteration function (returning false will early out and stop subsequent calls)
	 */
	ENGINE_API void ForEachConsolidatedCVar(const TFunctionRef<bool(const FString&, const FString&)>& FuncRef) const;
...
private:
	...
	mutable FCriticalSection CVarsLock;
