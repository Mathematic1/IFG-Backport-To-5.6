...
static int64 InitMips(FDDSFile* InDDS, const FDXGIFormatInfo* InFormatInfo)
{
	int64 AllMipsSize = 0;
...
static EDDSError AllocateMips(FDDSFile* InDDS, const FDXGIFormatInfo* InFormatInfo, uint32 InCreateFlags)
{
	...
	if (!(InCreateFlags & FDDSFile::CREATE_FLAG_NO_MIP_STORAGE_ALLOC)) 
	{
		...
		int64 AllMipsSize = InitMips(InDDS, InFormatInfo);
...
FDDSFile* FDDSFile::CreateFromDDSInMemory(const uint8* InDDS, int64 InDDSSize, EDDSError* OutError, bool bHeaderOnly)
{
	return CreateFromDDSInMemory(InDDS, InDDSSize, OutError, static_cast<EDDSReadMipMode>(bHeaderOnly));
}

/* static */ FDDSFile* FDDSFile::CreateFromDDSInMemory(const uint8* InDDS, int64 InDDSSize, EDDSError *OutError, EDDSReadMipMode InMipReadMode)
{
	...
	*OutError = ParseHeader(DDS, &DDSHeader, &DDS10Header);
	if (*OutError == EDDSError::OK)
	{
		if (InMipReadMode == UE::DDS::EDDSReadMipMode::Full)
		{
			*OutError = ReadPayload(DDS, ReadCursor, ReadEnd);
		}
		else if(InMipReadMode == UE::DDS::EDDSReadMipMode::HeaderWithMipInfo)
		{
			const FDXGIFormatInfo* FormatInfo = DXGIFormatGetInfo(DDS->DXGIFormat);

			DDS->Mips.Empty();
			DDS->Mips.SetNumZeroed(DDS->ArraySize * DDS->MipCount);

			// Fill mips info without allocating storage
			InitMips(DDS, FormatInfo);
		}
	}
