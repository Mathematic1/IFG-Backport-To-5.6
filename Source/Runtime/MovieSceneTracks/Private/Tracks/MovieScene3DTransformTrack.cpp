...
#if WITH_EDITORONLY_DATA
FText UMovieScene3DTransformTrack::GetDisplayName() const
{
	FText CustomDisplayName = Super::GetDisplayName();

	static FName Transform("Transform");
	if (CustomDisplayName.ToString() == Transform)
	{
		return NSLOCTEXT("MovieScene3DTransformTrack", "DefaultLabel", "Transform");
	}
