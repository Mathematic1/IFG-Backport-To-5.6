...
void FInterrogationChannels::QueryTransformOrigins(UMovieSceneEntitySystemLinker* Linker, const FMovieSceneSequenceHierarchy* Hierarchy, TArray<FTransform>& OutTransformOrigins, TArray<FMovieSceneSequenceID>& FocusedSubsequenceHierarchy, const UObject* InstanceData) const
{
	const FBuiltInComponentTypes* Components = FBuiltInComponentTypes::Get();

	TArray<TPair<FMovieSceneSequenceID, FMovieSceneSequenceID>> ChildToParentMapping;

	// This array will be treated as an NxM, two-dimensional array that will hold the transform origins for all sequences in the focused hierarchy
	// N is the number of interrogations, and M is the number of Sequences in the currently focused hierarchy in Sequencer
	TArray<TOptional<FTransform>> TransformOrigins;
	// Reserve room for every sequence in the focused hierarchy to have entries for every time being interrogated.
	TransformOrigins.Init(TOptional<FTransform>(), Interrogations.Num() * FocusedSubsequenceHierarchy.Num());

	const IMovieSceneTransformOrigin* RawInterface = Cast<const IMovieSceneTransformOrigin>(InstanceData);

	// Read the world context actor's instance data.
	...
	if (bHasInterface)
	{
		// Retrieve the current origin and use that for every sampled time for the root sequence transforms.
		const FTransform TransformOrigin = RawInterface ? RawInterface->GetTransformOrigin() : IMovieSceneTransformOrigin::Execute_BP_GetTransformOrigin(InstanceData);

		for (int32 OriginIndex = 0; OriginIndex < Interrogations.Num(); OriginIndex++)
		{
			TransformOrigins[OriginIndex] = TransformOrigin;
		}
	}
	
	// Read all transform origins from subsequences.
	auto GatherTransformOrigins = [Components, Hierarchy, &ChildToParentMapping, &TransformOrigins, &FocusedSubsequenceHierarchy, &InstanceData, this]
		(const FEntityAllocation* Allocation, const FMovieSceneSequenceID* SequenceIDs, const FInterrogationKey* InputKeys)
	{
		const int32 NumAllocations = Allocation->Num();
		const int32 StrideSize = Interrogations.Num();

		TOptionalComponentReader<double> DoubleComponents[6];
		for (int32 Index = 0; Index < 6; ++Index)
		{
			DoubleComponents[Index] = Allocation->TryReadComponents(Components->DoubleResult[Index]);
		}

		for (int32 ComponentIndex = 0; ComponentIndex < NumAllocations; ++ComponentIndex)
		{
			const FMovieSceneSequenceID CurrentSequence = SequenceIDs[ComponentIndex];
			const FInterrogationKey InterrogationKey = InputKeys[ComponentIndex];

			if (const FMovieSceneSequenceHierarchyNode* HierarchyNode = Hierarchy->FindNode(CurrentSequence))
			{
				// If one of the sequences isn't in the focused hierarchy, ignore it. This would be a sibling subsequence on the same track, and blending is not supported.
				// @todo support blending, and full subsequence hierarchies
				if (FocusedSubsequenceHierarchy.IndexOfByKey(CurrentSequence) == INDEX_NONE || FocusedSubsequenceHierarchy.IndexOfByKey(HierarchyNode->ParentID) == INDEX_NONE)
				{
					continue;
				}
				ChildToParentMapping.AddUnique(TPair<FMovieSceneSequenceID, FMovieSceneSequenceID>(CurrentSequence, HierarchyNode->ParentID));
			}
		
			FVector Translation = FVector::ZeroVector;
			FRotator Rotation = FRotator::ZeroRotator;

			if (DoubleComponents[0]) { Translation.X = DoubleComponents[0][ComponentIndex]; }
			if (DoubleComponents[1]) { Translation.Y = DoubleComponents[1][ComponentIndex]; }
			if (DoubleComponents[2]) { Translation.Z = DoubleComponents[2][ComponentIndex]; }
			if (DoubleComponents[3]) { Rotation.Roll = DoubleComponents[3][ComponentIndex]; }
			if (DoubleComponents[4]) { Rotation.Pitch = DoubleComponents[4][ComponentIndex]; }
			if (DoubleComponents[5]) { Rotation.Yaw = DoubleComponents[5][ComponentIndex]; }

			const int32 SequenceIndex = FocusedSubsequenceHierarchy.IndexOfByKey(CurrentSequence);
			if (SequenceIndex == INDEX_NONE)
			{
				continue;
			}
			const int32 TransformIndex = InterrogationKey.InterrogationIndex + (SequenceIndex * StrideSize);
			const FTransform TransformOrigin =  FTransform(Rotation, Translation);
			
			TransformOrigins[TransformIndex] = TransformOrigin;
		}
		
	};
	
	FEntityTaskBuilder()
	.Read(Components->SequenceID)
	.Read(Components->Interrogation.InputKey)
	.FilterAll({Components->Tags.SubInstance})
	.FilterAll({Components->Tags.ImportedEntity})
	.Iterate_PerAllocation(&Linker->EntityManager, GatherTransformOrigins);
	
	// Sort parent first
	Algo::Sort(ChildToParentMapping, [](TPair<FMovieSceneSequenceID, FMovieSceneSequenceID> A, TPair<FMovieSceneSequenceID, FMovieSceneSequenceID> B){return A.Key == B.Value;});

	// compose transform origins
	for (const TPair<FMovieSceneSequenceID, FMovieSceneSequenceID>& ChildParentPair : ChildToParentMapping)
	{
		const int32 ParentIndexStart = FocusedSubsequenceHierarchy.IndexOfByKey(ChildParentPair.Value) * Interrogations.Num();
		const int32 ChildIndexStart = FocusedSubsequenceHierarchy.IndexOfByKey(ChildParentPair.Key) * Interrogations.Num();

		for (int32 InterrogationIndex = 0; InterrogationIndex < Interrogations.Num(); ++InterrogationIndex)
		{
			const int32 ParentIndex = ParentIndexStart + InterrogationIndex;
			const int32 ChildIndex = ChildIndexStart + InterrogationIndex;

			// If the focused view in the sequence editor goes beyond the range of the subsequence, there will be interrogations where the subsequence isn't evaluated.
			if (TransformOrigins[ParentIndex].IsSet())
			{
				const FTransform ParentTransform = TransformOrigins[ParentIndex].GetValue();

				const bool bIsChildSet = TransformOrigins[ChildIndex].IsSet();

				TransformOrigins[ChildIndex] = bIsChildSet ? TransformOrigins[ChildIndex].GetValue() * ParentTransform : ParentTransform;
			}
		}
	}

	const int32 FocusedSequenceIndex = (FocusedSubsequenceHierarchy.Num() - 1) * Interrogations.Num();
	
	for (int32 OutTransformIndex = 0; OutTransformIndex < Interrogations.Num(); ++OutTransformIndex)
	{
		FTransform CurrentTransform = TransformOrigins[FocusedSequenceIndex + OutTransformIndex].IsSet() ? TransformOrigins[FocusedSequenceIndex + OutTransformIndex].GetValue() : FTransform::Identity;
		OutTransformOrigins.Add(CurrentTransform);
	}
}
...
void FSystemInterrogator::ImportTrack(UMovieSceneTrack* Track, FInterrogationChannel InChannel, FMovieSceneSequenceID SequenceID)
{
	ImportTrack(Track, Track->FindObjectBindingGuid(), InChannel, SequenceID);
}

void FSystemInterrogator::ImportTrack(UMovieSceneTrack* Track, const FGuid& ObjectBindingID, FInterrogationChannel InChannel, FMovieSceneSequenceID SequenceID)
{
	...
	{
		ExtraMetaData.Insert(SharedMetaDataIndex, FExtraMetaData{ FInterrogationInstance { OwningSequence }, InChannel, SequenceID });
	}
	else
	{
...
void FSystemInterrogator::InterrogateEntity(int32 InterrogationIndex, const FMovieSceneEvaluationFieldEntityQuery& Query)
{
	...
	if (SharedMetaDataIndex != INDEX_NONE && ensure(ExtraMetaData.IsValidIndex(SharedMetaDataIndex)))
	{
		...
		Params.Sequence.SequenceID = InterrogationMetaData.InterrogationSequenceID;
	}
...
FInterrogationChannel FSystemInterrogator::ImportLocalTransforms(USceneComponent* SceneComponent, IMovieScenePlayer* InPlayer, FMovieSceneSequenceID SequenceID)
{
	...
	FMovieSceneEvaluationState* State = InPlayer->GetEvaluationState();
	UMovieSceneSequence* Sequence = State->FindSequence(SequenceID);
	if (!Sequence)
	{
		return FInterrogationChannel::Invalid();
	}
	...
	FGuid ObjectBindingID = State->FindCachedObjectId(*SceneComponent, SequenceID, *InPlayer);
	if (ObjectBindingID.IsValid())
	{
		const FMovieSceneBinding* Binding = Sequence->GetMovieScene()->FindBinding(ObjectBindingID);
		if (ensure(Binding))
		{
			ImportTransformTracks(*Binding, Channel);
		}
	}
	...
	{
		FGuid OwnerObjectBindingID = State->FindCachedObjectId(*Owner, SequenceID, *InPlayer);
		if (OwnerObjectBindingID.IsValid())
		{
			const FMovieSceneBinding* Binding = Sequence->GetMovieScene()->FindBinding(OwnerObjectBindingID);
			if (ensure(Binding))
			{
				ImportTransformTracks(*Binding, Channel);
			}
