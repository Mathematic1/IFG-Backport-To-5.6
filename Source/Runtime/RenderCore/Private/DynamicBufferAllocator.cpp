...
FAutoConsoleVariableRef CVarReadBufferD3D11StaticAllocation(
	TEXT("r.ReadBuffer.D3D11StaticAllocation"),
...
FGlobalDynamicReadBuffer::FAllocation FGlobalDynamicReadBuffer::AllocateInternal(FDynamicReadBufferPool* BufferPool, uint32 Num)
{
	...
	if (Buffer == nullptr || ByteOffset + SizeInBytes > Buffer->NumBytes)
	{
		...
		if (Buffer == nullptr)
		{
			...
			// Cannot have the MinReadBufferRenderingBufferSize be greater than MaxViewDimensionForTypedBuffer for that platform
			const uint32 AdjustedMinReadBufferRenderingBufferSize = FMath::Min<uint32>(GMinReadBufferRenderingBufferSize, MaxViewDimensionForTypedBuffer);
			const uint32 NewBufferSize = FMath::Max(AlignedNum, AdjustedMinReadBufferRenderingBufferSize);
			if (UNLIKELY(NewBufferSize > MaxViewDimensionForTypedBuffer))
			{
				UE_LOG(LogRendererCore, Warning, TEXT("FGlobalDynamicReadBuffer::AllocateInternal, trying to allocate a Buffer of size (in elements) %d that is greater than the platform max of %llu"), NewBufferSize, MaxViewDimensionForTypedBuffer);
			}
			Buffer = new FDynamicAllocReadBuffer();
			BufferPool->Buffers.Add(Buffer);
			// Use static in D3D11 because we're running out of zeroed pages, and that causes the upload of this buffer to take a very long time. Static uses more memory and might be slightly less efficient
			// on the GPU, but it beats stalling for tens of milliseconds when committing the buffer.
			EBufferUsageFlags Flags = BUF_Dynamic;
			if (GReadBufferD3D11StaticAllocation && GDynamicRHI->GetInterfaceType() == ERHIInterfaceType::D3D11)
			{
				Flags = BUF_Static;
			}
			Buffer->Initialize(*RHICmdList, TEXT("FGlobalDynamicReadBuffer_AllocateInternal"), sizeof(Type), NewBufferSize, Format, Flags);
		}
...
void FGlobalDynamicReadBuffer::IncrementTotalAllocations(uint32 Num)
{
	...
	{
		UE_LOG(LogRendererCore, Warning, TEXT("FGlobalReadBuffer::AllocateInternal(%u), will have allocated %" SIZE_T_FMT " total this frame"), Num, TotalAllocatedSinceLastCommit);
	}
}
