...
bool dtCreateNavMeshData(dtNavMeshCreateParams* params, unsigned char** outData, int* outDataSize)
{
	...
	if (params->offMeshConCount > 0)
	{
		...
		// This is to fix an issues observed with dtIntersectSegmentPoly2D. The poly verts need to be in clockwise order.
		dtReal bverts[3*4];
		bverts[0] = bmin[0]; bverts[2] = bmax[2];
		bverts[3] = bmax[0]; bverts[5] = bmax[2];
		bverts[6] = bmax[0]; bverts[8] = bmin[2];
		bverts[9] = bmin[0]; bverts[11] = bmin[2];
