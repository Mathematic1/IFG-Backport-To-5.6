...
bool rcCreateHeightfield(rcContext* /*ctx*/, rcHeightfield& hf, int width, int height,
						 ...
						 rcReal cs, rcReal ch, bool bAllocateTempSpanColumns)
{
...
//	rcAssert(ctx);
	...
	if (bAllocateTempSpanColumns)
	{
		hf.tempSpanColumns = (rcSpanData*)rcAlloc(sizeof(rcSpanData)*hf.width*hf.height, RC_ALLOC_PERM); 
		if (!hf.tempSpanColumns)
			return false;
		memset(hf.tempSpanColumns, 0, sizeof(rcSpanData)*hf.width*hf.height);
	}
#endif // EPIC_ADDITION_USE_NEW_RECAST_RASTERIZER
