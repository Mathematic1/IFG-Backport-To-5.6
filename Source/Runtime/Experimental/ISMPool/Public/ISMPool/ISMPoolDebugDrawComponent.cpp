...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ISMPoolDebugDrawComponent)

static TAutoConsoleVariable<int32> CVarISMPoolStats(
	TEXT("r.ISMPool.DebugStats"),
	0,
	TEXT("Show stats for the ISM pools"),
	FConsoleVariableDelegate::CreateLambda([](IConsoleVariable* InVariable) { UISMPoolDebugDrawComponent::UpdateAllTickEnabled(); }),
	ECVF_Default);

static TAutoConsoleVariable<int32> CVarISMPoolDebugDraw(
	TEXT("r.ISMPool.DebugDraw"),
	0,
	TEXT("Show debug drawing for the ISM pools"),
	FConsoleVariableDelegate::CreateLambda([](IConsoleVariable* InVariable) { UISMPoolDebugDrawComponent::UpdateAllTickEnabled(); FGlobalComponentRecreateRenderStateContext Context; }),
	ECVF_Default);


class FISMPoolDebugDrawSceneProxy final : public FDebugRenderSceneProxy
{
public:
	...
	FISMPoolDebugDrawSceneProxy(UISMPoolDebugDrawComponent const* InComponent)
...
UISMPoolDebugDrawComponent::UISMPoolDebugDrawComponent(const FObjectInitializer& ObjectInitializer)
...
{
	for (TObjectIterator<UISMPoolDebugDrawComponent> It; It; ++It)
	{
...
void UISMPoolDebugDrawComponent::BeginPlay()
{
	...
	OnScreenMessagesHandle = FCoreDelegates::OnGetOnScreenMessages.AddUObject(this, &UISMPoolDebugDrawComponent::GetOnScreenMessages);
#endif
	UpdateTickEnabled();
}
...
#if WITH_EDITOR
void UISMPoolDebugDrawComponent::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);

	if (PropertyChangedEvent.Property)
	{
		if (PropertyChangedEvent.Property->GetFName() == GET_MEMBER_NAME_CHECKED(UISMPoolDebugDrawComponent, bShowStats) ||
			PropertyChangedEvent.Property->GetFName() == GET_MEMBER_NAME_CHECKED(UISMPoolDebugDrawComponent, bShowBounds))
		{
			UpdateTickEnabled();
		}
	}
...
void UISMPoolDebugDrawComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
FDebugRenderSceneProxy* UISMPoolDebugDrawComponent::CreateDebugSceneProxy()
{
	const bool bForceShowBounds = CVarISMPoolDebugDraw.GetValueOnAnyThread() != 0;
	...
	FDebugRenderSceneProxy* DebugProxy = new FISMPoolDebugDrawSceneProxy(this);
...
void UISMPoolDebugDrawComponent::GetOnScreenMessages(TMultiMap<FCoreDelegates::EOnScreenMessageSeverity, FText>& OutMessages)
{
	const bool bForceShowStats = CVarISMPoolStats.GetValueOnAnyThread() != 0;
	...
	{
		AISMPoolActor const* ISMPoolActor = Cast<AISMPoolActor>(GetOuter());
		UISMPoolComponent const* ISMPoolComponent = ISMPoolActor ? ISMPoolActor->GetISMPoolComp() : nullptr;
		...
		if (ISMPoolComponent)
		{
			...
			for (FISMPoolISM const& ISM : ISMPoolComponent->Pool.ISMs)
			{
