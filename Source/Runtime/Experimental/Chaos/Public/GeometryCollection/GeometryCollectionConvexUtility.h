...
UENUM(meta=(DisplayName="Allow Convex Bone Merge Method"))
enum class EAllowConvexMergeMethod : uint8
{
	// Only allow merging convex hulls of bones that are in proximity
	ByProximity UMETA(DisplayName = "By Bone Proximity"),
...
{
	// Do not require convex hull geometry to be in proximity to merge
	...
	None,
	// Only consider convex hull merges if their bounding boxes are in proximity
	BoundingBox
};
...
public:
	...
	struct FClusterConvexHullSettings
	{
		...
		// Option to apply the same hull merging settings to leaf hulls
		bool bAllowMergingLeafHulls = false;
		// How to allow merges between hulls on different bones
		EAllowConvexMergeMethod AllowMergesMethod = EAllowConvexMergeMethod::ByProximity;
		// How to filter by proximity for all merges (including within bones)
		EConvexHullProximityFilter ProximityFilter = EConvexHullProximityFilter::None;
		float ProximityDistanceThreshold = 0.1f;
	...
	struct FConvexDecompositionSettings
	{
		...
		// How much to thicken a hull on failure to construct -- to allow 3D decomposition of planar or degenerate meshes and parts-of-meshes
		//~ Note Chaos::FConvex has its own process to thicken hulls that are smaller than .1 units across (specified by Chaos::TriQuadPrismInflation)
		//~ Applying some smaller thickening earlier in the decomposition process just allows degenerate parts to make it through the decomposition.
	...
	// Computed sphere covering, and optional associated transform/bone
	...
	static CHAOS_API void GenerateLeafConvexHulls(
	 ...
	 * @param GeometryCollection The collection to copy convex hulls to
	 * @param TransformIndices The transforms on the GeometryCollection to update w/ copied convex hulls. Must be same length as SourceTransformIndices array.
	 * @param SourceCollection The collection to copy convex hulls from
	 * @param SourceTransformIndices The transforms on the SourceCollection to copy from. Must be same length as TransformIndices array.
	 * @param bSkipIfEmpty Whether to skip copying from transforms that have no convex hulls
	 */
	static CHAOS_API bool CopyConvexHulls(FManagedArrayCollection& Collection, const TArray<int32>& TransformIndices,
		const FManagedArrayCollection& SourceCollection, const TArray<int32>& SourceTransformIndices, bool bSkipIfEmpty = false);
