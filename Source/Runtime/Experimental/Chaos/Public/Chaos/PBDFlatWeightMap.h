...
public:
	...
	*/
	...
	inline void ReorderIndicesAndShrink(const TArray<int32>& OrigToReorderedIndices, int32 NewNum);
	void ReorderIndices(const TArray<int32>& OrigToReorderedIndices)
	{
		return ReorderIndicesAndShrink(OrigToReorderedIndices, MapValues.Num());
	}

private:
	static TArray<FSolverReal> CalculateMapValues(const TConstArrayView<FRealSingle>& Multipliers, int32 ParticleCount, const FSolverVec2& MultiplierClampRange)
	{
		...
		if (Multipliers.Num() == ParticleCount && ParticleCount > 0)
		{
			...
			{
				Result[Index] = FMath::Clamp(Multipliers[Index], MultiplierClampRange[0], MultiplierClampRange[1]);
			}
		}
		return Result;
	}
...
inline FPBDFlatWeightMap::FPBDFlatWeightMap(
	...
	const TConstArrayView<FRealSingle>& Multipliers,
	int32 ParticleCount,
	const FSolverVec2& MultiplierClampRange)
	: MapValues(CalculateMapValues(Multipliers, ParticleCount, MultiplierClampRange))
...
inline FPBDFlatWeightMap::FPBDFlatWeightMap(
	const FSolverVec2& InWeightedValue,
	TArray<FRealSingle>&& Multipliers)
	: MapValues(MoveTemp(Multipliers))
	, MapView(InWeightedValue, MapValues, MapValues.Num())
{}
...
inline void FPBDFlatWeightMap::ReorderIndicesAndShrink(const TArray<int32>& OrigToReorderedConstraintIndices, int32 NewNum)
{
	if (MapValues.Num() == OrigToReorderedConstraintIndices.Num())
	{
		TArray<FSolverReal> ReorderedValues;
		ReorderedValues.SetNumUninitialized(NewNum);
		for (int32 OrigConstraintIndex = 0; OrigConstraintIndex < OrigToReorderedConstraintIndices.Num(); ++OrigConstraintIndex)
		{
			...
			if (ReorderedConstraintIndex != INDEX_NONE)
			{
