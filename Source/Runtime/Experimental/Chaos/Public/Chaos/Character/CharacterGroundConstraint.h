...
namespace Chaos
{
	...
	public:
		...
		/// This is a fractional value used to scale the ground mass in the motion target constraint.
		/// It controls how much tangential force is applied to the ground when the character is moving.
		/// 1 means the full reaction force is applied
		/// 0 means zero reaction force is applied
		void SetMotionTargetMassBias(const FReal& Value)
		{
			ConstraintSettings.Modify(true, DirtyFlags, Proxy, [&Value](FCharacterGroundConstraintSettings& Data) {
				Data.MotionTargetMassBias = FMath::Clamp(Value, 0.0, 1.0);
				});
		}
		FReal GetMotionTargetMassBias() const { return ConstraintSettings.Read().MotionTargetMassBias; }

		/// This is a fractional value used to scale the radial force so that the character can achieve
		/// the motion target
		/// 1 means full scaling is applied and the character will always be able to achieve the motion target
		/// 0 means no scaling is applied to the radial force limit
		void SetRadialForceMotionTargetScaling(const FReal& Value)
		{
			ConstraintSettings.Modify(true, DirtyFlags, Proxy, [&Value](FCharacterGroundConstraintSettings& Data) {
				Data.RadialForceMotionTargetScaling = FMath::Clamp(Value, 0.0, 1.0);
				});
		}
		FReal GetRadialForceMotionTargetScaling() const { return ConstraintSettings.Read().RadialForceMotionTargetScaling; }
		...
		/// Maximum ratio between the character mass and the ground body mass as used by the constraint solver
		/// E.g. a value of 2 means that the character can be at most twice as heavy as the ground body and
		/// if it is over that the constraint solve will inflate the mass of the ground body to reach a 2:1 ratio
		/// This is useful for stability when walking on a light mass.
		/// A negative value disables this mass conditioning feature.
		void SetMaxCharacterGroundMassRatio(const FReal& Value)
		{
			ConstraintSettings.Modify(true, DirtyFlags, Proxy, [&Value](FCharacterGroundConstraintSettings& Data) {
				Data.MaxCharacterGroundMassRatio = Value;
				});
		}
		FReal GetMaxCharacterGroundMassRatio() const { return ConstraintSettings.Read().MaxCharacterGroundMassRatio; }
