...
namespace Chaos
{
	...
	public:
		...
		void SetData(const FCharacterGroundConstraintDynamicData& InData)
		{
			...
			SetDirtyFlag(EDirtyDataFlags::Data);
		}

		void SetSettings(const FCharacterGroundConstraintSettings& InSettings)
		{
			Settings = InSettings;
			SetDirtyFlag(EDirtyDataFlags::Settings);
		}
		...
		void SetGroundParticle(FGeometryParticleHandle* InGroundParticle)
		{
			if (InGroundParticle != GroundParticle)
			{
				if (InGroundParticle)
				{
					if (FPBDRigidParticleHandle* PBDRigid = InGroundParticle->CastToRigidParticle())
					{
						if (PBDRigid->Disabled())
						{
							InGroundParticle = nullptr;
						}
					}
				}
				...
				SetDirtyFlag(EDirtyDataFlags::GroundParticle);
			}
		}
		...
		{
			return IsFlagSet(EDirtyDataFlags::GroundParticle);
		}

		bool HaveSettingsChanged() const
		{
			return IsFlagSet(EDirtyDataFlags::Settings);
		}

		bool HasDataChanged() const
		{
			return IsFlagSet(EDirtyDataFlags::Data);
		}
	...
	private:
		...
		enum class EDirtyDataFlags : uint8
		{
			GroundParticle	= 1 << 0,
			Settings		= 1 << 1,
			Data			= 1 << 2
		...
		uint8 DirtyFlags;
		bool IsFlagSet(EDirtyDataFlags Flag) const 
		{
			return (DirtyFlags & (uint8)Flag) == (uint8)Flag;
		}
		void SetDirtyFlag(EDirtyDataFlags Flag)
		{
			DirtyFlags |= (uint8)Flag;
		}
