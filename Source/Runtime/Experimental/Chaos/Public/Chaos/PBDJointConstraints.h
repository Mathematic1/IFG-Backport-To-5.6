...
namespace Private
	...
	*/
	...
	public:
		...
		void SetConstraintContainer(FPBDConstraintContainer* NewConstraintContainer) { ConstraintContainer = NewConstraintContainer;  }
		bool IsInContainer(FPBDConstraintContainer* TestContainer) { return TestContainer == ConstraintContainer;  }
	...
	*/
	...
	public:
		...
		*/
		...
		// This will force a re-evaluation of the sorting of constraints - needed if objects switch
		// between kinematic/dynamic, for example.
		void MarkJointsDirty() { bJointsDirty = true; }

		// @todo(chaos): functionality no longer supported here (see TPBDJointContainerSolver)
		UE_DEPRECATED(5.2, "Joint Solver API moved to TPBDJointContainerSolver")
		...
		bool GetUseLinearSolver() { return bUseLinearSolver; }

		void SetUseLinearSolver(const bool bUseLinearSolverIn) { bUseLinearSolver = bUseLinearSolverIn; }

	private:
		friend class FPBDJointConstraintHandle;

		friend struct FPBDJointCombinedConstraints;
	...
	struct FPBDJointCombinedConstraints
	{
		...
		int32 NumConstraints() { return LinearConstraints.NumConstraints() + NonLinearConstraints.NumConstraints(); }
