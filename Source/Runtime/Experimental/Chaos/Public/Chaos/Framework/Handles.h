...
namespace Chaos
{
	...
	public:
		...
		struct TIterator
		{
			...
			explicit TIterator(ArrayType& InArray, int32 InIndex)
				: Array(InArray)
				, It(InArray.Validity, InIndex)
			{
			}
			...
			[[nodiscard]] FORCEINLINE bool operator==(const TIterator& Rhs) const
			{
			...
			FConstHandle GetConstHandle() const
			{
				...
				return Array.GetConstHandle(Index);
			}

			FHandle GetHandle() const
			{
				const int32 Index = It.GetIndex();
				return Array.GetHandle(Index);
			}

			[[nodiscard]] FORCEINLINE ItElementType& operator*() const
			{
				const FHandle Handle = GetHandle();
				ItElementType* Element = Array.Get(Handle);
				return *Element;
			}
		...
		{
			return TRangedForIterator(*this, 0);
		}

		TRangedForConstIterator begin() const
		{
			return TRangedForConstIterator(*this, 0);
		}

		TRangedForIterator end()
		{
			return TRangedForIterator(*this, (int32)Num());
		}

		TRangedForConstIterator end() const
		{
			return TRangedForConstIterator(*this, (int32)Num());
		}
