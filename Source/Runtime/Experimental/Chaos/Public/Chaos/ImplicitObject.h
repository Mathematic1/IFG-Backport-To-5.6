...
public:
	...
	virtual FReal GetRadius() const { return 0.0; }
	virtual FRealSingle GetRadiusf() const { return 0.0f; }
	 UE_DEPRECATED(5.6, "Please use GetMarginf instead.")
	virtual FReal GetMargin() const { return Margin; }
	virtual FRealSingle GetMarginf() const { return Margin; }
	...
	virtual Chaos::FImplicitObjectPtr DeepCopyGeometry() const { return CopyGeometry(); } 
	virtual Chaos::FImplicitObjectPtr DeepCopyGeometryWithScale(const FVec3& Scale) const { return CopyGeometryWithScale(Scale); }
...
protected:
	...
	void SetMargin(FReal InMargin) { Margin = static_cast<FRealSingle>(InMargin); }
	void SetMargin(FRealSingle InMargin) { Margin = InMargin; }
	...
	static_assert(alignof(FRealSingle) <= alignof(FChaosRefCountedObject)); // Otherwise, padding will be added before Margin
	static_assert(alignof(bool) <= sizeof(FRealSingle)); // Otherwise, padding would be added between Margin the first bool field
	...
	static constexpr int AlignOfFImplicitObject = (int)FMath::Max(alignof(FChaosRefCountedObject), FMath::Max(alignof(FRealSingle), FMath::Max(alignof(bool), alignof(EImplicitObjectType))));
	static constexpr int PadBytes =
		AlignOfFImplicitObject - (sizeof(FChaosRefCountedObject) + sizeof(FRealSingle) + 4 * sizeof(bool) + 2 * sizeof(EImplicitObjectType)) % AlignOfFImplicitObject;
