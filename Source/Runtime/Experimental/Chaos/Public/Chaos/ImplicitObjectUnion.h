...
class FImplicitObjectUnion : public FImplicitObject
{
  public:
	...
	// Recurively locked the union. Once locked, it is an error to make a change to the Union or any children
	void SetLocked(const bool bInLocked)
	{
		Flags.bIsLocked = bInLocked;
	}

	void SetLockedRecursive(const bool bInLocked)
	{
		if (bInLocked != IsLocked())
		{
			for (FImplicitObject* Object : MObjects)
			{
				if (FImplicitObjectUnion* Union = Object->template GetObject<FImplicitObjectUnion>())
				{
					Union->SetLockedRecursive(bInLocked);
				}
			}
	...
	const TArray<Chaos::FImplicitObjectPtr>& GetObjects() const { return MObjects; }

	// Return the const list of objects that will be part of the union
	TArray<Chaos::FImplicitObjectPtr>& GetObjects() { return MObjects; }
  ...
  protected:
	...
	public:
		...
		struct
		{
			...
			uint8 bIsLocked : 1;				// If the union is locked, it is an error to change children
		};
		uint8 Bits;
	};
	...
	// Log an error we try to modify a union after it has been shared
