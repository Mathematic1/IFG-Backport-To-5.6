...
namespace Chaos
{
	...
	public:
		...
		bool VisitSweep(int32 Payload, FReal& CurrentLength)
		{
				...
				if(GJKRaycast2ImplSimd(Triangle, OtherGeom, Rotation, Translation, DirSimd, static_cast<FRealSingle>(CurrentLength), Time, OutPositionSimd, OutNormalSimd, bComputeMTD, GlobalVectorConstants::Float1000))
				{
					if(Time < OutTime)
					{
						...
						if(!bHole)
						{
							...
							// If there's an initial overlap, we can't terminate right away as the first hit might not be the "best" (min distance). 
							// We have to process all initial overlaps no matter what, however we can still update CurrentLength to not process non-initial overlaps.
							if(Time <= 0) //initial overlap or MTD, so stop
							{
								...
								CurrentLength = UE_KINDA_SMALL_NUMBER;
								return true;
							}
			...
			// There is no way to early terminate, even if there is an initial overlap since another cell might have a "better" initial overlap.
			return true;
		}
	...
	public:
		...
		bool VisitSweep(int32 CellIndex, FReal& CurrentLength)
		{
			...
			auto TestTriangle = [&](int32 FaceIndex, const VectorRegister4Float& A, const VectorRegister4Float& B, const VectorRegister4Float& C) -> bool
			{
				...
				VectorRegister4Float RadiusV = VectorSetFloat1(OtherGeom.GetRadiusf());
	...
	{
		auto ConversionFunc = [](const FReal InVal) -> FReal
		{
			return InVal;
		};

		BuildGeomData<FReal>(MakeArrayView(Height), MakeArrayView(InMaterialIndices), NumRows, NumCols, FVec3(1), ConversionFunc, GeomData, LocalBounds);
		CalcBounds();
		SetScale(InScale);
	}
	...
	Chaos::FHeightField::FHeightField(TArrayView<const uint16> InHeights, TArrayView<const uint8> InMaterialIndices, int32 InNumRows, int32 InNumCols, const FVec3& InScale)
		: FImplicitObject(EImplicitObject::HasBoundingBox, ImplicitObjectType::HeightField)
	{
		auto ConversionFunc = [](const uint16 InVal) -> FReal
		{
			return (FReal)((int32)InVal - 32768);
		};

		BuildGeomData<const uint16>(InHeights, InMaterialIndices, InNumRows, InNumCols, FVec3(1), ConversionFunc, GeomData, LocalBounds);
		CalcBounds();
		SetScale(InScale);
	}
	...
	void Chaos::FHeightField::EditHeights(TArrayView<const uint16> InHeights, int32 InBeginRow, int32 InBeginCol, int32 InNumRows, int32 InNumCols)
	{
		...
		if(ensure(InHeights.Num() == NumExpectedValues && InBeginRow >= 0 && InBeginCol >= 0 && EndRow < GeomData.NumRows && EndCol < GeomData.NumCols))
		{
			...
			EditGeomData<const uint16>(InHeights, InBeginRow, InBeginCol, InNumRows, InNumCols, ConversionFunc, GeomData, LocalBounds);
	...
	void Chaos::FHeightField::EditHeights(TArrayView<FReal> InHeights, int32 InBeginRow, int32 InBeginCol, int32 InNumRows, int32 InNumCols)
	{
		...
		if(ensure(InHeights.Num() == NumExpectedValues && InBeginRow >= 0 && InBeginCol >= 0 && EndRow < GeomData.NumRows && EndCol < GeomData.NumCols))
		{
			...
			EditGeomData<FReal>(InHeights, InBeginRow, InBeginCol, InNumRows, InNumCols, ConversionFunc, GeomData, LocalBounds);
	...
	**/
	...
	bool FHeightField::GJKContactPoint(const FSphere& QueryGeom, const FRigidTransform3& QueryTM, const FReal Thickness, FVec3& ContactLocation, FVec3& ContactNormal, FReal& ContactPhi, int32& ContactFaceIndex) const
	{
		return GJKContactPointImp(QueryGeom, QueryTM, Thickness, ContactLocation, ContactNormal, ContactPhi, ContactFaceIndex);
	}
	...
	bool FHeightField::GJKContactPoint(const TImplicitObjectScaled < FSphere>& QueryGeom, const FRigidTransform3& QueryTM, const FReal Thickness, FVec3& ContactLocation, FVec3& ContactNormal, FReal& ContactPhi, int32& ContactFaceIndex) const
	{
		return GJKContactPointImp(QueryGeom, QueryTM, Thickness, ContactLocation, ContactNormal, ContactPhi, ContactFaceIndex);
	}
	...
	bool FHeightField::OverlapGeomImp(const QueryGeomType& QueryGeom, const FRigidTransform3& QueryTM, const FReal Thickness, FMTDInfo* OutMTD) const
	{
		...
		auto OverlapTriangleNoMTD = [&](const VectorRegister4Float& A, const VectorRegister4Float& B, const VectorRegister4Float& C) -> bool
		{
			...
			return GJKIntersectionSameSpaceSimd(TriangleConvex, QueryGeom, FRealSingle(Thickness), VectorNormalizeSafe(Offset, GlobalVectorConstants::Float1000));
		};
		...
		if (GetGridIntersectionsBatch(FlatQueryBounds, Intersections, QueryBoundsSimd))
		{
			...
			else
			{
				...
				{
					Radius = QueryGeom.GetRadiusf();
					const FVec3f X1f = QueryGeom.GetX1f();
					const FVec3f X2f = QueryGeom.GetX2f();
				...
				{
					Radius = QueryGeom.GetRadiusf();
					const FVec3f X1f = QueryGeom.GetUnscaledObject()->GetX1f() * QueryGeom.GetScale();
					const FVec3f X2f = QueryGeom.GetUnscaledObject()->GetX2f() * QueryGeom.GetScale();
					X1 = VectorLoadFloat3(&X1f.X);
					X2 = VectorLoadFloat3(&X2f.X);
				}
				...
				{
					Radius = QueryGeom.GetRadiusf();
					const FVec3f X1f = QueryGeom.GetCenterf();
					X1 = VectorLoadFloat3(&X1f.X);
				}
				else if constexpr (std::is_same<QueryGeomType, TImplicitObjectScaled < Chaos::FSphere>>::value)
				{
					...
					const FVec3f X1f = QueryGeom.GetUnscaledObject()->GetCenterf() * QueryGeom.GetScale();
					X1 = VectorLoadFloat3(&X1f.X);
				}
	...
	bool FHeightField::OverlapGeom(const FSphere& QueryGeom, const FRigidTransform3& QueryTM, const FReal Thickness, FMTDInfo* OutMTD) const
	{
	...
	bool FHeightField::OverlapGeom(const TImplicitObjectScaled<FSphere>& QueryGeom, const FRigidTransform3& QueryTM, const FReal Thickness, FMTDInfo* OutMTD) const
	{
		return OverlapGeomImp(QueryGeom, QueryTM, Thickness, OutMTD);
	}
	...
	bool FHeightField::SweepGeom(const FSphere& QueryGeom, const FRigidTransform3& StartTM, const FVec3& Dir, const FReal Length, FReal& OutTime, FVec3& OutPosition, FVec3& OutNormal, int32& OutFaceIndex, FVec3& OutFaceNormal, const FReal Thickness, bool bComputeMTD) const
	{
	...
	bool FHeightField::SweepGeom(const TImplicitObjectScaled<FSphere>& QueryGeom, const FRigidTransform3& StartTM, const FVec3& Dir, const FReal Length, FReal& OutTime, FVec3& OutPosition, FVec3& OutNormal, int32& OutFaceIndex, FVec3& OutFaceNormal, const FReal Thickness, bool bComputeMTD) const
	{
		return SweepGeomImp(QueryGeom, StartTM, Dir, Length, OutTime, OutPosition, OutNormal, OutFaceIndex, Thickness, bComputeMTD);
	}
	...
	bool FHeightField::SweepGeomCCD(const FSphere& QueryGeom, const FRigidTransform3& StartTM, const FVec3& Dir, const FReal Length, const FReal IgnorePenetration, const FReal TargetPenetration, FReal& OutTOI, FReal& OutPhi, FVec3& OutPosition, FVec3& OutNormal, int32& OutFaceIndex, FVec3& OutFaceNormal) const
	{
	...
	bool FHeightField::SweepGeomCCD(const TImplicitObjectScaled<FSphere>& QueryGeom, const FRigidTransform3& StartTM, const FVec3& Dir, const FReal Length, const FReal IgnorePenetration, const FReal TargetPenetration, FReal& OutTOI, FReal& OutPhi, FVec3& OutPosition, FVec3& OutNormal, int32& OutFaceIndex, FVec3& OutFaceNormal) const
	{
		return SweepGeomCCDImp(QueryGeom, StartTM, Dir, Length, IgnorePenetration, TargetPenetration, OutTOI, OutPhi, OutPosition, OutNormal, OutFaceIndex);
	}
