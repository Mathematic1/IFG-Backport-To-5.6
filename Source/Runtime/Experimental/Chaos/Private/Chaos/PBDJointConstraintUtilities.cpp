...
namespace Chaos
{
	...
	void FPBDJointUtilities::GetSwingTwistAngles(const FRotation3& R0, const FRotation3& R1, FReal& TwistAngle, FReal& Swing1Angle, FReal& Swing2Angle)
	{
		...
		TwistAngle = GetTwistAngle(R01Twist);
	...
	bool FPBDJointUtilities::GetLinearIsFree(const FPBDJointSettings& JointSettings)
	{
		return JointSettings.LinearMotionTypes == TVector<EJointMotionType, 3>(EJointMotionType::Free)
			&& JointSettings.bLinearPositionDriveEnabled == TVector<bool, 3>(false) && bExcludeFreeJointInertiaConditioning;
	}
}
