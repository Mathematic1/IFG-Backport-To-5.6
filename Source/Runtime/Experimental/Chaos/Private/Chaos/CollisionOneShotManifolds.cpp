...
namespace Chaos
{
	...
	// Whether to sort sphere/capsule/convex vs mesh contacts by depth. Doing so leads to more consistent behaviour but is more expensive
	...
	// Whether to sort sphere/capsule/convex vs mesh contacts by distance from center of mass in the contact plane. Slow but better for convergence.
	...
	namespace Collisions
	{
			...
			FContactPointManifold& OutContactPoints)
		{
			...
			const FReal CapsuleRadius = Capsule.GetRadiusf();
			const FReal CapsuleHeight = Capsule.GetHeightf();
			const FLineSegment3 CapsuleAxisSegment = FLineSegment3(
				CapsuleToConvexTransform.TransformPositionNoScale(FVec3(Capsule.GetX1f())),
				CapsuleToConvexTransform.TransformVectorNoScale(FVec3(Capsule.GetAxisf())),
				CapsuleHeight);
		...
		void ConstructPlanarConvexTriangleOneShotManifold(const ConvexType& Convex, const FTriangle& Triangle, const FReal CullDistance, FContactPointManifold& OutContactPoints)
		{
			...
			const FReal ConvexMargin = MarginMultiplier * FReal(Convex.GetMarginf());
			FReal MaxMarginDelta = FReal(0);
			int32 VertexIndexA = INDEX_NONE, VertexIndexB = INDEX_NONE;
