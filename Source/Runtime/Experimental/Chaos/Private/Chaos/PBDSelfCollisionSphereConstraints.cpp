...
namespace Chaos::Softs
{
	namespace Private
	{
		static int32 Chaos_SelfCollisionSphere_ParallelCount = 256;
		static FAutoConsoleVariableRef CVarChaosSelfCollisionSphereParallelismEnabled(TEXT("p.Chaos.SelfCollisionSphere.ParallelCount"), Chaos_SelfCollisionSphere_ParallelCount, TEXT("Min number of spheres to enable parallelism for SelfCollisionSpheres constraint"));
	...
	void FPBDSelfCollisionSphereConstraintsBase::Init(const SolverParticlesOrRange& Particles)
	{
		...
		if (NumParticles == 0 || (VertexSetNoOffset && VertexSetNoOffset->IsEmpty()) || Stiffness <= 0.f)
		{
			return;
		}
		...
		SpatialHash.InitializePoints(Entries, Entries.Num() < Private::Chaos_SelfCollisionSphere_ParallelCount);
			...
			[this, &Particles, DiamSq, &ReferencePositionsView](const int32 i1, const int32 i2)
		{
			...
			return true;
		}, Entries.Num() < Private::Chaos_SelfCollisionSphere_ParallelCount
		);
	}
	...
	{
		if (IsSelfCollisionSphereRadiusMutable(PropertyCollection))
		{
			Radius = (FSolverReal)FMath::Max(GetSelfCollisionSphereRadius(PropertyCollection), 0.f);
		}
		if (IsSelfCollisionSphereStiffnessMutable(PropertyCollection))
		{
