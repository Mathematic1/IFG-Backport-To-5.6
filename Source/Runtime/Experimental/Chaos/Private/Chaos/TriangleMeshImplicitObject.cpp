...
struct FTriangleMeshOverlapVisitorNoMTD
{
	FTriangleMeshOverlapVisitorNoMTD(const TRigidTransform<FReal, 3>& WorldScaleQueryTM, const QueryGeomType& InQueryGeom, FReal InThickness, const FVec3& InTriMeshScale, const FTriangleMeshImplicitObject* InTriMesh)
		...
		, Thickness(FRealSingle(InThickness))
		, TriMesh(InTriMesh)
...
struct FTriangleMeshOverlapVisitorNoMTD<FCapsule>
{
	FTriangleMeshOverlapVisitorNoMTD(const TRigidTransform<FReal, 3>& WorldScaleQueryTM, const FCapsule& InQueryGeom, FReal InThickness, const FVec3f& InTriMeshScale, const FTriangleMeshImplicitObject* InTriMesh)
		: Radius(InQueryGeom.GetRadiusf() + FRealSingle(InThickness))
		, TriMesh(InTriMesh)
		, bFoundIntersection(false)
	{
		const FVec3f X1f = WorldScaleQueryTM.TransformPosition(FVec3(InQueryGeom.GetX1f()));
		const FVec3f X2f = WorldScaleQueryTM.TransformPosition(FVec3(InQueryGeom.GetX2f()));
...
struct FTriangleMeshOverlapVisitorNoMTD<TImplicitObjectScaled<FCapsule>>
{
	FTriangleMeshOverlapVisitorNoMTD(const TRigidTransform<FReal, 3>& WorldScaleQueryTM, const TImplicitObjectScaled< FCapsule >& InQueryGeom, FReal InThickness, const FVec3f& InTriMeshScale, const FTriangleMeshImplicitObject* InTriMesh)
		...
		, bFoundIntersection(false)
	{
		const FVec3f X1f = WorldScaleQueryTM.TransformPosition(FVec3(InQueryGeom.GetUnscaledObject()->GetX1f() * InQueryGeom.GetScale()));
		const FVec3f X2f = WorldScaleQueryTM.TransformPosition(FVec3(InQueryGeom.GetUnscaledObject()->GetX2f() * InQueryGeom.GetScale()));
		X1 = VectorLoadFloat3(&X1f.X);
		X2 = VectorLoadFloat3(&X2f.X);
		TriMeshScale = VectorLoadFloat3(&InTriMeshScale.X);
	}
...
struct FTriangleMeshOverlapVisitorNoMTD<Chaos::FSphere>
{
	FTriangleMeshOverlapVisitorNoMTD(const TRigidTransform<FReal, 3>& WorldScaleQueryTM, const Chaos::FSphere& InQueryGeom, FReal InThickness, const FVec3f& InTriMeshScale, const FTriangleMeshImplicitObject* InTriMesh)
		...
		, bFoundIntersection(false)
	{
		const FVec3f Xf = WorldScaleQueryTM.TransformPosition(FVec3(InQueryGeom.GetCenterf()));
...
struct FTriangleMeshOverlapVisitorNoMTD<TImplicitObjectScaled<Chaos::FSphere>>
{
	FTriangleMeshOverlapVisitorNoMTD(const TRigidTransform<FReal, 3>& WorldScaleQueryTM, const TImplicitObjectScaled< Chaos::FSphere >& InQueryGeom, FReal InThickness, const FVec3f& InTriMeshScale, const FTriangleMeshImplicitObject* InTriMesh)
		...
		, bFoundIntersection(false)
	{
		const FVec3f Xf = WorldScaleQueryTM.TransformPosition(FVec3(InQueryGeom.GetUnscaledObject()->GetCenterf()) * InQueryGeom.GetScale());
		X = VectorLoadFloat3(&Xf.X);
		TriMeshScale = VectorLoadFloat3(&InTriMeshScale.X);
		FReal R = InQueryGeom.GetRadius();
		FRealSingle Rf = InQueryGeom.GetRadiusf();
		ensure(Rf == R);
	}
...
const FTrimeshBVH& FTriangleMeshImplicitObject::GetBVH() const
{
	return FastBVH;
}
...
bool FTriangleMeshImplicitObject::GJKContactPoint(const FSphere& QueryGeom, const FRigidTransform3& QueryTM, const FReal WorldThickness, FVec3& Location, FVec3& Normal, FReal& ContactPhi, int32& ContactFaceIndex) const
{
...
bool FTriangleMeshImplicitObject::GJKContactPoint(const TImplicitObjectScaled< FSphere >& QueryGeom, const FRigidTransform3& QueryTM, const FReal WorldThickness, FVec3& Location, FVec3& Normal, FReal& ContactPhi, int32& ContactFaceIndex, FVec3 TriMeshScale) const
{
...
bool FTriangleMeshImplicitObject::OverlapGeom(const FSphere& QueryGeom, const FRigidTransform3& QueryTM, const FReal Thickness, FMTDInfo* OutMTD) const
{
...
bool FTriangleMeshImplicitObject::OverlapGeom(const TImplicitObjectScaled<FSphere>& QueryGeom, const FRigidTransform3& QueryTM, const FReal Thickness, FMTDInfo* OutMTD, FVec3 TriMeshScale) const
{
...
struct FTriangleMeshSweepVisitor
{
	...
	bool VisitSweep(const TSpatialVisitorData<int32>& VisitData, FRealSingle& CurDataLength)
	{
		...
		bool bIsParallelSweepAndCullsBackFace = false;
		if (CullsBackFaceSweepsCode != 0)
		{
			// For the parallel epsilon check to be consistent, we must normalize the triangle normal
			const VectorRegister4Float ReciprocalNormalLen = VectorReciprocalLen(TriNormal);
			const VectorRegister4Float NormalizedNormal = VectorMultiply(TriNormal, ReciprocalNormalLen);
			const VectorRegister4Float NDotD = VectorDot3(NormalizedNormal, VectorScaledDirNormalized);

			const VectorRegister4Float PositiveEpsilon = MakeVectorRegister(UE_KINDA_SMALL_NUMBER, UE_KINDA_SMALL_NUMBER, UE_KINDA_SMALL_NUMBER, UE_KINDA_SMALL_NUMBER);
			const VectorRegister4Float NegativeEpsilon = VectorNegate(PositiveEpsilon);

			// NDotD is only in the correct direction when all scale components are positive.
			// Here we correct it if we need to have changed the winding order of the triangle
			const VectorRegister4Float ScaleCorrectedNDotD = VectorMultiply(NDotD, VectorCullsBackFaceSweepsCode);

			// Now NDotD is correct we can see if we're approaching the front or back of the triangle
			const VectorRegister4Float IsAlongPositiveNormal = VectorCompareGT(ScaleCorrectedNDotD, PositiveEpsilon);
			const VectorRegister4Float IsAlongNegativeNormal = VectorCompareLT(ScaleCorrectedNDotD, NegativeEpsilon);
			const bool bIsBackFace = VectorMaskBits(IsAlongPositiveNormal) != 0;
			const bool bIsFrontFace = VectorMaskBits(IsAlongNegativeNormal) != 0;

			bIsParallelSweepAndCullsBackFace = (!bIsBackFace && !bIsFrontFace);

			// If this is a definitive back-face hit, always skip, even if there is an initial overlap.
			if (bIsBackFace)
			{
				return true;
			}
			// If this is parallel, we want to skip if there's no initial intersection. 
			// This is an early-out check to avoid calling GJK if there's no bounding volume intersection.
			if (bIsParallelSweepAndCullsBackFace)
			{
		...
		if(bIsHitting)
		{
			// Parallel sweeps should still be culled (if culls back-face is enabled) if there is no initial intersection. 
			// We still want to keep a sweep if there is initial intersection.
			if (Time > 0 && bIsParallelSweepAndCullsBackFace)
			{
				return true;
			}
			...
			if(Time < LengthScale * OutTime)
			{
				...
				// We have an initial overlap (MTD of 0 counts). We cannot terminate early as there might be a "better" hit (one with a more negative Time).
				if(Time <= 0)
				{
					// In an ideal world, we'd update CurrentLength to 0 so as to prevent getting any non-zero TOI intersections,
					// however GJKRaycast does not allow a ray length of 0. We could switch to GJKPenetration, 
					// however there is currently no SIMD implementation. To work around this, we can set CurrentLength to a small number instead.
					// Doing this will always produce the correct result (in the edge case we get a TOI between 0 and espilon, we'll still pick the smallest TOI).
					// This might be slightly slower in this edge case as GJK might've been able to terminate earlier.
					// #TODO Replace this pathway with a call to GJKPenetration when a SIMD version is created.
					CurDataLength = UE_KINDA_SMALL_NUMBER;
					return true;
				}
...
struct FTriangleMeshSweepVisitorCCD
{
	...
	bool VisitSweep(const TSpatialVisitorData<int32>& VisitData, FRealSingle& CurDataLength)
	{
		...
		VectorRegister4Float RadiusV = VectorSetFloat1(QueryGeom.GetRadiusf());
	...
	const bool bComputeMTD, FVec3 TriMeshScale) const
{
	...
	auto LambdaHelper = [&](const auto& Elements)
	{
		// When length is zero, the direction of the ray is currently a hardcoded value (since it's computed from the start/end which would be equal). 
		// We don't want the backface culling check to be arbitrary based upon the hardcoded direction, so we disable it with zero-length sweeps.
		const FReal CullsBackFaceRaycastCode = (bCullsBackFaceRaycast && Length != 0) ? GetWindingOrder(TriMeshScale) : 0.f;
...
bool FTriangleMeshImplicitObject::SweepGeom(const TImplicitObjectScaled<FSphere>& QueryGeom, const FRigidTransform3& StartTM, const FVec3& Dir, const FReal Length, FReal& OutTime, FVec3& OutPosition, FVec3& OutNormal, int32& OutFaceIndex, FVec3& OutFaceNormal, const FReal Thickness, const bool bComputeMTD, FVec3 TriMeshScale) const
{
	return SweepGeomImp(QueryGeom, StartTM, Dir, Length, OutTime, OutPosition, OutNormal, OutFaceIndex, OutFaceNormal, Thickness, bComputeMTD, TriMeshScale);
}
...
bool FTriangleMeshImplicitObject::SweepGeomCCD(const FSphere& QueryGeom, const FRigidTransform3& StartTM, const FVec3& Dir, const FReal Length, const FReal IgnorePenetration, const FReal TargetPenetration, FReal& OutTOI, FReal& OutPhi, FVec3& OutPosition, FVec3& OutNormal, int32& OutFaceIndex, FVec3& OutFaceNormal, const FVec3& TriMeshScale) const
{
...
bool FTriangleMeshImplicitObject::SweepGeomCCD(const TImplicitObjectScaled<FSphere>& QueryGeom, const FRigidTransform3& StartTM, const FVec3& Dir, const FReal Length, const FReal IgnorePenetration, const FReal TargetPenetration, FReal& OutTOI, FReal& OutPhi, FVec3& OutPosition, FVec3& OutNormal, int32& OutFaceIndex, FVec3& OutFaceNormal, const FVec3& TriMeshScale) const
{
	return SweepGeomCCDImp(QueryGeom, StartTM, Dir, Length, IgnorePenetration, TargetPenetration, OutTOI, OutPhi, OutPosition, OutNormal, OutFaceIndex, OutFaceNormal, TriMeshScale);
}
