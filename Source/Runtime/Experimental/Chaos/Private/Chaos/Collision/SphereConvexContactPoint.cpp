...
namespace Chaos
{
	...
	FContactPoint SphereConvexContactPointImpl(const FImplicitSphere3& Sphere, const ConvexType& Convex, const FRigidTransform3& SphereToConvexTransform)
	{
		if (Convex.NumPlanes() > 0)
		{
			...
			const FVec3 SpherePosConvex = SphereToConvexTransform.TransformPositionNoScale(FVec3(Sphere.GetCenterf()));
			...
			if (bHaveResult)
			{
				...
				PosSphere = Sphere.GetCenterf() + SphereToConvexTransform.InverseTransformVectorNoScale(-SphereRadius * NormalConvex);
