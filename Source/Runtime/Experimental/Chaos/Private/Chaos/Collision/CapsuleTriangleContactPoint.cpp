...
namespace Chaos
{
	...
	void ConstructCapsuleTriangleOneShotManifold2(const FImplicitCapsule3& Capsule, const FTriangle& Triangle, const FReal CullDistance, FContactPointManifold& OutContactPoints)
	{
		...
		// Capsule segment
		const FVec3 P0 = Capsule.GetX1f();
		const FVec3 P1 = Capsule.GetX2f();
		const FReal R = Capsule.GetRadiusf();
		const FVec3 Axis = Capsule.GetAxisf();
		const FReal L = Capsule.GetHeightf();
		...
		const FReal DistanceTolerance = FReal(1.e-5f * Capsule.GetHeightf());
