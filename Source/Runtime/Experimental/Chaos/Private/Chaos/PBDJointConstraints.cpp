...
namespace Chaos
{
	...
	void FPBDJointConstraintHandle::SetJointState(const FPBDJointState& NewState)
	{
		ConcreteContainer()->SetConstraintState(ConstraintIndex, NewState);
	}
	...
	TUniquePtr<FConstraintContainerSolver> FPBDJointConstraints::CreateSceneSolver(const int32 Priority)
	{
		if (bUseLinearSolver)
		{
			return MakeUnique<Private::TPBDJointContainerSolver<Chaos::FPBDJointCachedSolver>>(*this, Priority);
		}
		else
		{
			return MakeUnique<Private::TPBDJointContainerSolver<Chaos::FPBDJointSolver>>(*this, Priority);
		}
	}
	...
	void FPBDJointConstraints::SetConstraintState(int32 ConstraintIndex, const FPBDJointState& InState)
	{
		ConstraintStates[ConstraintIndex] = InState;
	}
	...
	void FPBDJointConstraints::DisconnectConstraints(const TSet<TGeometryParticleHandle<FReal, 3>*>& RemovedParticles)
	{
		for (TGeometryParticleHandle<FReal, 3>*RemovedParticle : RemovedParticles)
		{
			for (FConstraintHandle* ConstraintHandle : RemovedParticle->ParticleConstraints())
			{
				...
				{
					if (!JointHandle->IsInContainer(this))
					{
						continue;
					}
	...
	void FPBDJointConstraints::SortConstraints()
	{
		...
		// (so we should split TPBDJointContainerSolver into Scene and Group versions)
	...
	FPBDJointCombinedConstraints::FPBDJointCombinedConstraints()
	{
		LinearConstraints.SetUseLinearSolver(true);
		NonLinearConstraints.SetUseLinearSolver(false);
	}

	//This function moves one constraint info from linear constraint into nonlinear constraints
	void FPBDJointCombinedConstraints::ConvertLinearConstraintToNonLinear(const int32 LinearConstraintIndex)
	{
		if (!ensureMsgf(LinearConstraintIndex > INDEX_NONE && LinearConstraintIndex < LinearConstraints.NumConstraints(), TEXT("Linear constraint accessed is out of bounds!")))
		{
			return;
		}
		NonLinearConstraints.bJointsDirty = true;

		int NonLinearConstraintsNum = NonLinearConstraints.Handles.Num();
		NonLinearConstraints.Handles.Add(MoveTemp(LinearConstraints.Handles[LinearConstraintIndex]));
		NonLinearConstraints.ConstraintParticles.Add(MoveTemp(LinearConstraints.ConstraintParticles[LinearConstraintIndex]));
		NonLinearConstraints.ConstraintStates.Add(MoveTemp(LinearConstraints.ConstraintStates[LinearConstraintIndex]));
		NonLinearConstraints.ConstraintSettings.Add(MoveTemp(LinearConstraints.ConstraintSettings[LinearConstraintIndex]));
		NonLinearConstraints.ConstraintSettings[NonLinearConstraintsNum].bUseLinearSolver = false;
		NonLinearConstraints.Handles[NonLinearConstraintsNum]->SetConstraintIndex(NonLinearConstraintsNum);
		NonLinearConstraints.Handles[NonLinearConstraintsNum]->SetConstraintContainer(&NonLinearConstraints);

		LinearConstraints.Handles.RemoveAtSwap(LinearConstraintIndex);
		LinearConstraints.ConstraintParticles.RemoveAtSwap(LinearConstraintIndex);
		LinearConstraints.ConstraintStates.RemoveAtSwap(LinearConstraintIndex);
		LinearConstraints.ConstraintSettings.RemoveAtSwap(LinearConstraintIndex);
		LinearConstraints.bJointsDirty = true;
	}

	//This function moves one constraint info from nonlinear constraint into linear constraints
	void FPBDJointCombinedConstraints::ConvertNonLinearConstraintToLinear(const int32 NonLinearConstraintIndex)
	{
		if (!ensureMsgf(NonLinearConstraintIndex > INDEX_NONE && NonLinearConstraintIndex < NonLinearConstraints.NumConstraints(), TEXT("NonLinear constraint accessed is out of bounds!")))
		{
			return; 
		}

		LinearConstraints.bJointsDirty = true;
		int LinearConstraintsNum = LinearConstraints.Handles.Num();
		LinearConstraints.Handles.Add(MoveTemp(NonLinearConstraints.Handles[NonLinearConstraintIndex]));
		LinearConstraints.ConstraintParticles.Add(MoveTemp(NonLinearConstraints.ConstraintParticles[NonLinearConstraintIndex]));
		LinearConstraints.ConstraintStates.Add(MoveTemp(NonLinearConstraints.ConstraintStates[NonLinearConstraintIndex]));
		LinearConstraints.ConstraintSettings.Add(MoveTemp(NonLinearConstraints.ConstraintSettings[NonLinearConstraintIndex]));
		LinearConstraints.ConstraintSettings[LinearConstraintsNum].bUseLinearSolver = true;
		LinearConstraints.Handles[LinearConstraintsNum]->SetConstraintIndex(LinearConstraintsNum);
		LinearConstraints.Handles[LinearConstraintsNum]->SetConstraintContainer(&LinearConstraints);

		NonLinearConstraints.Handles.RemoveAtSwap(NonLinearConstraintIndex);
		NonLinearConstraints.ConstraintParticles.RemoveAtSwap(NonLinearConstraintIndex);
		NonLinearConstraints.ConstraintStates.RemoveAtSwap(NonLinearConstraintIndex);
		NonLinearConstraints.ConstraintSettings.RemoveAtSwap(NonLinearConstraintIndex);
		NonLinearConstraints.bJointsDirty = true;
	}

	void FPBDJointCombinedConstraints::MergeLinearConstraintsIntoNonLinearConstraints()
	{
		while (LinearConstraints.NumConstraints() > 0)
		{
			ConvertLinearConstraintToNonLinear(0);
		}
	}

	void FPBDJointCombinedConstraints::MergeNonLinearConstraintsIntoLinearConstraints()
	{
		while (NonLinearConstraints.NumConstraints() > 0)
		{
			ConvertNonLinearConstraintToLinear(0);
		}
	}
