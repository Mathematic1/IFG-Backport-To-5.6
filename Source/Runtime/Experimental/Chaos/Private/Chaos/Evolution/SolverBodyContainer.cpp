...
namespace Chaos
{
	inline void UpdateSolverBodyFromParticle(FSolverBody& SolverBody, const FGeometryParticleHandle* Particle, const FReal Dt)
	{
		...
		if ((RigidParticle != nullptr) && RigidParticle->IsDynamic())
		{
			...
			if (!RigidParticle->IsSleeping()) // Awake
			{
			...
			{
				// In the solver, we want sleeping particles to behave like kinematics
				// since they do not experience any positional impulse and position correction.
				// Thus, we set velocity, mass, and inertia to zero while using the position and orientation of the rigid particle.
	...
	FSolverBody* FSolverBodyContainer::FindOrAdd(FGenericParticleHandle InParticle)
	{
		...
		{
			// NOTE: IsDynamic() is true whether the particle is awake or asleep.
			// The SolverBodyIndex() for sleeping particles does not seem to be reset at each tick.
			// As a workaround, we use the same procedure as kinematic particles to make sure it is persistent.
			if (InParticle->IsDynamic() && !InParticle->IsSleeping())
			{
		...
		check(ItemIndex < SolverBodies.Num());
