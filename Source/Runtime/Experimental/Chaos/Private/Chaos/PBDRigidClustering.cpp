...
{
	extern CHAOS_API bool BuildGeometryForChildrenOnPT(const FSimulationParameters& Params);
}
...
namespace Chaos
{
	...
	namespace
	{
		...
		{
			const FChaosSolverDestructionSettings& DestructionSettings = Evolution.GetRigidClustering().GetDestructionSettings();
			...
			const int32 Range = FMath::Max(0, (DestructionSettings.ClusteringParticleReleaseThrottlingMaxCount - DestructionSettings.ClusteringParticleReleaseThrottlingMinCount));
			const int32 OverMinCount = FMath::Max(0, (NumActiveParticles - DestructionSettings.ClusteringParticleReleaseThrottlingMinCount));
		...
		{
			if (!Clustering.ShouldThrottleParticleRelease())
			{
				return;
			}
		...
		const FClusterCreationParameters& Parameters)
	{
		...
		if (Parent)
		{
			...
			NewParticle->SetInertiaConditioningEnabled(Parent->InertiaConditioningEnabled());
			NewParticle->SetCCDEnabled(Parent->CCDEnabled());
			NewParticle->SetMACDEnabled(Parent->MACDEnabled());
			NewParticle->SetOneWayInteraction(Parent->OneWayInteraction());
			NewParticle->SetLinearEtherDrag(Parent->LinearEtherDrag());
			NewParticle->SetAngularEtherDrag(Parent->AngularEtherDrag());
			NewParticle->SetInitialOverlapDepenetrationVelocity(Parent->InitialOverlapDepenetrationVelocity());
			NewParticle->SetSleepThresholdMultiplier(Parent->SleepThresholdMultiplier());
			NewParticle->SetIterationSettings(Parent->IterationSettings());
		}

		for (auto& Constituent : Children)
		{
			MEvolution.DoInternalParticleInitilization(Constituent, NewParticle);
		}
	...
	void FRigidClustering::SendBreakingEvent(FPBDRigidClusteredParticleHandle* ClusteredParticle, bool bFromCrumble)
	{
		...
		if (FGeometryCollectionPhysicsProxy* ConcreteProxy = GetConcreteProxy(ClusteredParticle))
		{
			...
			if (SimParams.bGenerateBreakingData || SimParams.bGenerateGlobalBreakingData)
			{
				...
				ClusterBreak.Orientation = ClusteredParticle->GetR();
	...
	void FRigidClustering::RemoveChildFromParent(FPBDRigidParticleHandle* Child, FPBDRigidClusteredParticleHandle* ClusteredParent)
	{
		if (ensure(Child != nullptr && ClusteredParent != nullptr))
		{
			...
			const FVec3f PDiff = Child->GetP() - ClusteredParent->GetP();
			Child->SetVf(Child->GetVf() + ClusteredParent->GetVf() + FVec3f::CrossProduct(ClusteredParent->GetWf(), PDiff));
			Child->SetWf(Child->GetWf() + ClusteredParent->GetWf());
			Child->SetPreVf(Child->GetPreVf() + ClusteredParent->GetPreVf() + FVec3f::CrossProduct(ClusteredParent->GetPreWf(), PDiff));
	...
	{	
		...
		{
			// early out so components that are added from static and skeletal meshes that rely on SingleParticlePhysicsProxy will not fall to pieces immediately!
			// TODO: This means they are currently unbreakable which is currently ok, but we may need them to have damage in the future
			return ActivatedChildren;
		}
		...
		if (ConcreteGCProxy)
		{
			...
			if (GeometryCollectionPhysicsProxy::Private::BuildGeometryForChildrenOnPT(SimParams) == false)
			{
			...
			if (MCollisionImpulseArrayDirty || bPotentialBreak)
			{
				...
				if(GClusterBreakOnlyStrained == 1)
				{
					...
					for (int32 Index = 0; Index < ParticlesToProcess.Num(); Index++)
					{
						...
						{
							NumBreaks++;
							if (NumBreaks >= DestructionSettings.PerAdvanceBreaksAllowed)
							{
					...
					if (DestructionSettings.PerAdvanceBreaksRescheduleLimit > 0)
					{
						// Add back the non processed parent clusters for next tick
						for (int32 Index = 0; Index < DestructionSettings.PerAdvanceBreaksRescheduleLimit; Index++)
						{
	...
	{
		return (DestructionSettings.ClusteringParticleReleaseThrottlingMinCount >= 0 && DestructionSettings.ClusteringParticleReleaseThrottlingMaxCount >= 0);
	}
	...
	void FRigidClustering::ApplySettings(const FChaosSolverConfiguration& Settings)
	{
		SetClusterConnectionFactor(Settings.ClusterConnectionFactor);
		SetClusterUnionConnectionType(ToInternalConnectionMethod(Settings.ClusterUnionConnectionType));
		DestructionSettings.PerAdvanceBreaksAllowed = FMath::Min(Settings.DestructionSettings.PerAdvanceBreaksAllowed, GPerAdvanceBreaksAllowed);
		DestructionSettings.PerAdvanceBreaksRescheduleLimit = FMath::Min(Settings.DestructionSettings.PerAdvanceBreaksRescheduleLimit, GPerAdvanceBreaksRescheduleLimit);
		DestructionSettings.ClusteringParticleReleaseThrottlingMinCount = FMath::Max(Settings.DestructionSettings.ClusteringParticleReleaseThrottlingMinCount, ClusteringParticleReleaseThrottlingMinCount);
		DestructionSettings.ClusteringParticleReleaseThrottlingMaxCount = FMath::Max(Settings.DestructionSettings.ClusteringParticleReleaseThrottlingMaxCount, ClusteringParticleReleaseThrottlingMaxCount);
	}
