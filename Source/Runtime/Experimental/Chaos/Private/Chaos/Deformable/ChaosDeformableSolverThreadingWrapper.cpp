...
namespace Chaos::Softs
{
	...
	void FDeformableSolver::FPhysicsThreadAccess::LoadRestartData()
	{
		if (Solver)
		{
			Solver->LoadRestartData();
		}
	}
	...
	void FDeformableSolver::FGameThreadAccess::PushRestartPackage(int32 InFrame, FDeformableDataMap&& InPackage)
	{
		if (Solver)
		{
			Solver->PushRestartPackage(InFrame, MoveTemp(InPackage));
		}	
	}
	...
	TArray<float> FDeformableSolver::FPhysicsThreadAccess::GetParticleMuscleActivation()
	{
		TArray<float> ParticleMuscleActivations;
		ParticleMuscleActivations.Init(-1.f, Solver->Evolution->Particles().Size());
		if (Solver->MuscleActivationConstraints)
		{
			for (int32 MuscleIndex = 0; MuscleIndex < Solver->MuscleActivationConstraints->NumMuscles(); ++MuscleIndex)
			{
				const int32 VertexOffset = Solver->MuscleActivationConstraints->GetMuscleVertexOffset(MuscleIndex);
				const int32 VertexCount = Solver->MuscleActivationConstraints->GetMuscleVertexCount(MuscleIndex);
				for (int32 VertexIdx = VertexOffset; VertexIdx < VertexOffset + VertexCount; ++VertexIdx)
				{
					ParticleMuscleActivations[VertexIdx] = Solver->MuscleActivationConstraints->GetMuscleActivation(MuscleIndex);
				}
			}
		}
		return ParticleMuscleActivations;
	}
}; // Namespace Chaos::Softs