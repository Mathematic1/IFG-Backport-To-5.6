...
void FTriangleMesh::Init(const TConstArrayView<TVec3<int32>>& Elements, const int32 StartIdx, const int32 EndIdx, const bool CullDegenerateElements)
{
	MElements = Elements;
	MStartIdx = 0;
	MNumIndices = 0;
...
void FTriangleMesh::Serialize(FArchive& Ar)
{
	Ar << MElements;
	Ar << MStartIdx;
	Ar << MNumIndices;
	...
	const int32 MaxIters) const
{
	...
	do {
		...
		TArray<bool> Success = Chaos::SmoothProject<T>(Points, MElements, PointNormals, Pos, CandidateTris, CandidateWeights, /*UseFirstFound = */ false);
		double MinDistanceSq = FLT_MAX; // find closest triangle
		for (int32 i = 0; i < Success.Num(); i++)
		{
			if (Success[i])
			{
				FVec3 InterpPos = FVec3(0);
				for (int32 LocalIdx = 0; LocalIdx < 3; ++LocalIdx)
				{
					InterpPos += Points[MElements[CandidateTris[i]][LocalIdx]] * CandidateWeights[i][LocalIdx];
				}
				double DistSq = (Pos - InterpPos).SizeSquared();
	...
	TFunctionRef<bool(const int32 PointIndex, const int32 TriangleIndex)> BroadphaseTest, TArray<TTriangleCollisionPoint<T>>& Result) const
{
	...
	if (MinTriId != INDEX_NONE)
	{
		...
		const TVec3<T> ClosestPoint = FindClosestPointAndBaryOnTriangle(A, B, C, PointPosition, Bary);
		const T DistSq = (PointPosition - ClosestPoint).SizeSquared();
		const TVec3<T> Normal = Triangle.GetNormal();
		...
		CollisionPoint.Normal = Normal;
		CollisionPoint.Phi = FMath::Sqrt(DistSq);
		if (TVec3<T>::DotProduct(Normal, PointPosition - A) > 0) //Point is inside of triangle (normal is pointing inward)
		{
