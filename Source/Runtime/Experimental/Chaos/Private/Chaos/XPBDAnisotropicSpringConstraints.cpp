...
FXPBDAnisotropicEdgeSpringConstraints::FXPBDAnisotropicEdgeSpringConstraints(
	...
	bool SavePreResizedSimPosition3D,
		...
		ParticleCount)
{
	...
	InitFromPatternData(Particles.Size(), bUse3dRestLengths, FaceVertexPatternPositions, TriangleMesh, SavePreResizedSimPosition3D, PreResizedSimPosition3D, MeshScale);
	InitColor(Particles);
}
...
{
	Init(); // Lambda initialization
	InitFromPatternData(Particles.Size(), bUse3dRestLengths, FaceVertexPatternPositions, TriangleMesh, /*SavePreResizedSimPosition3D = */false, /*PreResizedSimPosition3D = */TArray<FVector3f>(), /*MeshScale = */ (FSolverReal)1.);
	InitColor(Particles);
}
...
void FXPBDAnisotropicEdgeSpringConstraints::InitFromPatternData(int32 ParticlesSize, bool bUse3dRestLengths, const TArray<TVec3<FVec2f>>& FaceVertexPatternPositions, const FTriangleMesh& TriangleMesh, bool SavePreResizedSimPosition3D, const TArray<FVector3f>& PreResizedSimPosition3D, FSolverReal MeshScale)
{
	...
	for (int32 ConstraintIdx = 0; ConstraintIdx < Constraints.Num(); ++ConstraintIdx)
	{
		...
		if (!bUse3dRestLengths)
		{
			if (SavePreResizedSimPosition3D && (ParticlesSize == PreResizedSimPosition3D.Num()))
			{
				const FSolverReal Denom = MeshScale * (PreResizedSimPosition3D[Constraints[ConstraintIdx][0]] - PreResizedSimPosition3D[Constraints[ConstraintIdx][1]]).Size();
				const FSolverReal Scale = Denom > UE_SMALL_NUMBER ? Dists[ConstraintIdx] / Denom : 1;
				BaseDists[ConstraintIdx] *= Scale;
			}
...
void FXPBDAnisotropicAxialSpringConstraints::InitFromPatternData(int32 ParticlesSize, bool bUse3dRestLengths, const TArray<TVec3<FVec2f>>& FaceVertexPatternPositions, const FTriangleMesh& TriangleMesh, bool SavePreResizedSimPosition3D, const TArray<FVector3f>& PreResizedSimPosition3D, FSolverReal MeshScale)
{
	if (bUse3dRestLengths)
	{
		BaseDists = Dists; // Use Dists calculated by base class using 3D positions
	...
	for (int32 ConstraintIdx = 0; ConstraintIdx < Constraints.Num(); ++ConstraintIdx)
	{
		...
		const FSolverVec2 UVDiff = GetSpringDiff(UV1, UV2, UV3, Barys[ConstraintIdx]);
		const FSolverVec2 UVDiffAbs = UVDiff.GetAbs();
		FSolverVec3 UVDiffTransformed;
		if (UVDiffAbs.X > UVDiffAbs.Y)
		{
		...
		if (!bUse3dRestLengths)
		{
			...
			if (SavePreResizedSimPosition3D && (ParticlesSize == PreResizedSimPosition3D.Num()))
			{
				const FSolverReal Denom = MeshScale * GetSpringDiff(
					PreResizedSimPosition3D[Constraint[0]], 
					PreResizedSimPosition3D[Constraint[1]], 
					PreResizedSimPosition3D[Constraint[2]], 
					Barys[ConstraintIdx]).Size();
				const FSolverReal Scale = Denom > UE_SMALL_NUMBER ? Dists[ConstraintIdx] / Denom : 1;
				BaseDists[ConstraintIdx] *= Scale;
			}
			Dists[ConstraintIdx] = BaseDists[ConstraintIdx];
