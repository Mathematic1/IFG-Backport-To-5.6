	...
	*/
	bool FChaosResultsChannel::AdvanceResult()
	{
		if(FPullPhysicsData* PotentialNext = ResultsManager.PopPullData_External(ChannelIdx))
		{
			...
			// Move any non applied DirtyRigidErrors from previous to next
			if (Results.Prev->DirtyRigidErrors.Num() > 0)
			{
				Results.Next->DirtyRigidErrors.Append(Results.Prev->DirtyRigidErrors);
			}
