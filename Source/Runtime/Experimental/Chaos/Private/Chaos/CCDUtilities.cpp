...
namespace Chaos
{
	...
	void FCCDManager::ApplyCCD(const FReal Dt, TArrayView<FPBDCollisionConstraint* const> SweptConstraints, const int32 NumDynamicParticles)
	{
		if (SweptConstraints.Num() > 0)
		{
			ApplySweptConstraints(Dt, SweptConstraints, NumDynamicParticles);
			UpdateSweptConstraints(Dt);
			OverwriteXUsingV(Dt);
		}
	}

	void FCCDManager::ApplySweptConstraints(const FReal Dt, TArrayView<FPBDCollisionConstraint* const> SweptConstraints, const int32 NumDynamicParticles)
	{
		const bool bNeedCCDSolve = Init(Dt, SweptConstraints, NumDynamicParticles);
		if (!bNeedCCDSolve)
		{
			return;
		}
	...
	bool FCCDManager::Init(const FReal Dt, TArrayView<FPBDCollisionConstraint* const> SweptConstraints, const int32 NumDynamicParticles)
	{
		// We store pointers to CCDParticle in CCDConstraint and GroupedCCDParticles, therefore we need to make sure to reserve enough space 
		// for TArray CCDParticles so that reallocation does not happen in the for loop. If reallocation happens, some pointers may become 
		// invalid and this could cause serious bugs. We know that the number of CCDParticles cannot exceed SweptConstraints.Num() * 2 or NumDynamicParticles. 
		// Therefore this makes sure reallocation does not happen.
		CCDParticles.Reset();
		...
		for (FPBDCollisionConstraint* Constraint : SweptConstraints)
		{
			...
			// Create CCDParticle for all dynamic particles affected by swept constraints (UseCCD() could be either true or false). 
			// For static or kinematic particles, this pointer remains to be nullptr.
			...
			for (int i = 0; i < 2; i++)
			{
				...
				if (IsParticleDynamic)
				{
					...
					if (!FoundCCDParticle)
					{
						// See comments above - we cannot allow CCDParticles to resize
						// @todo(chaos): clean this up - use indices, chunked array, or something
						check(CCDParticles.Num() < CCDParticles.Max());
				...
				if (RigidParticle)
				{
					// One can also use P - X for dynamic particles. But notice that for kinematic particles, 
					// both P and X are end-frame positions and P - X won't work for kinematic particles.
	...
	void FCCDManager::UpdateSweptConstraints(const FReal Dt)
	{
	...
	void FCCDManager::ProjectCCD(const FReal Dt, TArrayView<FPBDCollisionConstraint* const> SweptConstraints)
	{
		// NOTE: The SweptConstraints list may be different that the one used in ApplyCCD
		// earlier in the frame but that's ok - we still want to enforce depenetration.
		// - some may be removed (or nulled) if particles were destroyed
		// - some may be added if an island was awakened
		TArray<FPBDCollisionConstraint*> Constraints = TArray<FPBDCollisionConstraint*>(SweptConstraints);

		// Remove null constraints in the active list. Also check for null pointers (which is probably not required 
		// any more - we used to just null the particle pointers in collisions and let them auto-clean up later
		// but now we destroy them immediately).
		if(Chaos::CVars::bClearNullCCDConstraintsOnApply)
		{
			Constraints.RemoveAll(
				[](FPBDCollisionConstraint* C)
				{
					return (C == nullptr)
						|| (C->GetParticle0() == nullptr)
						|| (C->GetParticle1() == nullptr);
				}
			);
		}
		...
		//
		// Sort order based on the two particle types in the collision:
		//		CCD - Dynamic
		//		CCD - CCD
		//		CCD - Kinematic
