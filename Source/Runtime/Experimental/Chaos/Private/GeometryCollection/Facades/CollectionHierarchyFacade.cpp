...
namespace Chaos::Facades
{
	...
	int32 FCollectionHierarchyFacade::GetRootIndex() const
	{
		if (IsValid())
		{
			...
			for (int32 TransformIndex = 0; TransformIndex < Parents.Num(); ++TransformIndex)
			{
			...
			ensureMsgf(Parents.Num() == 0, TEXT("Non-empty collection has invalid hierarchy (no root found)"));
		}
		return INDEX_NONE;
	}
