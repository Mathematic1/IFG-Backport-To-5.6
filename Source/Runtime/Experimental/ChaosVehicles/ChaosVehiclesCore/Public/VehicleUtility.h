...
namespace Chaos
{
	...
	public:
		...
		UE_API void Add(const FVec2& Value);
	...
	{
	public:
		UE_API FTimeAndDistanceMeasure(const FString& DescriptionIn, float InitialVelocityIn, float TargetVelocityIn, float TargetDistanceIn);

		UE_API void Reset();

		bool IsComplete() const { return MeasurementComplete; }

		UE_API void Update(float DeltaTime, const FVector& CurrentLocation, float CurrentVelocity);
