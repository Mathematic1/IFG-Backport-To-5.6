...
void UEnvQueryGenerator_OnCircle::GenerateItems(FEnvQueryInstance& QueryInstance) const
{
	...
	ArcDirectionOffsetDegrees.BindData(QueryOwner, QueryInstance.QueryID);

	const float AngleDegree = ArcAngle.GetValue();
	const float RadiusValue = CircleRadius.GetValue();
	const float ItemSpace = SpaceBetween.GetValue();
	const int32 NumPoints = NumberOfPoints.GetValue();
	const float DirectionOffsetDegrees = ArcDirectionOffsetDegrees.GetValue();
	...
	StartDirection = StartDirection.RotateAngleAxis(DirectionOffsetDegrees - AngleDegree/2, FVector::UpVector) * RadiusValue;
