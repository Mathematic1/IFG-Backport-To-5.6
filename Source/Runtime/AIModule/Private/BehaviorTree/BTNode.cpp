...
void UBTNode::PreSave(FObjectPreSaveContext SaveContext)
{
	if (const UBlackboardData* Blackboard = GetBlackboardAsset())
	{
		for (TFieldIterator<FStructProperty> StructPropIt(GetClass()); StructPropIt; ++StructPropIt)
		{
			if (StructPropIt->Struct && StructPropIt->Struct->IsChildOf(FValueOrBlackboardKeyBase::StaticStruct()))
			{
				if (FValueOrBlackboardKeyBase* ValueOrBBKey = StructPropIt->ContainerPtrToValuePtr<FValueOrBlackboardKeyBase>(this))
				{
					ValueOrBBKey->PreSave(this, *Blackboard, StructPropIt->GetFName());
				}
			}
			else if (StructPropIt->Struct && StructPropIt->Struct->IsChildOf(FBlackboardKeySelector::StaticStruct()))
			{
				if (FBlackboardKeySelector* KeySelector = StructPropIt->ContainerPtrToValuePtr<FBlackboardKeySelector>(this))
				{
					KeySelector->PreSave(this, *Blackboard, StructPropIt->GetFName());
				}
			}
		}
	}
...
const UObject* UBTNode::GetAssociatedAsset(TOptional<FBehaviorTreeNodeDebugContext> DebugContext) const
{
	// By default we return the blueprint the node is derived from. If the node is not a blueprint it will return nullptr.
	return UBlueprint::GetBlueprintFromClass(GetClass());
}
