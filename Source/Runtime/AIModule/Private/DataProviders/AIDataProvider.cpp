...
void FAIDataProviderValue::BindData(const UObject* Owner, int32 RequestId) const
{
	if (DataBinding && ensure(Owner))
	{
		...
		if (!DataField.IsNone())
		{
			FProperty* Prop = DataBinding->GetClass()->FindPropertyByName(DataField);
			if (ensureAlwaysMsgf(Prop != nullptr && IsMatchingType(Prop)
				, TEXT("We expect UEnvQueryNode::PostEditChangeProperty to prevent copying an incompatible provider value on top of an existing one.")))
			{
				CachedProperty = Prop;
			}
		}
