...
DEFINE_LOG_CATEGORY_STATIC(LogInteractiveToolsTransformProxy, Log, All);
...
TUniquePtr<FToolCommandChange> FTransformProxyChangeSource::EndChange()
{
	if (Proxy.IsValid())
	{
		...
		if (!ActiveChange)
		{
			// One way this error could occur is if the Proxy call forced a re-entrant call to EndChange() by destroying
			// the gizmo which in turn will ForceTerminateSource() and end an ActiveChange.
			UE_LOG(LogInteractiveToolsTransformProxy, Error, TEXT("FTransformProxyChangeSource: Re-entrant EndChange invalidated ActiveChange. Did you delete a gizmo on EndPivotEditSequence/EndTransformEditSequence?"));
			return TUniquePtr<FToolCommandChange>();
		}
