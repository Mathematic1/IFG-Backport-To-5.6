...
void UInputBehaviorSet::CollectWantsForceEndCapture(TArray<UInputBehavior*>& ResultOut)
{
	for (const FBehaviorInfo& BehaviorInfo : Behaviors)
	{
		if (UInputBehavior* const Behavior = BehaviorInfo.Behavior)
		{
			if (Behavior->WantsForceEndCapture())
			{
				ResultOut.Add(Behavior);
			}
		}
	}
}
