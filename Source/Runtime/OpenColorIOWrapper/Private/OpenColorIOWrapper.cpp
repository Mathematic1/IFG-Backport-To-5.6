		...
		if (NewConfig != nullptr)
		{
			if (InOption == WCS_AsInterchangeSpace)
			{
				...
				if (InterchangeCS != nullptr && NewConfig->getColorSpace(AnsiWorkingColorSpaceName.Get()) == nullptr)
				{
					...
					WorkingCS->setDescription("The global engine working color space, as defined in project settings (defaults to sRGB/Rec.709).");
...
FString FOpenColorIOWrapperConfig::GetColorSpaceDescription(const TCHAR* InColorSpaceName) const
{
	if (IsValid())
	{
		OCIO_NAMESPACE::ConstColorSpaceRcPtr ColorSpace = Pimpl->Config->getColorSpace(StringCast<ANSICHAR>(InColorSpaceName).Get());
		...
		if (ColorSpace != nullptr)
		{
			return StringCast<TCHAR>(ColorSpace->getDescription()).Get();
		}
	}
...
FString FOpenColorIOWrapperConfig::GetDisplayViewDescription(const TCHAR* InDisplayName, const TCHAR* InViewName) const
{
	if (IsValid())
	{
		const char* DisplayViewDescription = Pimpl->Config->getDisplayViewDescription(StringCast<ANSICHAR>(InDisplayName).Get(), StringCast<ANSICHAR>(InViewName).Get());

		return StringCast<TCHAR>(DisplayViewDescription).Get();
	}

	return {};
}

FString FOpenColorIOWrapperConfig::GetDisplayViewTransformDescription(const TCHAR* InDisplayName, const TCHAR* InViewName) const
{
	if (IsValid())
	{
		const char* TransformName = Pimpl->Config->getDisplayViewTransformName(StringCast<ANSICHAR>(InDisplayName).Get(), StringCast<ANSICHAR>(InViewName).Get());

		OCIO_NAMESPACE::ConstViewTransformRcPtr ViewTransform = Pimpl->Config->getViewTransform(TransformName);
		
		if (ViewTransform != nullptr)
		{
			const char* TransformDescription = ViewTransform->getDescription();

			return StringCast<TCHAR>(TransformDescription).Get();
		}
	}
	...
	, GPUPimpl(MakePimpl<FOpenColorIOGPUProcessorPimpl, EPimplPtrMode::DeepCopy>())
{
	...
	OCIO_EXCEPTION_HANDLING_TRY();
		...
		if (ParentProcessor.IsValid())
		{
			...
			ShaderDescription->setLanguage(GPU_LANGUAGE_HLSL_SM_5_0);
