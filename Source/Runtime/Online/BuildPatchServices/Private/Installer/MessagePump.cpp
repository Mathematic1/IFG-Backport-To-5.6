...
namespace BuildPatchServices
{
	...
	void FMessagePump::SendMessage(FGenericMessage Message)
	{
	...
	bool FMessagePump::SendRequest(FChunkUriRequest Request, TFunction<void(FChunkUriResponse)> OnResponse)
	{
	...
	void FMessagePump::PumpMessages()
	{
		...
		while (MessageQueue.Dequeue(MessageUnion))
		{
			...
			else if (MessagePumpHelpers::TryPump<FGenericMessage>(Handlers, MessageUnion))
			{
				continue;
			}
		}
