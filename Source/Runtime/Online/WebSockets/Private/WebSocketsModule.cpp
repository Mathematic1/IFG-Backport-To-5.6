...
void FWebSocketsModule::StartupModule()
{
	...
	// Subscrive to config changes so we can forward to WebSocketsManager  
	FCoreDelegates::TSOnConfigSectionsChanged().AddRaw(this, &FWebSocketsModule::OnConfigSectionsChanged);
#endif
}
...
#if WITH_WEBSOCKETS
void FWebSocketsModule::OnConfigSectionsChanged(const FString& IniFilename, const TSet<FString>& SectionNames)
{
	if (WebSocketsManager)
	{
		WebSocketsManager->UpdateConfigs();
	}
}
#endif
