...
void* FGPUDefragAllocator::Allocate(int64 AllocationSize, int32 Alignment, TStatId InStat, bool bAllowFailure)
{
	...
	if (!BestChunk)
	{
		if (!bAllowFailure)
		{
			...
			DumpAllocs();
			UE_LOG(LogRHI, Fatal, TEXT("Ran out of memory for allocation in best-fit allocator of size %" INT64_FMT " KByte"), AllocationSize / 1024);
#endif
		}
		return nullptr;
	}
...
void FGPUDefragAllocator::SetUserPayload(const void* Pointer, void* UserPayload)
{
	FScopeLock Lock(&SynchronizationObject);
	FMemoryChunk* MatchingChunk = PointerToChunkMap.FindRef(Pointer);
	checkf(MatchingChunk, TEXT("Couldn't find matching chunk for %p"), Pointer);
	if (MatchingChunk)
	{
