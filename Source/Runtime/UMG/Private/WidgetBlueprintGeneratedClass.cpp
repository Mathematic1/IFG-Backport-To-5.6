	...
	, const TArrayView<const FDelegateRuntimeBinding> InBindings)
{
	...
	if (CreatedWidgetTree)
	{
		...
		// Support navigation rules set on dynamically created user widgets by resolving the navigation rules against itself
		// if this widget is pre-placed in another widget, the navigation rules will be resolved afterward against the outer widget below in CreatedWidgetTree->ForEachWidget
		if (UserWidget->Navigation)
		{
			UserWidget->Navigation->ResolveRules(UserWidget, CreatedWidgetTree);
		}
