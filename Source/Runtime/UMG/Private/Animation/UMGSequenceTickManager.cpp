...
// Array of widgets who started an animation while we were updating animated widgets.
// Implemented as a static member to prevent modifications of the class layout for a 5.6.1 hotfix.
// This is acceptable since UUMGSequenceTickManager is supposed to be a singleton.
static TArray<TWeakObjectPtr<UUserWidget>> PendingWeakUserWidgetData;
...
void UUMGSequenceTickManager::AddWidget(UUserWidget* InWidget)
{
	...
	{
		PendingWeakUserWidgetData.Add(WeakWidget);
	}
	else
	{
...
void UUMGSequenceTickManager::OnWidgetTicked(UUserWidget* InWidget)
{
	...
	{
		PendingWeakUserWidgetData.Add(InWidget);
	}
	else
	{
...
void UUMGSequenceTickManager::TickWidgetAnimations(float DeltaSeconds)
{
	...
	if (PendingWeakUserWidgetData.Num() > 0)
	{
		for (TWeakObjectPtr<UUserWidget> WeakWidget : PendingWeakUserWidgetData)
		{
			WeakUserWidgetData.Add(WeakWidget, FSequenceTickManagerWidgetData());
		}
		PendingWeakUserWidgetData.Reset();
	}
