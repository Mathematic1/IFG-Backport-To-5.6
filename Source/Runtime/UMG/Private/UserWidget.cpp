...
void UUserWidget::TickActionsAndAnimation(float InDeltaTime)
{
	...
	// starting another animation. So iterate with indices always compared 
	// to the new array size, and not an iterator.
	for (int32 Index = 0; Index < ActiveAnimations.GetMaxIndex(); ++Index)
	{
		if (ActiveAnimations.IsAllocated(Index))
		{
			FWidgetAnimationState& State = ActiveAnimations[Index];
