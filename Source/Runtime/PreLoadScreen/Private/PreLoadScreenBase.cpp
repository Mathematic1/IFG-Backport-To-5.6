...
{	
    ...
    if (ensureAlwaysMsgf(Config, TEXT("Unable to find .ini file for %s"), *ConfigFileName))
    {
    	...
    	//Parse time to update the downloaded data size
    	float TimeToUpdateDownloadedDataSize = 0.0f;
    	Config->GetFloat(*UIConfigSection, TEXT("TimeToUpdateDownloadedDataSize"), TimeToUpdateDownloadedDataSize);
    	FPreLoadSettingsContainerBase::Get().TimeToUpdateDownloadedDataSize = TimeToUpdateDownloadedDataSize;
