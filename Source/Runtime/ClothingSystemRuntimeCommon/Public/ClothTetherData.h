...
struct FClothTetherData
{
	...
	CLOTHINGSYSTEMRUNTIMECOMMON_API void GenerateTethers(
		...
		bool bUseGeodesicDistance);  // Whether to use geodesic (walking along the surface) or euclidean (beeline) distances to find the tethers.
	
	/**
	 * Generate the tethers by following the triangle mesh network from the closest kinematic tether end to each dynamic point.
	 * Inside array items can be processed concurrently, but the outside array must be iterated on sequentially.
	 */
	CLOTHINGSYSTEMRUNTIMECOMMON_API void GenerateTethers(
		const TConstArrayView<FVector3f>& Points,  // Reference pose
		const TConstArrayView<uint32>& Indices,  // Triangle mesh
		const TConstArrayView<float>& MaxDistances,  // Mask for sorting the kinematic from the dynamic points
		const TConstArrayView<float>& TetherEnds,  // Separate mask for specifying tether ends (using same < 0.1 cutoff as MaxDistances). Actual tether end set is the intersection of tether ends and kinematic points.
		bool bUseGeodesicDistance);  // Whether to use geodesic (walking along the surface) or euclidean (beeline) distances to find the tethers.
