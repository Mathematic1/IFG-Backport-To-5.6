...
struct FLumenCVarsState
{
	FLumenCVarsState()
	{
		...
		// No need to register to changes : all primitive components are invalidated when this CVar dynamically changes so render states of landscape components get appropriately re-created
