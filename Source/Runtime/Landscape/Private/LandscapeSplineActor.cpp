...
// Is this in a valid state to be safe to call LandscapeActorToWorld?
bool ALandscapeSplineActor::IsSplineOwnerValid() const
{
	ULandscapeInfo* Info = GetLandscapeInfo();
	return Info != nullptr && Info->LandscapeActor != nullptr;
}
...
bool ALandscapeSplineActor::ShouldPartitionSpline() const
{
	return !IsEditorOnly() && bIsSpatiallyLoaded;
}

bool ALandscapeSplineActor::HasGeneratedLandscapeSplineMeshesActors() const
{
	return ShouldPartitionSpline() && IsValid(LandscapeActor) && LandscapeActor->GetUseGeneratedLandscapeSplineMeshesActors();
}
